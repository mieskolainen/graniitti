...
Tensor1<std::complex<double>,4> MDirac::EpsSpin1(const M4Vec& k, int m) const {
	...
	// First define in \vec{z}-direction, then rotate spatial part such that \vec{k} = R \vec{z}
	...
	// \eps^{\mu,-1}
	if (m == -1) {
		//e = {0.0,  (e1[0] + zi*e2[0]) / msqrt(2.0),  (e1[1] + zi*e2[1]) / msqrt(2.0), 0.0};
...
//    representation. Invidual sub-amplitudes are gauge dependent, their sum is not.
// 3. Helicity amplitudes do not interfere, sum incoherently \sum_{config} |M_i|^2
//
//
// --------------------------------------------------------------------
// Conventions:
...
// Polarization vector eps^{(m)\mu}(k) for massive spin-1 with helicity m = -1,0,1
//
...
// Polarization tensor eps^{(m)\mu\nu}(k) for massive spin-2 with helicity m = -2,-1,0,1,2
// 
// [t,x,y,z] convention!
//
// Should obey: (eps_{\mu\nu}^{(m)}(k))^* eps^{(n)\mu\nu}(k) ) = \delta_{mn}
...
Tensor2<std::complex<double>,4,4> MDirac::EpsMassiveSpin2(const M4Vec& k, int m) const {
	...
	for (const auto& nu : LI) {
		...
		for (const int& m2 : {-1,0,1}) {
			...
			// ClebschGordan(double j1, double j2, double m1, double m2, double j, double m)
...
std::vector<std::complex<double>> MDirac::Bar(const std::vector<std::complex<double>>& spinor) const {
...
// Feynman slash matrix operator: \slash{a} = \gamma_\mu a^\mu = \gamma^\mu a_\mu
//
...
// ----------------------------------------------------------------------
// High Energy (E >> m) relation for the vertex current when p = p' (e.g. forward scattering)
//
...
bool MDirac::SpinorHELimit(const M4Vec& pi, const M4Vec& pf) const {
	...
	for (const auto& hi : {1,2}) {
		...
		const std::vector<std::complex<double>> ubar = Bar(uDirac(pf, hf));
			...
			if (hi != hf) { std::cout << std::endl; continue; } // Helicity conservation
			...
			for (const auto& mu : LI) {
				for (const auto& nu : LI) {
					...
					printf(" hi = %2d, hf = %2d | (mu,nu) = (%lu,%lu) | Re[lhs] = %0.3E \t rhs = %0.3E \t |lhs| / |rhs| = %0.5f \n",
...
// Test gamma matrix anticommutation relation {\gamma^\mu, \gamma^nu} = 2 g^{\mu\nu} I_4
bool MDirac::GammaAntiCommutation() const {

	for (const auto& mu : LI) {
		for (const auto& nu : LI) {
			const MMatrix<std::complex<double>> AC_lo = gamma_lo[mu] * gamma_lo[nu] + gamma_lo[nu] * gamma_lo[mu];
			std::cout << "gamma_lo:: mu:" << mu << " nu: " << nu << std::endl;
			AC_lo.Print();

			const MMatrix<std::complex<double>> AC_up = gamma_up[mu] * gamma_up[nu] + gamma_up[nu] * gamma_up[mu];
...
// 
// 
bool MDirac::DiracSpinorComplete(const M4Vec& p, const std::string& type, const std::string& basis) const {
	...
	for (const auto& lambda : SPINORSTATE) {
		...
		// Check normalization
		const double nlhs = std::real( gra::matoper::VecVecMultiply(spinorbar, spinor) ); // Take real to cast to double
		const double nrhs = SIGN * 2.*p.M();

		printf("s = %2d : Normalization = ", lambda);
		if (gra::aux::AssertRatio(nlhs, nrhs, 5E-3)) {
			std::cout << rang::fg::green << nlhs << " | 2 x p.M = " << nrhs << " OK"     << rang::fg::reset << std::endl;
		} else {
			std::cout << rang::fg::red   << nlhs << " | 2 x p.M = " << nrhs << " NOT OK" << rang::fg::reset << std::endl;
		}
	...
	// Compare
	lhs.Print("sum_{s}{spinor_s(p) barspinor_s(p)}"); std::cout << std::endl;
	rhs.Print("slash{p} + I4*m*SIGN");                std::cout << std::endl;

	const double frobnorm = (lhs-rhs).FrobNorm();
	if (frobnorm < 0.1) {
		std::cout << rang::fg::green << "Completeness relation OK, Frobenius norm = "     << frobnorm << rang::fg::reset << std::endl;
	} else {
		std::cout << rang::fg::red   << "Completeness relation NOT OK, Frobenius norm = " << frobnorm << rang::fg::reset << std::endl;
	}
	std::cout << std::endl;
