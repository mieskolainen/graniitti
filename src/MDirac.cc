...
void MDirac::InitGammaMatrices(const std::string &basis) {
  ...
  if (basis == "DIRAC") {
    ...
    throw std::invalid_argument(
        "MDirac::InitGammaMatrices: Unknown gamma basis chosen: " + basis);
  }
  ...
  MMatrix<std::complex<double>> y1_up{
      std::vector<std::complex<double>>{0.0, 0.0, 0.0, 1.0},
      std::vector<std::complex<double>>{0.0, 0.0, 1.0, 0.0},
      std::vector<std::complex<double>>{0.0, -1.0, 0.0, 0.0},
      std::vector<std::complex<double>>{-1.0, 0.0, 0.0, 0.0}};

  MMatrix<std::complex<double>> y2_up{
      std::vector<std::complex<double>>{0.0, 0.0, 0.0, -zi},
  ...
  MMatrix<std::complex<double>> y3_up{
      std::vector<std::complex<double>>{0.0, 0.0, 1.0, 0.0},
      std::vector<std::complex<double>>{0.0, 0.0, 0.0, -1.0},
      std::vector<std::complex<double>>{-1.0, 0.0, 0.0, 0.0},
      std::vector<std::complex<double>>{0.0, 1.0, 0.0, 0.0}};
  ...
  sigma_up = std::vector<std::vector<MMatrix<std::complex<double>>>>(
      4, std::vector<MMatrix<std::complex<double>>>(
             4, MMatrix<std::complex<double>>(4, 4)));
...
//
std::vector<std::complex<double>> MDirac::XiSpinor(const M4Vec &p,
                                                   int helicity) const {
...
//
std::vector<std::complex<double>> MDirac::uHelChiral(const M4Vec &p,
                                                     int helicity) const {
  if (BASIS != "C") {
  ...
  const double N =
      1.0 / (std::sqrt(2.0) * msqrt(E + m)); // Volume normalization to 2E

  switch (helicity) {
  case 1:
    return {c * neg * N, s * phase * neg * N, c * pos * N, s * phase * pos * N};
  case -1:
    return {-s * pos * N, c * phase * pos * N, -s * neg * N,
            c * phase * neg * N};
  }
...
// <@@ DEFINED IN CHIRAL GAMMA MATRIX REPRESENTATION @@>
//
std::vector<std::complex<double>> MDirac::vHelChiral(const M4Vec &p,
                                                     int helicity) const {
  if (BASIS != "C") {
...
//
std::vector<std::complex<double>> MDirac::uHelDirac(const M4Vec &p,
                                                    int helicity) const {
  if (BASIS != "D") {
  ...
  switch (helicity) {
  case 1:
    return {N * c, N * phase * s, N * p3 / (E + m) * c,
            N * p3 / (E + m) * phase * s};
  case -1:
    return {-N * s, N * phase * c, N * p3 / (E + m) * s,
            -N * p3 / (E + m) * phase * c};
  }
...
// <@@ DEFINED IN DIRAC GAMMA-MATRIX REPRESENTATION @@>
//
std::vector<std::complex<double>> MDirac::vHelDirac(const M4Vec &p,
                                                    int helicity) const {
  if (BASIS != "D") {
...
// <@@ DEFINED IN DIRAC GAMMA MATRIX REPRESENTATION @@>
//
std::vector<std::complex<double>> MDirac::uDirac(const M4Vec &p,
                                                 int spin) const {
  if (BASIS != "D") {
  ...
  if (spin != -1 && spin != 1) {
    throw std::invalid_argument(
        "MDirac::uDirac: spin state argument != -1 or 1");
  }
  const double E = p.E();
  const double m = p.M();
  ...
  if (spin == 1) {
    ...
    return {0.0, N * 1.0, N * (p[1] - zi * p[2]) / (E + m),
            N * (-p[3] / (E + m))};
  }
}
...
// <@@ DEFINED IN DIRAC GAMMA MATRIX REPRESENTATION @@>
//
std::vector<std::complex<double>> MDirac::vDirac(const M4Vec &p,
                                                 int spin) const {
  if (BASIS != "D") {
  ...
  if (!(spin == -1 || spin == 1)) {
    throw std::invalid_argument(
        "MDirac::vDirac: spin state argument != -1 or 1");
  }
...
// [t,x,y,z] order convention!
//
Tensor1<std::complex<double>, 4> MDirac::EpsMassiveSpin1(const M4Vec &k,
                                                         int m) const {
  ...
  if (m != 0) {
    throw std::invalid_argument(
        "MDirac::EpsMassiveSpin1: helicity m should be -1, 0 or 1");
  }
...
// Should obey: (eps_{\mu\nu}^{(m)}(k))^* eps^{(n)\mu\nu}(k) ) = \delta_{mn}
//
Tensor2<std::complex<double>, 4, 4> MDirac::EpsMassiveSpin2(const M4Vec &k,
                                                            int m) const {
  if (!(m == -2 || m == -1 || m == 0 || m == 1 || m == 2)) {
    throw std::invalid_argument(
        "MDirac::EpsMassiveSpin2: m is not -2,-1,0,1,2");
  }
...
MDirac::Bar(const std::vector<std::complex<double>> &spinor) const {
  ...
  return gra::matoper::VecMatMultiply(gra::matoper::VecDagger(spinor),
                                      gamma_up[0]);
}
...
bool MDirac::SpinorHELimit(const M4Vec &pi, const M4Vec &pf) const {
  ...
  for (const auto &hi : {1, 2}) {
    ...
    for (const auto &hf : {1, 2}) {
      ...
      for (const auto &mu : LI) {
        for (const auto &nu : LI) {
          const std::vector<std::complex<double>> prod =
              (gamma_up[mu] * (psum % nu)) * u;

          const std::complex<double> lhs =
              gra::matoper::VecVecMultiply(ubar, prod);
...
std::complex<double> MDirac::sProd(const M4Vec &p1, const M4Vec &p2,
                                   int helicity) const {
  return gra::matoper::VecVecMultiply(Bar(uGauge(p1, helicity)),
                                      uGauge(p2, -helicity));
}

// Helicity u-spinor via massless gauge vector
//
std::vector<std::complex<double>> MDirac::uGauge(const M4Vec &p,
                                                 int helicity) const {
  const M4Vec l(100, 0, 0, 100);
  return ((FSlash(p) + I4 * p.M()) / msqrt(2.0 * (p * l))) *
         uHelDirac(l, -helicity);
}

// Helicity u-spinor via massless gauge vector
//
std::vector<std::complex<double>> MDirac::vGauge(const M4Vec &p,
                                                 int helicity) const {
  const M4Vec l(100, 0, 0, 100);
  return (-(FSlash(p) - I4 * p.M()) / msqrt(2.0 * (p * l))) *
         vHelDirac(l, -helicity);
}
...
bool MDirac::DiracSpinorComplete(const M4Vec &p, const std::string &type,
  ...
  for (const auto &lambda : SPINORSTATE) {
    ...
    if (type == "u") {
      ...
      throw std::invalid_argument(
          "DiracSpinorComplete: Unknown type (set u or v)");
    }
    // Adjoint
    const std::vector<std::complex<double>> spinorbar = Bar(spinor);

    // Check normalization
    const double nlhs = std::real(gra::matoper::VecVecMultiply(
        spinorbar, spinor)); // Take real to cast to double
  ...
  if (frobnorm < 0.1) {
    std::cout << rang::fg::green
              << "Completeness relation OK, Frobenius norm = " << frobnorm
              << rang::fg::reset << std::endl;
  } else {
    std::cout << rang::fg::red
              << "Completeness relation NOT OK, Frobenius norm = " << frobnorm
              << rang::fg::reset << std::endl;
  }
  std::cout << std::endl;
