...
void InitTMatrix(gra::HELMatrix &hc, const gra::MParticle &p,
  ...
  const bool identical =
      (p1.pdg == p2.pdg) ? true : false; // e.g. pi0 pi0 vs pi+pi-

  // -------------------------------------------------------------------
  // Helicity decay amplitude matrix

  hc.T =
      MMatrix<std::complex<double>>(static_cast<unsigned int>(2 * s1 + 1),
  ...
  std::cout << "gra::spin:: "
            ...
            << " , Particle 2: " << (is_boson2 ? "boson" : "fermion")
            << std::endl;

  std::cout << "gra::spin::InitTMatrix: Calculating SU(2) decomposition: "
            << std::endl;
      ...
      for (int i = 0; i < static_cast<int>(2 * s1 + 1); ++i) {
        for (int j = 0; j < static_cast<int>(2 * s2 + 1); ++j) {
          const double lambda1 =
              -s1 + static_cast<double>(i); // From negative to positive
          const double lambda2 =
              -s2 + static_cast<double>(j); // From negative to positive
          ...
          // <J \lambda | ls0\lambda>
          const double cg1 = gra::spin::ClebschGordan(static_cast<double>(l),
                                                      static_cast<double>(s),
                                                      0.0, lambda, J, lambda);

          // <s \lambda | s1s2\lambda1,-\lambda2>
          const double cg2 = gra::spin::ClebschGordan(
              s1, s2, lambda1, -lambda2, static_cast<double>(s), lambda);
          ...
          if (std::abs(cg1 * cg2) > 1e-15) {
            ...
            printf("[l=%d,s=%d] :: lambda1 = %4.1f, lambda2 = "
                   "%4.1f : cg1*cg2 = %6.3f \n",
  ...
  const std::string str0 =
      "gra::spin::InitTMatrix:: [" + gra::aux::Spin2XtoString(2 * J) + "^" +
  ...
  if (nonzero == 0) {
    const std::string str =
        ": Decay is impossible (spin-parity-statistics) [.P_conservation = " +
  ...
  if (tag_set) {
    ...
    const std::string middle = ", Following helicity decay alpha_ls-couplings "
...
//
std::complex<double> SpinAmp(const gra::LORENTZSCALAR &lts,
  ...
  if (lts.decaytree.size() == 2) {
    ...
    gra::kinematics::LorentzBoost(lts.pfinal[0], lts.pfinal[0].M(),
                                  boosted_daughter, -1);

    fA = fMatrix(resonance.hc.T, resonance.p.spinX2 / 2.0, s1, s2,
  ...
  if (lts.decaytree[B].legs.size() == 2) {
    ...
    const MMatrix<std::complex<double>> T((unsigned int)(2 * s1 + 1),
                                          (unsigned int)(2 * s2 + 1), 1.0);

    // Rotate and boost daughters to the frame where z-spanned by B-flight
    // direction
    // (B-helicity rest frame)
    std::vector<M4Vec> daughter = {lts.decaytree[B].legs[B1].p4,
                                   lts.decaytree[B].legs[B2].p4};
    gra::kinematics::HEframe(daughter);

    fB = fMatrix(T, lts.decaytree[B].p.spinX2 / 2.0, s1, s2,
                 daughter[B1].Theta(), daughter[B1].Phi());
  }
  ...
  if (lts.decaytree[C].legs.size() == 2) {
    ...
    std::vector<M4Vec> daughter = {lts.decaytree[C].legs[C1].p4,
                                   lts.decaytree[C].legs[C2].p4};
    gra::kinematics::HEframe(daughter);

    fC = fMatrix(T, lts.decaytree[C].p.spinX2 / 2.0, s1, s2,
                 daughter[C1].Theta(), daughter[C1].Phi());
  }
  ...
  // rotation.
  const MMatrix<std::complex<double>> D = gra::spin::DMatrix(
      resonance.p.spinX2 / 2.0, theta_rotation, phi_rotation);

  // rho_rot = D*rho*D^dagger
  const MMatrix<std::complex<double>> rho_ROT =
      D * resonance.hc.rho * D.Dagger();
...
MMatrix<std::complex<double>> fMatrix(const MMatrix<std::complex<double>> &T,
  ...
  if (T.size_row() != static_cast<unsigned int>(2 * s1 + 1) ||
    ...
    throw std::invalid_argument("gra::spin::fMatrix: Input T matrix "
                                "(2s1+1)x(2s2+1) with wrong dimensions: " +
                                std::to_string(T.size_row()) + " x " +
                                std::to_string(T.size_col()));
  }
  // Number of final state configurations
  const unsigned int N = static_cast<unsigned int>(2 * s1 + 1) *
                         static_cast<unsigned int>(2 * s2 + 1);
  ...
  for (std::size_t i = 0; i < N; ++i) {
    ...
    for (std::size_t j = 0; j < 2 * J + 1; ++j) {
      ...
      f[i][j] = WignerD(theta, phi, lambda, M[j], J) *
                T[lambda_index[i][0]][lambda_index[i][1]];
    }
  }
  // std::cout << "f:::" << std::endl;
...
MMatrix<std::complex<double>> DMatrix(double J, double theta_rotation,
                                      double phi_rotation) {
...
double ClebschGordan(double j1, double j2, double m1, double m2, double j,
                     double m) {
  // Selection rules
  ...
  return std::pow(-1.0, m + j1 - j2) * msqrt(2 * j + 1) *
         W3j(j1, j2, j, m1, m2, -m);
}
...
double CGrules(double j1, double j2, double m1, double m2, double j, double m) {
  ...
  if (!chalfint(j1) || !chalfint(j2) || !chalfint(j)) {
    if (PRINT_ON)
      printf("CGrules:: (j1,j2,j) not integer or half-integer\n");

    return false;
  }
  if (!cint(j1 + j2 + j)) { // "Integer perimeter rule"
    if (PRINT_ON)
      printf("CGrules:: j1 + j2 + j not integer\n");

    return false;
  }
  if (!chalfint(m1) || !chalfint(m2) || !chalfint(m)) {
    if (PRINT_ON)
      printf("CGrules:: (m1,m2,m) not integer or half-integer\n");

    return false;
  }
  if (!isequal(m1 + m2, m)) {
    if (PRINT_ON)
      printf("CGrules:: m1 + m2 != m\n");

    return false;
  }
  if (!isequal(j1 - m1, std::floor(j1 - m1))) {
    if (PRINT_ON)
      printf("CGrules:: parity of 2j_1 and 2m_1 does not match\n");

    return false;
  }
  if (!isequal(j2 - m2, std::floor(j2 - m2))) {
    if (PRINT_ON)
      printf("CGrules:: parity of 2j_2 and 2m_2 does not match\n");

    return false;
  }
  if (!isequal(j - m, std::floor(j - m))) {
    if (PRINT_ON)
      printf("CGrules:: parity of 2j and 2m does not match\n");

    return false;
  }
  if ((j1 + j2) < j || j < std::abs(j1 - j2)) {
    if (PRINT_ON)
      printf("CGrules:: j over valid domain\n");

    return false;
  }
  if (std::abs(m1) > j1) {
    if (PRINT_ON)
      printf("CGrules:: |m1| > j1\n");

    return false;
  }
  if (std::abs(m2) > j2) {
    if (PRINT_ON)
      printf("CGrules:: |m2| > j2\n");
...
double W3j(double j1, double j2, double j3, double m1, double m2, double m3) {
  ...
  for (int k = lower; k <= upper; ++k) {
    fsum += std::pow(-1.0, k) /
            (gra::math::factorial(k) * gra::math::factorial(c1 + k) *
...
double TriangleCoeff(double j1, double j2, double j3) {
  return gra::math::factorial(j1 + j2 - j3) *
         gra::math::factorial(j1 - j2 + j3) *
         gra::math::factorial(-j1 + j2 + j3) /
         gra::math::factorial(j1 + j2 + j3 + 1);
}

// Selection rules
bool W3jrules(double j1, double j2, double j3, double m1, double m2,
              double m3) {
  const bool PRINT_ON = false;
...
double WignerSmalld(double theta, double m, double mp, double J) {
  const double factor = gra::math::msqrt(
      gra::math::factorial(J + m) * gra::math::factorial(J - m) *
  ...
  if (std::isnan(s)) {
    std::string str =
        "gra::spin::WignerSmalld: NaN value with (theta,m,mp,J) = " +
        std::to_string(theta) + " " + std::to_string(m) + " " +
        std::to_string(mp) + " " + std::to_string(J);
    throw std::invalid_argument(str);
  }
  return factor * s;
}
...
// m, mp, J all in integers (not half-integers)
//
std::complex<double> WignerD(double theta, double phi, double m, double mp,
                             double J) {
  return WignerSmalld(theta, m, mp, J) *
         std::exp(std::complex<double>(0, 1) * mp * phi);
}
...
bool Positivity(const MMatrix<std::complex<double>> &rho, unsigned int J) {
  ...
  if (std::abs(tracerho - 1.0) > 1e-2) {
    std::string str = "gra::spin::Positivity: Density matrix is not properly "
                      "normalized (Tr[rho] = " +
  ...
  if (J == 1) {
    ...
    const double pc1 =
        -4 * b * b - 4 * c * c - 2 * d * d + a - (3 * a * a) / 2.0 + 1 / 2.0;
    const double pc2 = -(3 * (2 * d - a + 1) *
                         (2 * a * d - a + a * a + 4 * b * b + 4 * c * c)) /
                       2.0;
    ...
    if (pc1 >= NEG_EPS && pc2 >= NEG_EPS) {
      ...
      std::string str = "gra::spin::Positivity: Eigenvalues of the density "
                        "matrix are not >= 0 !";
      gra::matoper::PrintMatrixSeparate(rho);
      throw std::invalid_argument(str);
      return false;
  ...
  // Spin-2
    ...
    const double pc1 = -6 * a * a - 8 * a * b + 4 * a - 6 * b * b + 4 * b -
                       4 * c * c - 4 * d * d - 4 * e * e - 4 * f * f -
                       4 * g * g - 4 * h * h - 4 * j * j - 4 * k * k -
                       2 * l * l - 2 * m * m;
    const double pc2 =
        -12 * a * a * a - 48 * a * a * b + 6 * a * a - 48 * a * b * b +
        24 * a * b + 12 * a * c * c - 12 * a * d * d + 12 * a * e * e +
        12 * a * f * f - 12 * a * g * g + 12 * a * h * h - 24 * a * j * j -
        24 * a * k * k + 12 * a * m * m - 12 * b * b * b + 6 * b * b +
        12 * b * c * c - 24 * b * d * d + 12 * b * e * e + 12 * b * f * f -
        24 * b * g * g + 12 * b * h * h - 12 * b * j * j - 12 * b * k * k +
        12 * b * l * l - 12 * c * c + 24 * c * d * j + 24 * c * e * l -
        24 * c * e * m - 24 * c * g * k + 12 * d * d * m - 24 * d * e * j +
        24 * d * f * k - 24 * d * h * k - 12 * e * e + 24 * e * g * k -
        12 * f * f + 24 * f * g * j + 24 * f * h * l - 24 * f * h * m +
        12 * g * g * m - 24 * g * h * j - 12 * h * h - 12 * j * j * l -
        12 * k * k * l - 6 * l * l - 6 * m * m;
    const double pc3 =
        48 * a * a * b - 168 * a * a * b * b - 96 * a * a * a * b +
        96 * a * a * c * c + 96 * a * a * e * e + 96 * a * a * f * f +
        96 * a * a * h * h - 48 * a * a * j * j - 48 * a * a * k * k +
        72 * a * a * l * l - 96 * a * b * b * b + 48 * a * b * b +
        144 * a * b * c * c - 96 * a * b * d * d + 144 * a * b * e * e +
        144 * a * b * f * f - 96 * a * b * g * g + 144 * a * b * h * h -
        96 * a * b * j * j - 96 * a * b * k * k + 96 * a * b * l * l +
        96 * a * b * m * m - 48 * a * c * c + 96 * a * c * d * j -
        96 * a * c * e * l + 192 * a * c * e * m - 96 * a * c * g * k -
        96 * a * d * e * j + 96 * a * d * f * k - 96 * a * d * h * k -
        48 * a * e * e + 96 * a * e * g * k - 48 * a * f * f +
        96 * a * f * g * j - 96 * a * f * h * l + 192 * a * f * h * m -
        96 * a * g * h * j - 48 * a * h * h - 96 * a * j * j * l -
        96 * a * k * k * l - 48 * a * l * l + 96 * b * b * c * c -
        48 * b * b * d * d + 96 * b * b * e * e + 96 * b * b * f * f -
        48 * b * b * g * g + 96 * b * b * h * h + 72 * b * b * m * m -
        48 * b * c * c + 96 * b * c * d * j - 192 * b * c * e * l +
        96 * b * c * e * m - 96 * b * c * g * k + 96 * b * d * d * m -
        96 * b * d * e * j + 96 * b * d * f * k - 96 * b * d * h * k -
        48 * b * e * e + 96 * b * e * g * k - 48 * b * f * f +
        96 * b * f * g * j - 192 * b * f * h * l + 96 * b * f * h * m +
        96 * b * g * g * m - 96 * b * g * h * j - 48 * b * h * h -
        48 * b * m * m + 24 * c * c * c * c + 48 * c * c * d * d -
        48 * c * c * e * e + 48 * c * c * f * f + 48 * c * c * g * g +
        48 * c * c * h * h + 48 * c * c * j * j + 48 * c * c * k * k +
        48 * c * c * l * m + 96 * c * d * d * e + 96 * c * d * j * l -
        96 * c * d * j * m - 192 * c * e * f * h + 96 * c * e * g * g +
        96 * c * e * j * j + 96 * c * e * k * k + 96 * c * e * l -
        96 * c * e * m - 96 * c * g * k * l + 96 * c * g * k * m +
        48 * d * d * e * e + 48 * d * d * f * f + 96 * d * d * f * h +
        48 * d * d * h * h + 48 * d * d * l * l - 96 * d * e * j * l +
        96 * d * e * j * m + 96 * d * f * k * l - 96 * d * f * k * m -
        96 * d * h * k * l + 96 * d * h * k * m + 24 * e * e * e * e +
        48 * e * e * f * f + 48 * e * e * g * g + 48 * e * e * h * h +
        48 * e * e * j * j + 48 * e * e * k * k + 48 * e * e * l * m +
        96 * e * g * k * l - 96 * e * g * k * m + 24 * f * f * f * f +
        48 * f * f * g * g - 48 * f * f * h * h + 48 * f * f * j * j +
        48 * f * f * k * k + 48 * f * f * l * m + 96 * f * g * g * h +
        96 * f * g * j * l - 96 * f * g * j * m + 96 * f * h * j * j +
        96 * f * h * k * k + 96 * f * h * l - 96 * f * h * m +
        48 * g * g * h * h + 48 * g * g * l * l - 96 * g * h * j * l +
        96 * g * h * j * m + 24 * h * h * h * h + 48 * h * h * j * j +
        48 * h * h * k * k + 48 * h * h * l * m + 48 * j * j * m * m +
        48 * k * k * m * m + 24 * l * l * m * m;
    const double pc4 =
        120 * (c * c + 2 * c * e + e * e + f * f + 2 * f * h + h * h - a * b -
               a * l + b * m + l * m) *
        (a * l - 2 * c * e - a * b - 2 * f * h - b * m + l * m + 2 * a * b * b +
         2 * a * a * b - 2 * a * c * c - 2 * b * c * c - 2 * a * e * e +
         2 * b * d * d - 2 * a * f * f - 2 * b * e * e - 2 * b * f * f -
         2 * a * h * h + 2 * b * g * g - 2 * b * h * h + 2 * a * j * j +
         2 * a * k * k - 2 * a * a * l + 2 * b * b * m - 2 * d * d * l -
         2 * g * g * l + 2 * j * j * m + 2 * k * k * m + c * c + e * e + f * f +
         h * h + 4 * a * c * e + 4 * b * c * e - 2 * a * b * l + 4 * a * f * h +
         2 * a * b * m + 4 * b * f * h - 4 * c * d * j + 4 * d * e * j +
         4 * c * g * k - 4 * d * f * k + 4 * d * h * k - 4 * e * g * k -
         4 * f * g * j + 4 * g * h * j - 2 * a * l * m - 2 * b * l * m);

    if (pc1 >= NEG_EPS && pc2 >= NEG_EPS && pc3 >= NEG_EPS && pc4 >= NEG_EPS) {
      return true;
    } else {
      printf("pc1 = %0.5f, pc2 = %0.5f, pc3 = %0.5f, pc4 = "
             "%0.5f \n",
             pc1, pc2, pc3, pc4);
      std::string str = "gra::spin::Positivity: Eigenvalues of the density "
                        "matrix are not >= 0 !";
...
// spin.
MMatrix<std::complex<double>> RandomRho(unsigned int J, bool parity,
                                        MRandom &rng) {
