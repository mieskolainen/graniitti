...
void InitTMatrix(gra::HELMatrix& hc, const gra::MParticle& p, const gra::MParticle& p1, const gra::MParticle& p2) {
		...
		for (int l = 0; l <= static_cast<int>(J + s); ++l) 	{
			...
			for (int i = 0; i < static_cast<int>(2*s1+1); ++i) {
				for (int j = 0; j < static_cast<int>(2*s2+1); ++j) {
					...
					// <J \lambda | ls0\lambda> 
					const double cg1 = gra::spin::ClebschGordan(static_cast<double>(l),static_cast<double>(s),0.0,lambda, J,lambda);

					// <s \lambda | s1s2\lambda1,-\lambda2>
					const double cg2 = gra::spin::ClebschGordan(s1,s2,lambda1,-lambda2, static_cast<double>(s),lambda);
                	...
                	if (hc.P_conservation) {
						...
						// Parity of the final state (hold for both bosons/fermions)
                		const int P_tot = P1 * P2 * std::pow(-1, l);
                		if (P_tot != P) { continue; } // not conserved
                	}

                	// Bose-Symmetry for identical Boson-Pairs (symmetric wavefunction)
                	if (is_boson1 && is_boson2 && identical)   {
                		if (!BoseSymmetry(l,s)) { continue; } // not right
                	}

                	// Fermi-Symmetry for identical Fermion-Pairs (antisymmetric wavefunction)
                	if (!is_boson1 && !is_boson2 && identical) {
	...
	const std::string str0 = "gra::spin::InitTMatrix:: ["
		+ gra::aux::Spin2XtoString(2*J)  + "^" + gra::aux::ParityToString(P)  + " -> "
		+ gra::aux::Spin2XtoString(2*s1) + "^" + gra::aux::ParityToString(P1) + " "
	...
	if (tag_set) {
		...
		const std::string middle = ", Following helicity decay alpha_ls-couplings missing from BRANCHING: (l,s) = ";
...
std::complex<double> SpinAmp(const gra::LORENTZSCALAR& lts, gra::PARAM_RES& resonance) {
	...
	if (lts.decaytree[B].legs.size() == 2) {
		...
		fB = fMatrix(T, lts.decaytree[B].p.spinX2/2.0, s1, s2, daughter[B1].Theta(), daughter[B1].Phi());
	}
	...
	if (lts.decaytree[C].legs.size() == 2) {
		...
		fC = fMatrix(T, lts.decaytree[C].p.spinX2/2.0, s1, s2, daughter[C1].Theta(), daughter[C1].Phi());
	}
	...
	// ------------------------------------------------------------------
	// Construct the D-matrix for an event-by-event spin space density operator rotation
	...
	// In Collins-Soper frame
		...
		// Throw exception
		std::string str = "gra::spin::SpinAmp2: Unknown polarization Lorentz rest FRAME chosen: \""
						  + resonance.hc.FRAME + "\" (valid are: \"SR\" (no rotation), \"HE\" (helicity), \"CS\" (Collins-Soper) )";
	...
	// (same cross section obtained as with spin-0 if density matrix == I == totally unpolarized)
...
//
// f_{\lambda_1\lambda_2,M}(\theta,\phi) = D_{\lambda M}^J (\theta,\phi) T_{\lambda_1, \lambda_2}
//
...
double W3j(double j1, double j2, double j3, double m1, double m2, double m3) {
	...
	for (int k = lower; k <= upper; ++k) {
	    fsum += std::pow(-1.0, k) /
	        ( gra::math::factorial(k)
	        * gra::math::factorial(c1+k)
	        * gra::math::factorial(c2+k)
	        * gra::math::factorial(c3-k)
	        * gra::math::factorial(c4-k)
	        * gra::math::factorial(c5-k) );
	}

	// Evaluate by Racah formula
	const double wc = std::pow(-1.0, j1-j2-m3)
	        * msqrt( TriangleCoeff(j1,j2,j3)
	        	* gra::math::factorial(j1+m1) * gra::math::factorial(j1-m1)
	        	* gra::math::factorial(j2+m2) * gra::math::factorial(j2-m2)
	        	* gra::math::factorial(j3+m3) * gra::math::factorial(j3-m3) ) * fsum;
	return wc;
}
...
double VonNeumannEntropy(const MMatrix<std::complex<double>>& rho) {
	...
	for (std::size_t i = 0; i < N; ++i) {
		...
		const double eigval =
		    std::real(lambda); // Take real part for numerics
		if (eigval > epsilon) {
...
double WignerSmalld(double theta, double m, double mp, double J) {
  ...
  const double factor = gra::math::msqrt(gra::math::factorial(J+m)*gra::math::factorial(J-m)
  ...
  for (int k = 0; k <= 2*J; ++k) {
      ...
      if ( J-mp-k >= 0 && J+m-k >= 0 && k+mp-m >= 0 && k >= 0) {
      	...
      	val1 = std::pow(-1, mp-m+k) / (gra::math::factorial(J-mp-k) *
      		             			   gra::math::factorial(J+m-k)  *
      		             			   gra::math::factorial(k+mp-m) *
      		             	   		   gra::math::factorial(k));
...
bool Positivity(const MMatrix<std::complex<double>>& rho, unsigned int J) {
	...
	if (J == 1) {
		...
		const double pc1 = -4 * b * b - 4 * c * c - 2 * d * d + a -
		                   (3 * a * a) / 2.0 + 1 / 2.0;
		const double pc2 =
		    -(3 * (2 * d - a + 1) *
	...
	// Spin-2
		...
		const double pc1 =
		    -6 * a * a - 8 * a * b + 4 * a - 6 * b * b + 4 * b -
		    4 * c * c - 4 * d * d - 4 * e * e - 4 * f * f - 4 * g * g -
		    4 * h * h - 4 * j * j - 4 * k * k - 2 * l * l - 2 * m * m;
		const double pc2 =
		    -12 * a * a * a - 48 * a * a * b + 6 * a * a -
		    48 * a * b * b + 24 * a * b + 12 * a * c * c -
		    12 * a * d * d + 12 * a * e * e + 12 * a * f * f -
		    12 * a * g * g + 12 * a * h * h - 24 * a * j * j -
		    24 * a * k * k + 12 * a * m * m - 12 * b * b * b +
		    6 * b * b + 12 * b * c * c - 24 * b * d * d +
		    12 * b * e * e + 12 * b * f * f - 24 * b * g * g +
		    12 * b * h * h - 12 * b * j * j - 12 * b * k * k +
		    12 * b * l * l - 12 * c * c + 24 * c * d * j +
		    24 * c * e * l - 24 * c * e * m - 24 * c * g * k +
		    12 * d * d * m - 24 * d * e * j + 24 * d * f * k -
		    24 * d * h * k - 12 * e * e + 24 * e * g * k - 12 * f * f +
		    24 * f * g * j + 24 * f * h * l - 24 * f * h * m +
		    12 * g * g * m - 24 * g * h * j - 12 * h * h -
		    12 * j * j * l - 12 * k * k * l - 6 * l * l - 6 * m * m;
		const double pc3 =
		    48 * a * a * b - 168 * a * a * b * b - 96 * a * a * a * b +
		    96 * a * a * c * c + 96 * a * a * e * e +
		    96 * a * a * f * f + 96 * a * a * h * h -
		    48 * a * a * j * j - 48 * a * a * k * k +
		    72 * a * a * l * l - 96 * a * b * b * b + 48 * a * b * b +
		    144 * a * b * c * c - 96 * a * b * d * d +
		    144 * a * b * e * e + 144 * a * b * f * f -
		    96 * a * b * g * g + 144 * a * b * h * h -
		    96 * a * b * j * j - 96 * a * b * k * k +
		    96 * a * b * l * l + 96 * a * b * m * m - 48 * a * c * c +
		    96 * a * c * d * j - 96 * a * c * e * l +
		    192 * a * c * e * m - 96 * a * c * g * k -
		    96 * a * d * e * j + 96 * a * d * f * k -
		    96 * a * d * h * k - 48 * a * e * e + 96 * a * e * g * k -
		    48 * a * f * f + 96 * a * f * g * j - 96 * a * f * h * l +
		    192 * a * f * h * m - 96 * a * g * h * j - 48 * a * h * h -
		    96 * a * j * j * l - 96 * a * k * k * l - 48 * a * l * l +
		    96 * b * b * c * c - 48 * b * b * d * d +
		    96 * b * b * e * e + 96 * b * b * f * f -
		    48 * b * b * g * g + 96 * b * b * h * h +
		    72 * b * b * m * m - 48 * b * c * c + 96 * b * c * d * j -
		    192 * b * c * e * l + 96 * b * c * e * m -
		    96 * b * c * g * k + 96 * b * d * d * m -
		    96 * b * d * e * j + 96 * b * d * f * k -
		    96 * b * d * h * k - 48 * b * e * e + 96 * b * e * g * k -
		    48 * b * f * f + 96 * b * f * g * j - 192 * b * f * h * l +
		    96 * b * f * h * m + 96 * b * g * g * m -
		    96 * b * g * h * j - 48 * b * h * h - 48 * b * m * m +
		    24 * c * c * c * c + 48 * c * c * d * d -
		    48 * c * c * e * e + 48 * c * c * f * f +
		    48 * c * c * g * g + 48 * c * c * h * h +
		    48 * c * c * j * j + 48 * c * c * k * k +
		    48 * c * c * l * m + 96 * c * d * d * e +
		    96 * c * d * j * l - 96 * c * d * j * m -
		    192 * c * e * f * h + 96 * c * e * g * g +
		    96 * c * e * j * j + 96 * c * e * k * k + 96 * c * e * l -
		    96 * c * e * m - 96 * c * g * k * l + 96 * c * g * k * m +
		    48 * d * d * e * e + 48 * d * d * f * f +
		    96 * d * d * f * h + 48 * d * d * h * h +
		    48 * d * d * l * l - 96 * d * e * j * l +
		    96 * d * e * j * m + 96 * d * f * k * l -
		    96 * d * f * k * m - 96 * d * h * k * l +
		    96 * d * h * k * m + 24 * e * e * e * e +
		    48 * e * e * f * f + 48 * e * e * g * g +
		    48 * e * e * h * h + 48 * e * e * j * j +
		    48 * e * e * k * k + 48 * e * e * l * m +
		    96 * e * g * k * l - 96 * e * g * k * m +
		    24 * f * f * f * f + 48 * f * f * g * g -
		    48 * f * f * h * h + 48 * f * f * j * j +
		    48 * f * f * k * k + 48 * f * f * l * m +
		    96 * f * g * g * h + 96 * f * g * j * l -
		    96 * f * g * j * m + 96 * f * h * j * j +
		    96 * f * h * k * k + 96 * f * h * l - 96 * f * h * m +
		    48 * g * g * h * h + 48 * g * g * l * l -
		    96 * g * h * j * l + 96 * g * h * j * m +
		    24 * h * h * h * h + 48 * h * h * j * j +
		    48 * h * h * k * k + 48 * h * h * l * m +
		    48 * j * j * m * m + 48 * k * k * m * m +
		    24 * l * l * m * m;
		const double pc4 =
		    120 * (c * c + 2 * c * e + e * e + f * f + 2 * f * h +
		           h * h - a * b - a * l + b * m + l * m) *
		    (a * l - 2 * c * e - a * b - 2 * f * h - b * m + l * m +
		     2 * a * b * b + 2 * a * a * b - 2 * a * c * c -
		     2 * b * c * c - 2 * a * e * e + 2 * b * d * d -
		     2 * a * f * f - 2 * b * e * e - 2 * b * f * f -
		     2 * a * h * h + 2 * b * g * g - 2 * b * h * h +
		     2 * a * j * j + 2 * a * k * k - 2 * a * a * l +
		     2 * b * b * m - 2 * d * d * l - 2 * g * g * l +
		     2 * j * j * m + 2 * k * k * m + c * c + e * e + f * f +
		     h * h + 4 * a * c * e + 4 * b * c * e - 2 * a * b * l +
		     4 * a * f * h + 2 * a * b * m + 4 * b * f * h -
		     4 * c * d * j + 4 * d * e * j + 4 * c * g * k -
		     4 * d * f * k + 4 * d * h * k - 4 * e * g * k -
		     4 * f * g * j + 4 * g * h * j - 2 * a * l * m -
		     2 * b * l * m);

		if (pc1 >= NEG_EPS && pc2 >= NEG_EPS && pc3 >= NEG_EPS &&
		    pc4 >= NEG_EPS) {
			return true;
		} else {
			printf(
	...
	while (true) {
		...
		if (parity) {
			...
			if (J == 2) {
				...
				// 1.2 Parity constrained
				rho[1][4] = -std::real(rho[0][3]) +
				            zi * std::imag(rho[0][3]);
				rho[2][3] = -std::real(rho[1][2]) +
				            zi * std::imag(rho[1][2]);
				rho[2][4] = std::conj(rho[0][2]);
				rho[3][4] = -std::real(rho[0][1]) +
				            zi * std::imag(rho[0][1]);
