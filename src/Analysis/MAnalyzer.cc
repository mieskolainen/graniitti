...
MAnalyzer::MAnalyzer() {
  ...
  // Energy
  hE_Pions = std::make_unique<TH1D>("Energy #pi (GeV)", ";Energy (GeV);Events",
                                    NBINS, 0, sqrts / 2.0);
  hE_Gamma = std::make_unique<TH1D>(
      "Energy #gamma (GeV)", ";Energy (GeV);Events", NBINS, 0, sqrts / 2.0);
  hE_Neutron = std::make_unique<TH1D>("Energy n (GeV)", ";Energy (GeV);Events",
                                      NBINS, 0, sqrts / 2.0);
  hE_GammaNeutron = std::make_unique<TH1D>(
      "Energy y+n (GeV)", ";Energy (GeV);Events", NBINS, 0, sqrts / 2.0);

  // Feynman-x
  hXF_Pions =
      std::make_unique<TH1D>("xF #pi", ";Feynman-x;Events", NBINS, -1.0, 1.0);
  hXF_Gamma = std::make_unique<TH1D>("xF #gamma", ";Feynman-x;Events", NBINS,
                                     -1.0, 1.0);
  hXF_Neutron =
      std::make_unique<TH1D>("xF n", ";Feynman-x;Events", NBINS, -1.0, 1.0);

  // Forward systems
  hEta_Pions =
      std::make_unique<TH1D>("#eta pi", ";#eta;Events", NBINS, -12, 12);
  hEta_Gamma = std::make_unique<TH1D>("#eta y", ";#eta;Events", NBINS, -12, 12);
  hEta_Neutron =
      std::make_unique<TH1D>("#eta n", ";#eta;Events", NBINS, -12, 12);
  hM_NSTAR = std::make_unique<TH1D>("M (GeV)", ";M (GeV);Events", NBINS, 0, 10);

  // Legendre polynomials, DO NOT CHANGE THE Y-RANGE [-1,1]
  for (std::size_t i = 0; i < 8; ++i) {
    hPl[i] = std::make_unique<TProfile>(Form("hPl%lu", i + 1), "", 100, 0.0,
                                        4.0, -1, 1);
    ...
    for (std::size_t j = 0; j < NFR; ++j) {
      h2CosTheta[i][j] = std::make_unique<TH2D>(
          Form("%s^{+} cos(theta) %s vs %s", pstr.c_str(),
               frame_labels[i].Data(), frame_labels[j].Data()),
  ...
  for (std::size_t i = 0; i < NFR; ++i) {
    hPhi_Meson_p[i] = std::make_unique<TH1D>(
        ...
        Form(";%s^{+} #phi %s; Events", pstr.c_str(), frame_labels[i].Data()),
  ...
  for (std::size_t i = 0; i < NFR; ++i) {
    h2M_CosTheta[i] = std::make_unique<TH2D>(
        ...
        Form(" ;M (GeV);%s^{+} cos(#theta) %s", pstr.c_str(),
             frame_labels[i].Data()),
  ...
  for (std::size_t i = 0; i < NFR; ++i) {
    h2M_Phi[i] = std::make_unique<TH2D>(
        ...
        Form(" ;M (GeV);%s^{+} #phi %s (rad)", pstr.c_str(),
             frame_labels[i].Data()),
...
//
double MAnalyzer::HepMC3_OracleFill(
    const std::string input, unsigned int multiplicity, int finalPDG,
  ...
  const std::string totalpath =
      gra::aux::GetBasePath(2) + "/output/" + input + ".hepmc3";
  HepMC3::ReaderAscii input_file(totalpath);

  if (input_file.failed()) {
    throw std::invalid_argument("MAnalyzer::HepMC3Read: Cannot open file " +
                                totalpath);
  }
  ...
  while (true) {
    ...
    if (cs) {
      ...
      std::cout << "Problem accessing 'GenCrossSection' attribute!"
                << std::endl;
    }
    // --------------------------------------------------------------
    ...
    int sign = -1;
    if (finalPDG == PDG::PDG_gamma || finalPDG == PDG::PDG_gluon ||
        finalPDG == 113) {
    ...
    for (HepMC3::ConstGenParticlePtr p1 : HepMC3::applyFilter(
             HepMC3::Selector::PDG_ID == finalPDG, evt.particles())) {
    ...
    try {
      ...
      if (multiplicity == 2) {
        ...
        hP["hP_S_M_PL2"]->h[SID]->Fill(M, math::LegendrePl(2, rf[0].CosTheta()),
                                       W);
        hP["hP_S_M_PL4"]->h[SID]->Fill(M, math::LegendrePl(4, rf[0].CosTheta()),
                                       W);
    ...
    if (events_read % 10000 == 0) {
      std::cout << std::endl
                << "Events processed: " << events_read << std::endl;
    }
...
double MAnalyzer::CheckEnergyMomentum(HepMC3::GenEvent &evt) const {
  std::vector<HepMC3::GenParticlePtr> all_init = HepMC3::applyFilter(
      HepMC3::Selector::STATUS == PDG::PDG_BEAM, evt.particles()); // Beam
  ...
  if (!gra::math::CheckEMC(beam - final)) {
    ...
    std::cout << rang::fg::red << "Energy-Momentum not conserved!"
...
void MAnalyzer::FrameObservables(double W, HepMC3::GenEvent &evt,
                                 const M4Vec &p_beam_plus,
                                 const M4Vec &p_beam_minus,
                                 const M4Vec &p_final_plus,
                                 const M4Vec &p_final_minus,
  ...
  for (std::size_t l = 0; l < 8; ++l) { // note l+1
    ...
    double value =
        gra::math::LegendrePl((l + 1), pions[FRAMENUMBER][0].CosTheta());
...
void MAnalyzer::NStarObservables(double W, HepMC3::GenEvent &evt) {
  ...
  std::vector<HepMC3::GenParticlePtr> search_gammas = HepMC3::applyFilter(
      HepMC3::Selector::PDG_ID == PDG::PDG_gamma, evt.particles());

  std::vector<HepMC3::GenParticlePtr> search_neutrons = HepMC3::applyFilter(
      HepMC3::Selector::PDG_ID == PDG::PDG_n, evt.particles());

  std::vector<HepMC3::GenParticlePtr> search_pip = HepMC3::applyFilter(
      HepMC3::Selector::PDG_ID == PDG::PDG_pip, evt.particles());

  std::vector<HepMC3::GenParticlePtr> search_pim = HepMC3::applyFilter(
      HepMC3::Selector::PDG_ID == PDG::PDG_pim, evt.particles());

  std::vector<HepMC3::GenParticlePtr> search_nstar = HepMC3::applyFilter(
      HepMC3::Selector::PDG_ID == PDG::PDG_NSTAR, evt.particles());
...
double powerlaw(double *x, double *par) {
  return par[0] /
         std::pow(1.0 + std::pow(x[0], 2) / (std::pow(par[1], 2) * par[2]),
                  par[2]);
}
...
void MAnalyzer::PlotAll(const std::string &titlestr) {
  ...
  */
  ...
  c3.SaveAs(Form("%s/figs/%s/h2_frame_correlations_phi.pdf",
                 gra::aux::GetBasePath(2).c_str(), inputfile.c_str()));
  ...
  c4.SaveAs(Form("%s/figs/%s/h2_costheta_phi.pdf",
                 gra::aux::GetBasePath(2).c_str(), inputfile.c_str()));
  ...
  c5.SaveAs(Form("%s/figs/%s/h2_M_costheta_phi.pdf",
                 gra::aux::GetBasePath(2).c_str(), inputfile.c_str()));
  ...
  c115->SaveAs(Form("%s/figs/%s/hPl_1to4.pdf", gra::aux::GetBasePath(2).c_str(),
                    inputfile.c_str()));
  c116->SaveAs(Form("%s/figs/%s/hPl_6to8.pdf", gra::aux::GetBasePath(2).c_str(),
                    inputfile.c_str()));
