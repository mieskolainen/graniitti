...
MAnalyzer::MAnalyzer() {
  ...
  hE_Gamma =
      std::make_unique<TH1D>("Energy #gamma (GeV)", ";Energy (GeV);Events", NBINS, 0, sqrts / 2.0);
  hE_Neutron =
  ...
  for (std::size_t i = 0; i < NFR; ++i) {
    for (std::size_t j = 0; j < NFR; ++j) {
      h2CosTheta[i][j] =
          std::make_unique<TH2D>(Form("%s^{+} cos(theta) %s vs %s", pstr.c_str(),
  ...
  for (std::size_t i = 0; i < NFR; ++i) {
    h2CosTheta_Phi[i] =
        std::make_unique<TH2D>(Form("cos(#theta) vs #phi %s", frame_labels[i].Data()),
  ...
  while (true) {
    ...
    for (const HepMC3::GenParticlePtr &p1 : final_protons) {
      ...
      std::vector<HepMC3::GenParticlePtr> results = HepMC3::applyFilter(
          HepMC3::Selector::PDG_ID == PDG::PDG_NSTAR || HepMC3::Selector::PDG_ID == PDG::PDG_system,
