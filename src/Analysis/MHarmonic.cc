...
void MHarmonic::Init(const HPARAM &hp) {
  ...
  std::cout << "  {F} Fiducial  == Events within the strict fiducial "
            ...
            << std::endl;
  std::cout << "  {S} Selected  == Events after the detector efficiency losses"
            << std::endl;
  std::cout << std::endl;
  ...
  std::cout << std::endl;
  std::cout
      << "  The basic idea is to define {G} such that minimal extrapolation "
      << std::endl;
  std::cout
      << "  is required from the strict fiducial (geometric) phase space {F}. "
      << std::endl;
  std::cout
      << "  Flatness requirement of {G} is strictly required to represent "
         "moments in "
         "an unmixed basis (non-flat phase space <=> geometric moment mixing)."
      << std::endl;
  std::cout << std::endl;
  std::cout << rang::fg::yellow
            << "EXAMPLE OF A FORMALLY VALID DEFINITION:" << rang::fg::reset
            << std::endl;
...
void MHarmonic::PlotAll(const std::string &outputpath) const {
  ...
  // 2D
  std::vector<std::vector<int>> OBSERVABLES = {
      {0, 1}, {0, 2}, {1, 2}}; // Different pairs

  for (const auto &OBSERVABLE2 : OBSERVABLES) {
    // **** EFFICIENCY DECOMPOSITION ****
    PlotFigures2D(fla, OBSERVABLE2, "{Response}[FLAT_REFERENCE]<fla>", 17,
                  outputpath);
    PlotFigures2D(fid, OBSERVABLE2, "{Response}[FIDUCIAL_ACCEPTANCE]<fid>", 33,
                  outputpath);
    PlotFigures2D(det, OBSERVABLE2, "{Response}[ACCEPTANCE_x_EFFICIENCY]<det>",
                  29, outputpath);
...
void MHarmonic::Plot2DExpansion(
  ...
  for (const auto &source : tensor) {
    ...
    for (std::size_t bin = 0; bin < BINS; ++bin) {
      ...
      if (ALGO == "MPP") {
        Z = spherical::Y_real_synthesize(source.second(cell).t_lm_MPP, ACTIVE,
                                         N, costheta, phi, true);
      } else if (ALGO == "EML") {
        Z = spherical::Y_real_synthesize(source.second(cell).t_lm_EML, ACTIVE,
                                         N, costheta, phi, true);
      }
  ...
  for (std::size_t bin = 0; bin < BINS; ++bin) {
    ...
    for (const auto &source : tensor) {
      ...
      TText *t2 = new TText(
          0.4, 0.85, Form("%s : [%0.2f, %0.2f] %s", FRAME.c_str(),
                          grid[OBSERVABLE][bin].min, grid[OBSERVABLE][bin].max,
                          xlabels[OBSERVABLE].c_str()));
      t2->SetNDC();
      t2->SetTextAlign(22);
...
void GetLegendPosition2(unsigned int N, double &x1, double &x2, double &y1,
                        double &y2, const std::string &legendposition) {
  // North-East
  x1 = 0.63;
  x2 = x1 + 0.18;
  y1 = 0.75 - 0.01 * N;
...
void MHarmonic::PlotFigures(
  ...
  std::vector<std::vector<TGraphErrors *>> gr(
      tensor.size(), std::vector<TGraphErrors *>(ACTIVENDF, NULL));
  ...
  for (int l = 0; l <= param.LMAX; ++l) {
    for (int m = -l; m <= l; ++m) {
      ...
      if (k == 0) {
        if (SPACE == "det") {
          mg[k]->SetTitle(
              Form("%s | #it{lm} = <%d,%d>", TITLES[0].c_str(), l, m));
        }
        if (SPACE == "fid") {
          mg[k]->SetTitle(
              Form("%s | #it{lm} = <%d,%d>", TITLES[1].c_str(), l, m));
        }
        if (SPACE == "fla") {
          mg[k]->SetTitle(
              Form("%s | #it{lm} = <%d,%d>", TITLES[2].c_str(), l, m));
        }
      } else {
      ...
      if (k != 0) {
        TLine *line =
            new TLine(grid[OBSERVABLE][0].min, 0.0,
  ...
  if (BINS > 1) {
    ...
    const std::string subpath =
        "OBS_" + std::to_string(OBSERVABLE) + "_" + FRAME;
    const std::string fullpath =
        "./figs/harmonicfit/" + outputpath + "/" + subpath;
    aux::CreateDirectory(fullpath);
    c1->Print(Form("%s/%s.pdf", fullpath.c_str(), TYPESTRING.c_str()));

    // Merge pdfs using Ghostscript (gs)
    const std::string cmd =
        "gs -dBATCH -dNOPAUSE -q -sDEVICE=pdfwrite -sOutputFile=" + fullpath +
...
//
void MHarmonic::Plot1DEfficiency(unsigned int OBSERVABLE,
                                 const std::string &outputpath) const {
  // ------------------------------------------------------------------
  TCanvas *c1 = gra::rootstyle::AutoGridCanvas(ACTIVENDF);
  // ------------------------------------------------------------------
  ...
  std::vector<std::vector<TGraphErrors *>> gr(
      3, std::vector<TGraphErrors *>(ACTIVENDF, NULL));
...
void MHarmonic::PlotFigures2D(
    const std::map<gra::spherical::Meta, MTensor<gra::spherical::SH>> &tensor,
    const std::vector<int> &OBSERVABLE2, const std::string &TYPESTRING,
    int barcolor, const std::string &outputpath) const {
  /*
...
//
void MHarmonic::HyperLoop(void (*fitfunc)(int &, double *, double &, double *,
                                          int),
                          const std::vector<gra::spherical::Omega> &MC,
                          const std::vector<gra::spherical::Data> &DATA,
                          const HPARAM &hp) {
    ...
    for (const auto &i : indices(grid[0])) {
      for (const auto &j : indices(grid[1])) {
        for (const auto &k : indices(grid[2])) {
          ...
          fid[META]({i, j, k}).t_lm_MPP = gra::spherical::SphericalMoments(
              DATA_events, DATA_ind, param.LMAX, "fid");
          det[META]({i, j, k}).t_lm_MPP = gra::spherical::SphericalMoments(
              DATA_events, DATA_ind, param.LMAX, "det");
          ...
          std::cout << "Algebraic (mixed) moments in the detector space:"
                    << std::endl;
          ...
          if (DATA[ind].META.MODE == "MC") {
            ...
            std::cout << rang::fg::yellow
                      << "MC GROUND TRUTH: " << rang::fg::reset << std::endl;
...
double MHarmonic::PrintOutHyperCell(const gra::spherical::Meta &META,
  ...
  if (param.EML == true) {
    ...
    const double sum_fla =
        fla[META](cell).t_lm_EML[gra::spherical::LinearInd(0, 0)];
  ...
  // Algebraic inverse
  const double sum_fla =
      fla[META](cell).t_lm_MPP[gra::spherical::LinearInd(0, 0)];
...
//
void MHarmonic::MomentFit(const gra::spherical::Meta &META,
                          const std::vector<std::size_t> &cell,
                          void (*fitfunc)(int &, double *, double &, double *,
                                          int)) {
  ...
  // Init TMinuit
  TMinuit *gMinuit =
      new TMinuit(NCOEF); // initialize TMinuit with a maximum of N params
...
void MHarmonic::logLfunc(int &npar, double *gin, double &f, double *par,
                         int iflag) const {
  ...
  if (METHOD == 2) {
    nhat = gra::spherical::HarmDotProd(det_DET(activecell).E_lm, T, ACTIVE,
                                       param.LMAX);
  }
