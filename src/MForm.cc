...
void PrintParam(double sqrts, bool forceprint) {
	if (_printcalls == 0 || forceprint) {
		...
		std::cout << rang::style::bold
		          << "Monopolium process parameters:" << rang::style::reset << std::endl
		          << std::endl;
...
gra::PARAM_RES ReadResonance(const std::string &resparam_str, MRandom &rng) {
	...
	try {
		...
		if (!(res.p.P == -1 || res.p.P == 1)) {
			std::string str =
			    "MAux:ReadResonance:: <" + resparam_str + "> Invalid P (not -1 or 1) !";
			throw std::invalid_argument(str);
		}
		...
		if (res.p.spinX2 != 0) {
			...
			if (j.at("PARAM_RES").at("random_rho") > 0) {
				for (std::size_t k = 0; k < j.at("PARAM_RES").at("random_rho");
				     ++k) {
					rho = gra::spin::RandomRho(res.p.spinX2 / 2.0,
					                           P_conservation, rng);
				}
				...
				for (std::size_t a = 0; a < rho.size_row(); ++a) {
					for (std::size_t b = 0; b < rho.size_col(); ++b) {
						const double Re =
						    j.at("PARAM_RES").at("rho_real").at(a).at(b);
						const double Im =
						    j.at("PARAM_RES").at("rho_imag").at(a).at(b);
						rho[a][b] = Re + zi * Im;
					}
				}
				...
				if (gra::spin::Positivity(rho, res.p.spinX2 / 2.0) == false) {
					std::string str =
					    "gra::form::ReadResonance: <" + resparam_str +
...
double S3HPL(double tau, double t) {
	...
	return (4.0 / tau) * pow2(F_pi) *
	       (2.0 * tau -
	        std::pow(1.0 + tau, 3.0 / 2.0) * std::log((sqrtau + 1.0) / (sqrtau - 1.0)) +
	        std::log((m * m) / (mpi * mpi)));
}
// ----------------------------------------------------------------------
...
double IncohFlux(double xi, double t, double pt, double M2) {
	...
	double f = alpha_EM(0) / PI * (pt2 / (pt2 + xi * (M2 - mp2) + xi2 * mp2)) *
	           ((1.0 - xi) * (pt2 / (pt2 + xi * (M2 - mp2) + xi2 * mp2)) * F2xQ2(xbj, Q2) /
	                (Q2 + M2 - mp2) +
...
double DZFlux(double x) {
	...
	double f =
	    alpha_EM(0) / (2.0 * PI * x) * (1.0 + pow2(1.0 - x)) *
