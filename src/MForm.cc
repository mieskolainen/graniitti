...
std::string GetHashString() {
  std::string str =
      std::to_string(PARAM_SOFT::DELTA_P) +
      std::to_string(PARAM_SOFT::ALPHA_P) + std::to_string(PARAM_SOFT::gN_P) +
...
double PsiMP(double n) {
  return (1.0 / msqrt(PI)) *
         std::pow(PARAM_MONOPOLE::M0 / (8.0 * form::alpha_EM(0) * n),
                  3.0 / 2.0);
}
...
void PrintParam(double sqrts, bool forceprint) {
  if (_printcalls == 0 || forceprint) {
    ...
    if (M > sqrts) {
      std::string str = "gra::form::PARAM_MONOPOLE::PrintParam: FATAL error "
...
gra::PARAM_RES ReadResonance(const std::string &resparam_str, MRandom &rng) {
  ...
  std::string inputfile = gra::aux::GetBasePath(2) + "/modeldata/" +
                          gra::MODELPARAM + "/" + resparam_str;
  ...
  try {
    ...
    throw std::invalid_argument("form::ReadResonance: Error parsing '" +
                                resparam_str + "'");
  }
  ...
  try {
    ...
    if (res.p.mass < 0) {
      std::string str =
          "MAux:ReadResonance:: <" + resparam_str + "> Invalid M < 0 !";
      throw std::invalid_argument(str);
    }
    ...
    if (res.p.width < 0) {
      std::string str =
          "MAux:ReadResonance:: <" + resparam_str + "> Invalid W < 0 !";
      throw std::invalid_argument(str);
    }
    ...
    if (res.p.spinX2 < 0) {
      std::string str =
          "MAux:ReadResonance:: <" + resparam_str + "> Invalid J < 0 !";
      throw std::invalid_argument(str);
    }
    ...
    if (!(res.p.P == -1 || res.p.P == 1)) {
      std::string str = "MAux:ReadResonance:: <" + resparam_str +
                        "> Invalid P (not -1 or 1) !";
      throw std::invalid_argument(str);
    }
    ...
    if (J == 0 && res.g_Tensor.size() != 2) {
      throw std::invalid_argument(
          "MAux::ReadResonance:: <" + resparam_str +
...
double S3FINEL(double t, double M2) {
  ...
  double f =
      std::pow(std::abs(t) / (M2 * (std::abs(t) + a)), 0.5 * (1 + DELTA_P));
...
double F2xQ2(double xbj, double Q2) {
  ...
  if (F2TYPE == "DL") {
    ...
    const double F2 =
        ...
        std::pow(xbj, 1.0 - alpha_R) * (B_u * std::pow(1 - xbj, n_Q2) +
...
double IncohFlux(double xi, double t, double pt, double M2) {
  ...
  double f = alpha_EM(0) / PI * (pt2 / (pt2 + xi * (M2 - mp2) + xi2 * mp2)) *
             ((1.0 - xi) * (pt2 / (pt2 + xi * (M2 - mp2) + xi2 * mp2)) *
                  F2xQ2(xbj, Q2) / (Q2 + M2 - mp2) +
              (xi2 / (4.0 * pow2(xbj))) * 2.0 * xbj * F1xQ2(xbj, Q2) /
                  (Q2 + M2 - mp2));
...
double DZFlux(double x) {
  ...
  double f = alpha_EM(0) / (2.0 * PI * x) * (1.0 + pow2(1.0 - x)) *
             (std::log(A) - 11.0 / 6.0 + 3.0 / A - 3.0 / (2.0 * pow2(A)) +
              1.0 / (3 * pow3(A)));
...
std::complex<double> CBW(const gra::LORENTZSCALAR &lts,
  ...
  case 3:
    return CBW_BF(lts.m2, resonance.p.mass, resonance.p.width,
                  resonance.p.spinX2 / 2.0, lts.decaytree[0].p4.M(),
                  lts.decaytree[1].p4.M());
  case 4:
    return CBW_JR(lts.m2, resonance.p.mass, resonance.p.width,
                  resonance.p.spinX2 / 2.0);
...
std::complex<double> CBW_BF(double m2, double M0, double Gamma, int J,
                            double mA, double mB) {
  const double u =
      msqrt((m2 - pow2(mA + mB)) * (m2 - pow2(mA - mB))) / (2 * msqrt(m2));
  const double d =
      msqrt((M0 * M0 - pow2(mA + mB)) * (M0 * M0 - pow2(mA - mB))) / (2 * M0);
