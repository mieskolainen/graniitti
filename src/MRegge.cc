...
void MRegge::InitReggeAmplitude(int PDG, const std::string& MODELPARAM) {
	...
	try {
		...
		throw std::invalid_argument(
		    "MRegge::InitReggeAmplitude_: Parse error in " + fullpath);
	}
}
...
//
//
// [REFERENCE: Gribov, A Reggeon Diagram Technique, Soviet JETP, 1968, http://jetp.ac.ru/cgi-bin/dn/e_026_02_0414.pdf]
...
std::complex<double> MRegge::ME2(gra::LORENTZSCALAR& lts, int mode) const {
	...
	if (mode == 1) { // Elastic
		...
		const double MX_2 =
		    (lts.ss[1][1] > 1.0) ? lts.ss[1][1] : lts.ss[2][2];

		const double A =
		    pow3(PARAM_SOFT::gN_P)    // Proton-Pomeron coupling ^ 3
		...
		const double A =
		    pow2(PARAM_SOFT::gN_P)  // Proton-Pomeron ^ 2
		...
		throw std::invalid_argument(
		    "MRegge::ME2: Unknown mode parameter: " +
		    std::to_string(mode));
	}
}
...
std::complex<double> MRegge::ME3HEL(gra::LORENTZSCALAR& lts, gra::PARAM_RES& resonance) const {
	...
	std::complex<double> common =
		PropOnly(lts.s1, lts.t1) * PARAM_SOFT::gN_P * FF_A *
	...
	for (std::size_t i = 0; i < N; ++i) {
		...
		if (g_Vertex(lts.t2, lambda[i][1],  lambda[i][3])
			 != std::pow(-1, lambda[i][1] - lambda[i][3]) * g_Vertex(lts.t2,-lambda[i][1],-lambda[i][3])) {
			continue;
		}
		*/
		//printf("i = %d :: lambda0 = %0.1f, lambda2 = %0.1f, lambda1 = %0.1f, lambda3 = %0.1f \n",
...
//
// g_{\lambda_1,\lambda_3}(t) = (-1)^{\lambda_1 - \lambda_3} \xi_1 g_{-\lambda_1-\lambda_3}(t)
...
//
//
std::complex<double> MRegge::gik_Vertex(double t1, double t2, double phi, int lambda_h, int J, int P) const {

	// Loop over Pomeron helicity m2, implicitly loop over m1 (see below)
	const int MAX_m = 1;

	// (we handle only Pomerons here for now, no secondary Reggeons - trivial extension)
	...
	for (int m2 = -MAX_m; m2 <= MAX_m; ++m2) { // -\inf < m_2 < inf
		...
		std::complex<double> val = gra::math::sign(m1*m2) * std::exp(gra::math::zi * static_cast<double>(m2) * phi) *
...
std::complex<double> MRegge::ME4(gra::LORENTZSCALAR& lts, double sign) const {
	...
	// sign applied here
	const std::complex<double> M_u = sign *
		  PropOnly(lts.ss[1][4], lts.t1) * PARAM_SOFT::gN_P * FF_A *
...
// ============================================================================
// Regge matrix element ansatz for 2->8 continuum spectrum (generalization of 2->6)
//
//  ======F======>
//        *
//        *
...
std::complex<double> MRegge::ME8(gra::LORENTZSCALAR& lts) const {
	...
	// Offshell propagator masses^2 [for mixed states, such as pi+ pi- K+ K- pi+ pi-]
	const double M2_A = pow2(lts.decaytree[0].p.mass);
	const double M2_B = pow2(lts.decaytree[2].p.mass);
...
std::complex<double> MRegge::PhotoProp(double s, double t, double m2, bool excite, double M2_forward) const {
	...
	const double B0 = std::pow(std::max(pow2(m_freeze), m2) / 2.0, -gra::math::PI / 2.0) + 3 * gra::math::PI / 2.0;
...
//
// see also:
// [REFERENCE: Kaidalov, Khoze, Martin, Ryskin, https://arxiv.org/pdf/hep-ph/0307064.pdf]
//
std::complex<double> JPC_CS_coupling(const gra::LORENTZSCALAR& lts, const gra::PARAM_RES& resonance) {
	...
	auto Q = [&]() { 
		return lts.t1 * lts.t2 * pow2(A[PP]) * pow2(std::sin(phi));
	};
	auto W = [&]() { 
		const  double xi1  = gra::math::csgn(A[PP]) * gra::math::csgn(A[LL]);
		return pow2( msqrt(lts.t1 * lts.t2) * A[LL] - xi1 * msqrt(lts.t1 * lts.t2) * A[PP] * std::cos(phi) );
	};
	auto E = [&]() {
	...
	auto R = [&]() { 
		return lts.t1 * lts.t2 * 0.5 * pow2(A[PM]);
	};
...
std::complex<double> JPCoupling(const gra::LORENTZSCALAR& lts,
	...
	//  ^  WA102 data (not fully sin(phi) symmetric after MC, phase space limits the reason?)
	//  |   .---.
	//  |  .     .
	//  | .       .
	//  ------------->
	...
	const double alpha = 1.0/3.0; // Controls the steepness (0 = maximal steepness, 1 = almost flat)
	
	double A = 0.0;
	if (!resonance.p.glue) { // "normal state"
		A = msqrt( alpha * pow2(msqrt(std::abs(lts.t1)) - msqrt(std::abs(lts.t2)) ) + msqrt(lts.t1*lts.t2) * pow2(std::sin(dphi/2.0)));
	} else {                 // "glue state"
		A = msqrt( alpha * pow2(msqrt(std::abs(lts.t1)) - msqrt(std::abs(lts.t2)) ) + msqrt(lts.t1*lts.t2) * pow2(std::cos(dphi/2.0)));
	}
