...
//
std::complex<double> MRegge::ME3HEL(gra::LORENTZSCALAR &lts,
  ...
  std::complex<double> common =
      PropOnly(lts.s1, lts.t1) * PARAM_SOFT::gN_P * FF_A * CBW(lts, resonance) *
      resonance.g * PropOnly(lts.s2, lts.t2) * PARAM_SOFT::gN_P * FF_B;
...
//
int MRegge::xi3(int J, int P, int P_i, int sigma_i, int P_k,
                int sigma_k) const {
...
//
//
std::complex<double> MRegge::gik_Vertex(double t1, double t2, double phi,
...
std::complex<double> MRegge::ME4(gra::LORENTZSCALAR &lts, double sign) const {
  ...
  // sign applied here
  const std::complex<double> M_u =
      sign * PropOnly(lts.ss[1][4], lts.t1) * PARAM_SOFT::gN_P * FF_A *
...
std::complex<double> MRegge::ME6(gra::LORENTZSCALAR &lts) const {
  ...
  for (const auto &i : indices(permutations4_)) {
    ...
    const std::complex<double> subamp =
        ...
        (*ff)(tt_ab, M2_A) * gpp_P * prop(tt_ab, M2_A) * (*ff)(tt_bc, M2_A) *
        gpp_P * PropOnly(lts.ss[b][c], tt_bc) * (*ff)(tt_bc, M2_B) * gpp_P *
        prop(tt_cd, M2_B) * (*ff)(tt_cd, M2_B) * gpp_P *
...
std::complex<double> MRegge::ME8(gra::LORENTZSCALAR &lts) const {
  ...
  for (const auto &i : indices(permutations6_)) {
    ...
    const std::complex<double> subamp =
        ...
        (*ff)(tt_ab, M2_A) * gpp_P * prop(tt_ab, M2_A) * (*ff)(tt_bc, M2_A) *
        gpp_P * PropOnly(lts.ss[b][c], tt_bc) * (*ff)(tt_bc, M2_B) * gpp_P *
        prop(tt_cd, M2_B) * (*ff)(tt_de, M2_B) * gpp_P *
        PropOnly(lts.ss[d][e], tt_de) * (*ff)(tt_de, M2_C) * gpp_P *
        prop(tt_ef, M2_C) * (*ff)(tt_ef, M2_C) * gpp_P *
...
// ===========
//
std::complex<double> MRegge::ME3(gra::LORENTZSCALAR &lts,
                                 gra::PARAM_RES &resonance) const {
  // Proton form factors
  const double FF_A = lts.excite1
                          ? gra::form::S3FINEL(lts.t1, lts.pfinal[1].M2())
  ...
  const std::complex<double> A_prod =
      2.0 * PropOnly(lts.s1, lts.t1) * FF_A * PARAM_SOFT::gN_P *
...
// ===========
//
std::complex<double> MRegge::PhotoME3(gra::LORENTZSCALAR &lts,
                                      gra::PARAM_RES &resonance) const {
  // Check spin
  ...
  double gammaflux1 =
      lts.excite1 ? IncohFlux(lts.x1, lts.t1, lts.qt1, lts.pfinal[1].M2())
                  : CohFlux(lts.x1, lts.t1, lts.qt1);
  double gammaflux2 =
      lts.excite2 ? IncohFlux(lts.x2, lts.t2, lts.qt2, lts.pfinal[2].M2())
  ...
  const std::complex<double> A_prod =
      (M1 + M2) * PARAM_REGGE::JPCoupling(lts, resonance);
...
std::complex<double> MRegge::PhotoProp(double s, double t, double m2,
                                       bool excite, double M2_forward) const {
  ...
  const double B0 =
      std::pow(std::max(pow2(m_freeze), m2) / 2.0, -gra::math::PI / 2.0) +
      3 * gra::math::PI / 2.0;
  ...
  const double FF =
      excite ? gra::form::S3FINEL(t, M2_forward) : std::exp(B0 * t / 2.0);
...
std::complex<double> JPCoupling(const gra::LORENTZSCALAR &lts,
  ...
  if (resonance.p.spinX2 == 2 &&
      resonance.p.P ==
          -1) { // 1- = vector, [e.g. rho(770), omega, phi(1020), K*(892)]
    return 1.0;
  }

  const double alpha =
      1.0 /
      3.0; // Controls the steepness (0 = maximal steepness, 1 = almost flat)
