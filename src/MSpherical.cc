...
MMatrix<double> GetGMixing(const std::vector<Omega> &events,
  ...
  const double N_generated = static_cast<double>(
      ind.size()); // Generated events within this mass interval
    ...
    for (int m = -l; m <= l; ++m) {
      ...
      for (int lprime = 0; lprime <= LMAX; ++lprime) {
        for (int mprime = -lprime; mprime <= lprime; ++mprime) {
          ...
          double error = CalcError(E2[index][indexprime], E[index][indexprime],
                                   N_generated);
...
GetELM(const std::vector<Omega> &MC, const std::vector<std::size_t> &ind,
  ...
  const double N_generated =
      (double)ind.size(); // Generated events within this hypercell
