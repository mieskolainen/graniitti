...
MMatrix<double> GetGMixing(const std::vector<Omega> &events, const std::vector<std::size_t> &ind,
		...
		for (int l = 0; l <= LMAX; ++l) {
			for (int m = -l; m <= l; ++m) {
				...
				const std::complex<double> Y =
				    gra::math::Y_complex_basis(costheta, phi, l, m);
				...
				for (int lprime = 0; lprime <= LMAX; ++lprime) {
					for (int mprime = -lprime; mprime <= lprime; ++mprime) {
						...
						const std::complex<double> Yprime =
						    gra::math::Y_complex_basis(costheta, phi,
						                               lprime, mprime);
						const double ReYprime =
						    gra::math::NReY(Yprime, lprime, mprime);

						// Sum to the MC integral and its squared version
						// (for uncertainty)
						const double f =
						    VOL * ReY * ReYprime; // Note volume term
	...
	if (mode == "fid" || mode == "det") {
		printf(
		    "Fiducial flat MC phase space events = %d (acceptance %0.3f "
		    "percent) \n",
		    fiducial, fiducial / static_cast<double>(ind.size()) * 100);
	}
	if (mode == "det") {
		printf(
		    "Selected flat MC phase space events = %d (efficiency %0.3f "
		    "percent) \n",
	...
	const double N_generated =
	    static_cast<double>(ind.size()); // Generated events within this mass interval
		...
		for (int m = -l; m <= l; ++m) {
			...
			for (int lprime = 0; lprime <= LMAX; ++lprime) {
				for (int mprime = -lprime; mprime <= lprime; ++mprime) {
					...
					double error = CalcError(E2[index][indexprime],
					                         E[index][indexprime], N_generated);
...
std::pair<std::vector<double>, std::vector<double>> GetELM(const std::vector<Omega> &MC,
	...
	if (mode == "fid" || mode == "det") {
		printf(
		    "Fiducial flat MC phase space events = %d (geometric-kinematic "
		    "acceptance "
		    "%0.3f percent) \n\n",
		    fiducial, fiducial / static_cast<double>(ind.size()) * 100);
	}
	if (mode == "det") {
		printf(
		    "Selected flat MC phase space events = %d (fiducial efficiency "
		    "%0.3f percent) "
		    "\n\n",
		    selected, selected / static_cast<double>(fiducial) * 100);
	}
	...
	for (int l = 0; l <= LMAX; ++l) {
		for (int m = -l; m <= l; ++m) {
			...
			const double error =
			    msqrt((E2[index] - std::pow(E[index], 2)) / N_generated);
			E_error[index] = error;
			sum += E[index];
			...
			for (const auto &k : ind) {
				...
				// Geometric x Efficiency ("Detector level")
					...
					throw std::invalid_argument(
					    "spherical::SphericalMoments: Unknown mode " + mode);
				}
...
std::vector<std::size_t> GetIndices(const std::vector<Omega> &events, const std::vector<double> &M,
	...
	for (const auto &i : indices(events)) {
		if (events[i].M > M[0] && events[i].M < M[1] && events[i].Pt > Pt[0] &&
		    events[i].Pt < Pt[1] && events[i].Y > Y[0] && events[i].Y < Y[1]) {
			ind.push_back(i);
		}
	}
	...
	std::cout << rang::fg::green;
	printf(
	    "MASS RANGE: [%0.3f, %0.3f] GeV, PT RANGE: [%0.3f, %0.3f] GeV, Y "
	    "RANGE: [%0.3f, %0.3f] "
	...
	for (int l = 0; l <= LMAX; ++l) {
		for (int m = -l; m <= l; ++m) {
			...
			printf("I_%d%d \t= %12.8f +- %12.8f   (rel.error %9.3f percent) \n", l, m,
			       E[ind], error, std::abs(error / E[ind]) * 100);
		}
	}
	std::cout << std::endl;
}
		...
		for (int m = -l; m <= l; ++m) {
			...
			for (const std::size_t &i : indices(costheta)) {
				for (const std::size_t &j : indices(phi)) {
					// Add value
					Z[i][j] += c_lm[index] *
					           math::Y_real_basis(costheta[i], phi[j], l, m);
				}
			}
		}
	}
