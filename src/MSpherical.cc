...
MMatrix<double> GetGMixing(const std::vector<Omega> &events, const std::vector<std::size_t> &ind,
  ...
  if (mode == "fid" || mode == "det") {
    if (fiducial < 50 || selected < 50) {
      std::cout << rang::fg::red << "GetGMixing:: Too low MC event count in the mass bin!"
...
double HarmDotProdError() { return 0.0; }
...
void PrintOutMoments(const std::vector<double> &x, const std::vector<double> &x_error,
  ...
  for (int l = 0; l <= LMAX; ++l) {
    for (int m = -l; m <= l; ++m) {
      ...
      if (ACTIVE[index]) {
        std::cout << "[" << rang::fg::green << "active" << rang::fg::reset << "]" << std::endl;
      } else {
        std::cout << "[" << rang::fg::red << "inactive" << rang::fg::reset << "]" << std::endl;
      }
    }
  }
  std::cout << std::endl;
...
std::vector<std::size_t> GetIndices(const std::vector<Omega> &events, const std::vector<double> &M,
  ...
  for (const auto &i : indices(events)) {
    if (events[i].M > M[0] && events[i].M < M[1] && events[i].Pt > Pt[0] && events[i].Pt < Pt[1] &&
...
// Standard error
double CalcError(double f2, double f, double N) { return sqrt((f2 - std::pow(f, 2)) / N); }
