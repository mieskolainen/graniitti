...
namespace spherical {
			...
			for(int m = -l; m <= l; ++m) {
				...
				for(int lprime = 0; lprime <= LMAX; ++lprime) {
					for(int mprime = -lprime; mprime <= lprime; ++mprime) {
						...
						double error =
							CalcError(E2[index][indexprime], E[index][indexprime], N_generated);
		...
		for(int l = 0; l <= LMAX; ++l) {
			for(int m = -l; m <= l; ++m) {
				...
				printf("E_%d%d \t= %12.8f +- %12.8f   (rel.error %9.3f percent) \n", l, m, E[index],
					   error, std::abs(error / E[index]) * 100);
			}
		}
		printf("SUM_lm =   %0.8f \n", sum);
	...
	std::vector<std::size_t> GetIndices(const std::vector<Omega>& events,
										const std::vector<double>& M, const std::vector<double>& Pt,
