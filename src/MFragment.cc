...
bool MFragment::SolveAlpha(double &alpha, double M0,
  ...
  while (true) {
    ...
    const double DY =
        E[0] * E[1] * (C - std::log(E[0] * E[1])) / (E[0] * E[3] - E[1] * E[2]);
    alpha -= DY;
...
// M0 is the N* mass
void MFragment::NstarDecayTable(double M0, std::vector<int> &pdgcode,
                                MRandom &rng) {
  int decaymode = 0;
  ...
  if (decaymode == 1) {
    ...
    // PDG-ID of subchannels
    const std::vector<std::vector<int>> ID = {{PDG_n, PDG_pip, PDG_pi0},
                                              {PDG_p, PDG_pip, PDG_pim},
                                              {PDG_p, PDG_pi0, PDG_pi0}};
...
void MFragment::GetForwardMass(double &mass1, double &mass2, bool &excite1,
                               bool &excite2, unsigned int excite,
                               MRandom &random) {
...
int MFragment::PickParticles(double M, unsigned int N, int B, int S, int Q,
  ...
  const std::vector<double> ratio3 = {1.0 / denom3, 0.1 / denom3,
                                      0.06 / denom3};
