...
void MGraniitti::InitFileOutput() {
  if (NEVENTS > 0) {
    ...
    FULL_OUTPUT_STR =
        gra::aux::GetBasePath(2) + "/output/" + OUTPUT + "." + FORMAT;
...
std::vector<std::string> MGraniitti::GetProcessNumbers() const {
  ...
  std::cout << rang::style::bold
            << "2->3 x 1->(N-2) LIPS:" << rang::style::reset << std::endl;
  std::vector<std::string> list1 = proc_F.PrintProcesses();
  std::cout << std::endl;

  std::cout << rang::style::bold << "2->N LIPS:" << rang::style::reset
            << std::endl;
...
void MGraniitti::ReadInput(const std::string &inputfile,
                           const std::string cmd_PROCESS) {
  // Save it for later use
  FULL_INPUT_STR = inputfile;

  std::cout << rang::fg::green << "MGraniitti::ReadInput: " + inputfile
...
void MGraniitti::ReadGeneralParam(const std::string &inputfile) {
  ...
  try {
    ...
    std::string str = "MGraniitti::ReadGeneralParam: Error parsing " +
...
void MGraniitti::ReadModelParam(const std::string &inputfile) {
  ...
  // Read and parse
  const std::string fullpath = gra::aux::GetBasePath(2) + "/modeldata/" +
  ...
  try {
    ...
    throw std::invalid_argument(
        "MGraniitti::ReadModelParam: Missing parameter in '" + inputfile +
        "' : " + e.what());
  }
}

// Process parameter initialization, Call proc->post_Constructor() after this
void MGraniitti::ReadProcessParam(const std::string &inputfile,
                                  const std::string cmd_PROCESS) {
  // Read and parse
  const std::string data = gra::aux::GetInputData(inputfile);
  json j;
  try {
    ...
    std::string str = "MGraniitti::ReadProcessParam: Error parsing " +
                      inputfile + " (Check for extra/missing commas)";
    throw std::invalid_argument(str);
  }
  ...
  pos =
      (pos1 != std::string::npos) ? pos1 : std::string::npos; // Try to find ->
  if (pos == std::string::npos) {
    pos = (pos2 != std::string::npos) ? pos2
                                      : std::string::npos; // Try to find &>
  }
...
void MGraniitti::ReadIntegralParam(const std::string &inputfile) {
  ...
  try {
    ...
    std::string str = "MGraniitti::ReadIntegralParam: Error parsing " +
                      inputfile + " (Check for extra/missing commas)";
    throw std::invalid_argument(str);
  }
  ...
  AssertRange(mpam.MIN_EVENTS, {10, (unsigned int)1e9}, "FLAT::MIN_EVENTS",
              true);
  SetMCParam(mpam);
  ...
  try {
    ...
    std::cout
        << "MGraniitti::ReadIntegralParam: Did not found VEGAS parameter block "
...
void MGraniitti::Initialize() {
  ...
  std::cout << std::endl;
  std::cout << rang::style::bold << "General setup:" << rang::style::reset
            << std::endl
            << std::endl;
...
int MGraniitti::Vegas(unsigned int init, unsigned int calls,
                      unsigned int itermin, unsigned int N) {
  ...
  for (std::size_t iter = 0; iter < itermin; ++iter) {
    ...
    // Chi2
    double chi2this =
        (VD.sumchi2 - pow2(VD.sumdata) / VD.sweight) / (iter + 1E-5);
    ...
    if (vparam.DEBUG >= 0) {
      ...
      printf("VEGAS:: local iter = %4lu integral = %0.5E +- std = "
             ...
             "chi2this = %0.2f \n",
             iter + 1, I_this, gra::math::msqrt(I2_this), stat.sigma,
             stat.sigma_err, chi2this);
...
void MGraniitti::VEGASMultiThread(unsigned int N, unsigned int THREAD_ID,
                                  unsigned int init, unsigned int LOCALcalls) {
  double zn = 0.0;
  double zo = 0.0;
  double ac = 0.0;
...
int MGraniitti::SaveEvent(MProcess *pr, double weight, double MAXWEIGHT,
  ...
  // fails numerically
  if ((hit_in && aux.Valid()) || (WEIGHTED && aux.Valid()) ||
      aux.forced_accept) {
    ...
    evt.weights().push_back(
        HepMC3_weight); // add more weights with .push_back()
  ...
  if (GMODE == 0) {
    ...
    if (proc->lts.DW_sum.Integral() > 0) { // Recursive phase space on
      ...
      const double PSvolume_exact_error =
          proc->lts.DW_sum_exact.IntegralError();
