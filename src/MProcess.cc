...
void MProcess::PrintSetup() const {
	...
	// Subprocess
	std::cout << rang::style::bold << "Subprocess parameters:" << rang::style::reset
	          << std::endl
	          << std::endl;
...
std::string MProcess::GetProcessDescriptor(std::string str) const {
	if (!ProcessExist(str)) {
		throw std::invalid_argument("MProcess::GetProcessDescriptor: Process by name " +
		                            str + " does not exist");
	}
		...
		for (std::size_t j = 0; j < MEikonalNumerics::NumberLoopKT + 1; ++j) {
			...
			if (lts.hamp.size() > 1) { // Helicity amps
				for (const auto &h : indices(lts.hamp)) {
					f_hamp[h][i][j] =
					    A_eik * lts.hamp[h] * kt; // note x kt (jacobian)
				}
			} else {
				f[i][j] = A_eik * A * kt;
			} // note x kt (jacobian)
...
double MProcess::GetFlatAmp2(const gra::LORENTZSCALAR &lts) const {
	...
	// Ansatz: |A|^2 ~ exp(bt1) exp(bt2) / shat
		...
		W = std::exp(PARAM_FLAT::b * lts.t1) * std::exp(PARAM_FLAT::b * lts.t2) / lts.s_hat;
	}
	// Constant
	else if (FLATAMP == 5) {
		W = 1.0;
					...
					for (std::size_t a = 0; a < 50; ++a) {
						try {
							const unsigned int l =
							    j[PDG_STR][SID]["alpha_ls"][a][0];
							const unsigned int s =
							    j[PDG_STR][SID]["alpha_ls"][a][1];
							const double Re =
							    j[PDG_STR][SID]["alpha_ls"][a][2];
							const double Im =
							    j[PDG_STR][SID]["alpha_ls"][a][3];
			...
			for (std::size_t i = 0; i < branch.legs.size(); ++i) {
				...
				if (!FLATMASS2) {
					daughter_masses += std::max(
					    0.0, random.RelativisticBWRandom(M, W, OFFSHELL));
				} else {
					daughter_masses += msqrt(
					    random.U(std::max(0.0, pow2(M - OFFSHELL * W)),
...
void MProcess::ExciteNstar(const M4Vec &nstar, gra::MDecayBranch &forward) {
	...
	for (const auto &i : indices(p)) {
		...
		if (branch.p.pdg == PDG::PDG_pi0) {
			...
			std::vector<M4Vec> pgamma;
			gra::kinematics::TwoBodyPhaseSpace(branch.p4, branch.p4.M(), mgamma, pgamma,
			                                   random);
...
bool MProcess::CommonCuts() const {
	...
	if (fcuts.active == true) {
		...
		if (msqrt(lts.m2) >= fcuts.M_min && msqrt(lts.m2) <= fcuts.M_max &&
		    lts.Y >= fcuts.Y_min && lts.Y <= fcuts.Y_max && lts.Pt >= fcuts.Pt_min &&
		    lts.Pt <= fcuts.Pt_max) {
			// fine, do not touch
		} else {
			return false; // not fine
...
bool MProcess::ConstructDecayKinematics(gra::MDecayBranch &branch) {
	...
	if (branch.legs.size() > 0) {
		...
		if (branch.legs.size() == 2) {
			w = gra::kinematics::TwoBodyPhaseSpace(branch.p4, branch.p4.M(), masses, p,
			                                       random);
			// 3-body
