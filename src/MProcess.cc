...
double MProcess::GetFlatAmp2(const gra::LORENTZSCALAR& lts) const {
	...
	// Constant
		...
		std::string str =
		    "MProcess::GetFlatAmp2: Unknown FLATAMP mode (|A|^2 is 0 = off, 1 = exp{b(t1+t2)}, 2 = exp{b(t1+t2)}/shat^{1/2}, 3 = exp{b(t1+t2)}/shat, 4 = exp{b(t1+t2)} x exp(-shat), 5 = 1.0) : input was " + std::to_string(FLATAMP);
		throw std::invalid_argument(str);
	}
	return W;
}
