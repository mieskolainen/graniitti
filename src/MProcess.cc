...
complex<double> MProcess::S3ScreenedAmp() {
	...
	if(lts.hamp.size() > 1) {
		for(const auto& h : indices(lts.hamp)) {
			hamp_loop[h] =
				norm * gra::math::Simpson38Integral2D(f_hamp[h], WSimpson, StepPhi, StepKT);
		}
	} else {
...
double MProcess::GetFlatAmp2(const gra::LORENTZSCALAR& lts) const {
	...
	// Ansatz: |A|^2 ~ exp(bt1) exp(bt2) / sqrt(shat)
		...
		W = std::exp(PARAM_FLAT::b * lts.t1) * std::exp(PARAM_FLAT::b * lts.t2) / msqrt(lts.s_hat);
	}
	...
	// Constant
		...
		std::string str = "MProcess::GetFlatAmp2: Unknown FLATAMP mode (|A|^2 is 0 = off, 1 = "
	...
	for(auto const& xpoint : lts.RESONANCES) {
		...
		if(found == true) {
			...
			if(lts.decaytree.size() == 2) {
				...
				double PS =
					gra::kinematics::PDW2body(pow2(res.p.mass), pow2(lts.decaytree[0].p.mass),
				...
				if(std::isnan(res.g_decay) || std::isinf(res.g_decay)) {
					std::string str = "MProcess::SetupBranching:: Kinematic coupling problem "
									  "to '" +
...
bool MProcess::ConstructDecayKinematics(gra::MDecayBranch& branch) {
	...
	if(branch.legs.size() > 0) {
		...
		if(branch.legs.size() == 2) {
			...
			w = gra::kinematics::NBodyPhaseSpace(branch.p4, branch.p4.M(), masses, p, UNWEIGHT,
												 random);
		}
		if(w.GetW() < 0) {
			std::string str = "MProcess::ConstructDecayKinematics: Fatal error: Weight < 0 (Check "
							  "your decay tree)";
