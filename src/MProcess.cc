...
void MProcess::PrintSetup() const {
  std::cout << std::endl;
  std::cout << rang::style::bold << "Process setup:" << rang::style::reset
            << std::endl
            << std::endl;
  std::cout << "- Random seed:      " << random.GetSeed() << std::endl;
  std::cout << "- Initial state:    " << beam1.name << " " << beam2.name
            << std::endl;
  printf("- Beam energies:   [%0.1f %0.1f] GeV \n", lts.pbeam1.E(),
         lts.pbeam2.E());
  printf("- CMS energy:       %0.1f GeV\n", lts.sqrt_s);
  std::cout << "- Process:          " << PROCESS << rang::fg::green << "  <"
            << GetProcessDescriptor(PROCESS) << ">" << rang::fg::reset
            << std::endl
            << std::endl;

  // Subprocess
  std::cout << rang::style::bold
            << "Subprocess parameters:" << rang::style::reset << std::endl
            << std::endl;
  std::cout << "- Pomeron loop screening:  " << std::boolalpha << SCREENING
            << std::endl;
  ...
  if (ProcPtr.ISTATE != "X") {
    ...
    if (EXCITATION == 0) {
      std::cout << rang::fg::green << "  <elastic>" << rang::fg::reset
                << std::endl;
    }
    if (EXCITATION == 1) {
      std::cout << rang::fg::green << "  <single>" << rang::fg::reset
                << std::endl;
    }
    if (EXCITATION == 2) {
      std::cout << rang::fg::green << "  <double>" << rang::fg::reset
                << std::endl;
    }
  }
  if (FLATAMP != 0) {
...
std::string MProcess::GetProcessDescriptor(std::string str) const {
  if (!ProcessExist(str)) {
    throw std::invalid_argument(
        "MProcess::GetProcessDescriptor: Process by name " + str +
        " does not exist");
  }
...
complex<double> MProcess::S3ScreenedAmp() {
  ...
  const static double StepKT =
      (MEikonalNumerics::MaxLoopKT - MEikonalNumerics::MinLoopKT) /
      MEikonalNumerics::NumberLoopKT;
  const double MinPhi = 0.0;
  const double MaxPhi = 2.0 * gra::math::PI;
  const static double StepPhi =
      (MaxPhi - MinPhi) / MEikonalNumerics::NumberLoopPHI;

  // Init 2D-Simpson weight matrix (will be calculated only once, being
  // static)
  const static MMatrix<double> WSimpson = gra::math::Simpson38Weight2D(
      MEikonalNumerics::NumberLoopPHI, MEikonalNumerics::NumberLoopKT);
  ...
  if (lts.hamp.size() > 1) {
    for (const auto &h : indices(lts.hamp)) {
      hamp_loop[h] = norm * gra::math::Simpson38Integral2D(f_hamp[h], WSimpson,
                                                           StepPhi, StepKT);
    }
  } else {
    A_loop =
...
void MProcess::SetInitialState(const std::vector<std::string> &beam,
  ...
  printf(
      "MProcess::SetInitialState: beam: [%s, %s], energy = [%0.1f, %0.1f] \n",
      beam[0].c_str(), beam[1].c_str(), energy[0], energy[1]);

  if (beam.size() != 2) {
    throw std::invalid_argument(
        "MProcess::SetInitialState: Input BEAM vector not dim 2!");
  }
  if (energy.size() != 2) {
    throw std::invalid_argument(
        "MProcess::SetInitialState: Input ENERGY vector not dim 2!");
  }
  ...
  lts.pbeam1 =
      M4Vec(0, 0, msqrt(pow2(E1) - pow2(beam1.mass)), E1); // positive z-axis
  lts.pbeam2 =
      M4Vec(0, 0, -msqrt(pow2(E2) - pow2(beam2.mass)), E2); // negative z-axis
  ...
  if (lts.sqrt_s < (beam1.mass + beam2.mass)) {
    std::string str =
        "MProcess::SetInitialState: Error with input CMS energy: " +
...
double MProcess::GetFlatAmp2(const gra::LORENTZSCALAR &lts) const {
  ...
  if (FLATAMP == 1) {
    W = std::exp(PARAM_FLAT::b * lts.t1) * std::exp(PARAM_FLAT::b * lts.t2);
  }
  ...
  // Constant
    ...
    std::string str =
        "MProcess::GetFlatAmp2: Unknown FLATAMP mode (|A|^2 is 0 = off, 1 = "
...
void MProcess::SetDecayMode(std::string str) {
  ...
  if (check.size() == 0 && !(PROCESS.find("<Q>") != std::string::npos) &&
    ...
    throw std::invalid_argument("MProcess::SetDecayMode: DECAY string input is "
  ...
  if (L != R) {
    std::string strerr = "MProces::SetDecayMode: ERROR: Decay tree has " +
                         std::to_string(L) + " left brackets { and " +
                         std::to_string(R) + " right brackets } !";
    throw std::invalid_argument(strerr);
  }
  if (arrows != L) {
    std::string strerr = "MProces::SetDecayMode: ERROR: Decay tree syntax with "
                         "arrows > not equal to { "
                         "} brackets!";
    throw std::invalid_argument(strerr);
  }
...
void MProcess::CalculateSymmetryFactor() {
  ...
  for (const auto &i : indices(lts.decaytree)) {
    ...
    for (const auto &j : indices(lts.decaytree)) {
      if (i != j && lts.decaytree[i].p.pdg == lts.decaytree[j].p.pdg &&
          marked[j] == false) {
        marked[j] = true;
        ++n;
      }
...
void MProcess::SetupBranching() {
  ...
  if (lts.RESONANCES.size() == 0) { // Empty one is not treated
    std::cout << rang::fg::red
              << "MProcess::SetupBranching: lts.RESONANCES.size() == 0 !"
              << rang::fg::reset << std::endl;
    return;
  }
  if (lts.decaytree.size() == 0) {
    std::cout << rang::fg::red
              << "MProcess::SetupBranching: decaytree.size() == 0 !"
              << rang::fg::reset << std::endl;
    return;
  }
  ...
  const std::string inputfile = gra::aux::GetBasePath(2) + "/modeldata/" +
                                gra::MODELPARAM + "/BRANCHING.json";
  ...
  for (auto const &xpoint : lts.RESONANCES) {
    ...
    if (found == true) {
      ...
      std::cout << "MProcess::SetupBranching: Resonance PDG = " +
                       std::to_string(res.p.pdg) + " / " + resonance_name
                << std::endl;
      ...
      for (std::size_t i = 0; i < MAXDECAYS; ++i) {
        ...
        if (std::is_permutation(refdecay.begin(), refdecay.end(),
                                decay.begin())) {
      ...
      if (lts.decaytree.size() == 2) {
        try {
          gra::spin::InitTMatrix(res.hc, res.p, lts.decaytree[0].p,
                                 lts.decaytree[1].p);
      ...
      if (lts.decaytree.size() == 2) {
        ...
        double PS = gra::kinematics::PDW2body(
            pow2(res.p.mass), pow2(lts.decaytree[0].p.mass),
        ...
        if (std::abs(PS) < 1e-9) {
          ...
          for (std::size_t i = 1; i <= N_width; ++i) {
            PS = gra::kinematics::PDW2body(pow2(res.p.mass + i * res.p.width),
                                           ...
                                           pow2(lts.decaytree[1].p.mass), amp2,
                                           sym);
        ...
        if (std::isnan(res.g_decay) || std::isinf(res.g_decay)) {
          std::string str =
              ...
              "to '" +
              DECAYMODE + "' with resonance PDG " + res.p.name + "(" +
              std::to_string(res.p.pdg) + ")" +
      ...
      std::string str = "MProcess::SetupBranching:: Did not find any "
  ...
  while (true) {
    ...
    if (branch.legs.size() > 0) {
      ...
      for (std::size_t i = 0; i < branch.legs.size(); ++i) {
        ...
        if (!FLATMASS2) {
          daughter_masses +=
              std::max(0.0, random.RelativisticBWRandom(M, W, OFFSHELL));
        } else {
          daughter_masses +=
              msqrt(random.U(std::max(0.0, pow2(M - OFFSHELL * W)),
...
int MProcess::ExciteContinuum(const M4Vec &nstar,
                              const HepMC3::GenParticlePtr gen_nstar,
                              HepMC3::GenEvent &evt, double Q2_scale, int B,
                              int Q) {
  ...
  const double QProb =
      2.0 / 3.0; // Probability for a charged particle (isospin)
  ...
  while (true) { // OUTER
    ...
    const double W =
        MFragment::TubeFragment(nstar, M, mass, products, q, T, maxpt, random);
...
void MProcess::BranchForwardSystem(const std::vector<M4Vec> &p,
  ...
  for (const auto &i : indices(p)) {
    ...
    HepMC3::GenParticlePtr gen_p = std::make_shared<HepMC3::GenParticle>(
        gra::aux::M4Vec2HepMC3(p[i]), pdgcode[i], status);
...
void MProcess::ExciteNstar(const M4Vec &nstar, gra::MDecayBranch &forward) {
  ...
  if (mass.size() == 3) {
    ...
    gra::kinematics::ThreeBodyPhaseSpace(nstar, nstar.M(), mass, p4, UNWEIGHT,
                                         random);
  }
  ...
  for (const auto &i : indices(p)) {
    ...
    if (branch.p.pdg == PDG::PDG_pi0) {
      ...
      std::vector<M4Vec> pgamma;
      gra::kinematics::TwoBodyPhaseSpace(branch.p4, branch.p4.M(), mgamma,
                                         pgamma, random);
...
bool MProcess::CommonCuts() const {
  ...
  if (fcuts.active == true) {
    ...
    if (CID != "P") { // Collinear class does not support these
      ...
      if (std::abs(lts.t1) >= fcuts.forward_t_min &&
          std::abs(lts.t1) <= fcuts.forward_t_max &&
          std::abs(lts.t2) >= fcuts.forward_t_min &&
          std::abs(lts.t2) <= fcuts.forward_t_max) {
        // fine
      } else {
        return false;
      }
      ...
      if (lts.excite1) {
        if (lts.pfinal[1].M() >= fcuts.forward_M_min &&
      ...
      if (lts.excite2) {
        if (lts.pfinal[2].M() >= fcuts.forward_M_min &&
...
void MProcess::PrintDecayTree(const gra::MDecayBranch &branch) const {
  ...
  printf("M = %0.2E, W = %0.2E (GeV) [<tau> = %0.1E s] PDG = [%s%d | %s] "
         "[Q=%s, J=%s] \n",
...
void MProcess::PrintPhaseSpace(const gra::MDecayBranch &branch) const {
  ...
  if (branch.W.GetW() > 0) { // There is decay information
    printf("%s> \t {1->%lu LIPS}:  %0.3E +- %0.3E \n", lines.c_str(),
           branch.legs.size(), branch.W.Integral(), branch.W.IntegralError());
  }
  for (const auto &i : indices(branch.legs)) {
...
bool MProcess::ConstructDecayKinematics(gra::MDecayBranch &branch) {
  ...
  if (branch.legs.size() > 0) {
    ...
    if (branch.legs.size() == 2) {
      w = gra::kinematics::TwoBodyPhaseSpace(branch.p4, branch.p4.M(), masses,
                                             p, random);
      // 3-body
    } else if (branch.legs.size() == 3) {
      w = gra::kinematics::ThreeBodyPhaseSpace(branch.p4, branch.p4.M(), masses,
                                               p, UNWEIGHT, random);
      // N-body
    } else {
      w = gra::kinematics::NBodyPhaseSpace(branch.p4, branch.p4.M(), masses, p,
                                           UNWEIGHT, random);
    }
    if (w.GetW() < 0) {
      std::string str =
    ...
    for (const auto &i : indices(branch.legs)) {
      if (!ConstructDecayKinematics(branch.legs[i])) {
        return false;
      }
    }
  }
...
void MProcess::WriteDecayKinematics(gra::MDecayBranch &branch,
  ...
  if (branch.legs.size() > 0) {
    ...
    HepMC3::GenVertexPtr vertex = std::make_shared<HepMC3::GenVertex>(
        gra::aux::M4Vec2HepMC3(branch.decay_position));
...
void MProcess::SampleForwardMasses(std::vector<double> &mvec,
  ...
  if (M2_f_max <= M2_f_min) {
    throw std::invalid_argument(
        "MProcess::SampleForwardMasses: Forward leg Xi : [max] gives mass = " +
        std::to_string(msqrt(M2_f_max)) +
        " GeV, below the inelastic threshold. Increase the upper (max) bound.");
  }

  if (EXCITATION == 1) {
    const double mforward =
        msqrt(M2_f_min + (M2_f_max - M2_f_min) * randvec[0]);
    if (random.U(0, 1) < 0.5) {
      mvec[0] = mforward;
      lts.excite1 = true;
    } else {
...
void MProcess::PrintFiducialCuts() const {
  ...
  std::cout << std::endl;
  printf("Final state symmetry factor (1/S = 1/%0.0f) applied at cross section "
         "level \n",
         S_factor);
...
bool MProcess::CommonRecord(HepMC3::GenEvent &evt) {
  ...
  HepMC3::GenParticlePtr gen_q = std::make_shared<HepMC3::GenParticle>(
      gra::aux::M4Vec2HepMC3(lts.pfinal[0]), PDG::PDG_system,
  ...
  for (const auto &i : indices(lts.decaytree)) {
    const int STATE =
        (lts.decaytree[i].legs.size() > 0) ? PDG::PDG_DECAY : PDG::PDG_STABLE;
    ...
    if (STATE == PDG::PDG_DECAY) {
      ...
      lts.decaytree[i].decay_position =
          lts.decaytree[i].p4.PropagatePosition(tau, MM);
      // ----------------------------------------------------------
    }

    HepMC3::GenParticlePtr particle = std::make_shared<HepMC3::GenParticle>(
        gra::aux::M4Vec2HepMC3(lts.decaytree[i].p4), lts.decaytree[i].p.pdg,
        STATE);
  ...
  if (lts.excite1 == true) {
    ...
    for (const auto &i : indices(lts.decayforward1.legs)) {
      const int STATE = (lts.decayforward1.legs[i].legs.size() > 0)
                            ? PDG::PDG_DECAY
                            : PDG::PDG_STABLE;
      // TBD: ADD HERE THE ctau > 1.0 cm definition for the status code

      HepMC3::GenParticlePtr particle = std::make_shared<HepMC3::GenParticle>(
          gra::aux::M4Vec2HepMC3(lts.decayforward1.legs[i].p4),
          lts.decayforward1.legs[i].p.pdg, STATE);
  ...
  if (lts.excite2 == true) {
    ...
    for (const auto &i : indices(lts.decayforward2.legs)) {
      const int STATE = (lts.decayforward2.legs[i].legs.size() > 0)
                            ? PDG::PDG_DECAY
                            : PDG::PDG_STABLE;
      // TBD: ADD HERE THE ctau > 1.0 cm definition for the status code

      HepMC3::GenParticlePtr particle = std::make_shared<HepMC3::GenParticle>(
          gra::aux::M4Vec2HepMC3(lts.decayforward2.legs[i].p4),
          lts.decayforward2.legs[i].p.pdg, STATE);
...
void MProcess::SetProcess(std::string &str,
                          const std::vector<aux::OneCMD> &syntax) {
  // SET IT HERE!
  PROCESS = str;

  // Call this always first
  PDG.ReadParticleData(gra::aux::GetBasePath(2) +
                       "/modeldata/mass_width_2018.mcd");
  ...
  for (const auto &i : indices(syntax)) {
    if (syntax[i].id.find("PDG") != std::string::npos) {
      ...
      if (left == std::string::npos || right == std::string::npos) {
        throw std::invalid_argument(
            "MProcess::SetProcess: invalid @PDG[]{} syntax");
      }
      // Read pdg id / number
      const int pdg =
          std::stoi(syntax[i].id.substr(left + 1, right - left - 1));
      ...
      try {
        ...
        throw std::invalid_argument(
            "MProcess::SetProcess: syntax problem in @PDG[ID] with ID = " +
            std::to_string(pdg));
      }
    }
  }
  ...
  if (Processes.count(str)) {
    ...
    throw std::invalid_argument("MProcess::SetProcess: Unknown PROCESS: " +
                                str);
  }
}
