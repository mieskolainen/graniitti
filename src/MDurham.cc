...
//
std::complex<double> MDurham::DurhamQCD(gra::LORENTZSCALAR &lts,
  ...
  if (gra::GlobalSudakovPtr->initialized == false) {
    try {
      ...
      gra::g_mutex
          .unlock(); // need to release here, otherwise get infinite lock
      throw;
    }
  }
...
MDurham::DHelProj(const std::vector<std::complex<double>> &A,
...
inline void MDurham::DScaleChoise(double qt2, double q1_2, double q2_2,
                                  double &Q1_2_scale,
  ...
  if (Durham::PDF_scale == "MIN") {
    ...
    throw std::invalid_argument(
        "MDurham::DScaleChoise: Unknown 'Durham::PDF_scale' option!");
  }
}
...
//
std::complex<double>
MDurham::DQtloop(gra::LORENTZSCALAR &lts,
                 std::vector<std::vector<std::complex<double>>> Amp) {
  ...
  const static MMatrix<double> WSimpson =
      math::Simpson38Weight2D(Durham::N_qt, Durham::N_phi);
  ...
  for (std::size_t i = 0; i < Durham::N_qt + 1; ++i) {
    ...
    for (std::size_t j = 0; j < Durham::N_phi + 1; ++j) {
      ...
      const double fg_1 =
          gra::GlobalSudakovPtr->fg_xQ2M(lts.x1, Q1_2_scale, MU);
      const double fg_2 =
          gra::GlobalSudakovPtr->fg_xQ2M(lts.x2, Q2_2_scale, MU);
  ...
  for (std::size_t h = 0; h < f.size(); ++h) {
    sum[h] = math::Simpson38Integral2D(f[h], WSimpson, Durham::qt_STEP,
                                       Durham::phi_STEP);
  }
  ...
  for (std::size_t h = 0; h < f.size(); ++h) {
    ...
    lts.hamp[h] *= lts.excite1 ? gra::form::S3FINEL(lts.t1, lts.pfinal[1].M2())
                               : gra::form::S3F(lts.t1);
    lts.hamp[h] *= lts.excite2 ? gra::form::S3FINEL(lts.t2, lts.pfinal[2].M2())
                               : gra::form::S3F(lts.t2);
...
void MDurham::Dgg2chic0(const gra::LORENTZSCALAR &lts,
  ...
  const double alpha_s =
      gra::GlobalSudakovPtr->AlphaS_Q2(lts.s_hat / Durham::alphas_scale);
  gra::g_mutex.unlock();
  // @@ MUTEX LOCK @@
  ...
  std::complex<double> A =
      K_NLO * 8.0 * math::zi * gs2 / M0 * msqrt(0.075) / msqrt(PI * M0 * NC);
...
void MDurham::Dgg2gg(const gra::LORENTZSCALAR &lts,
  ...
  for (std::size_t h = 0; h < fs2.size(); ++h) {
    ...
    if (fs2[h] == PP || fs2[h] == MM) {
      ...
      is[MM] = 0; // -- => +- or -+
      is[MP] = negphase * norm *
               ((fs2[h] == PM) ? 1 / aux23 : aux23); // -+ => +- or -+
      is[PM] = posphase * norm *
               ((fs2[h] == PM) ? aux23 : 1 / aux23); // +- => +- or -+
      is[PP] = 0;                                    // ++ => +- or -+
    }
    Amp[h] = is;
  ...
  if (DEBUG) {
    ...
    for (unsigned int i = 0; i < 16; ++i) {
      printf("i=%2d : |mad|^2 = %0.3f , |amp|^2 = %0.3f \n", i,
             math::abs2(lts.hamp[i]), math::abs2(dualAmp[i]));
...
std::vector<double> MDurham::EvalPhi(int N, int pdg) const {
  ...
  double fM = 0.0;
  static const std::vector<double> supported = {111, 211, 321,
                                                311}; // pi0, pi+, K+, K0
...
void MDurham::Dgg2MMbar(const gra::LORENTZSCALAR &lts,
  ...
  const double delta_AB =
      8;               // Sum over \delta^AB [gluons in with the same color]
  ...
  const double costheta =
      (1.0 + 2.0 * lts.t_hat / lts.s_hat - 2.0 * pow2(m) / lts.s_hat) / beta;
  ...
  auto T_SFS_PP = [&](double x, double y) {
    ...
    return 1.0 / (x * y * (1.0 - x) * (1.0 - y)) * (1.0 + costheta2) /
           pow2(1.0 - costheta2);
  };
  ...
  static const std::vector<double> xval =
      math::linspace<std::vector>(CUTOFF, 1.0 - CUTOFF, Nx + 1);
  ...
  const double norm =
      (delta_AB / NC) * (64.0 * math::PIPI * pow2(alpha_s)) / shat;
  ...
  if ((std::find(SFO_PDG.begin(), SFO_PDG.end(), pdg0) != SFO_PDG.end())) {
    ...
    is[PP] = 0.0;
    is[MM] = 0.0;
    is[PM] = norm * math::Simpson38Integral2D(f_PM, WSimpson, STEPx, STEPx) *
             posphase;
...
void MDurham::Dgg2qqbar(const gra::LORENTZSCALAR &lts,
                        std::vector<std::vector<std::complex<double>>> &Amp) {
  throw std::invalid_argument(
      "MDurham::DurhamQCD: qqbar amplitude in the next version");
}
