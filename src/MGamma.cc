...
std::complex<double> MGamma::yyffbar(gra::LORENTZSCALAR &lts) {
  ...
  const double mass =
      lts.decaytree[0].p4.M(); // lepton, quark (or monopole) mass
  const double mass2 = pow2(mass);
  const bool MONOPOLE_MODE =
      (lts.decaytree[0].p.pdg == PDG::PDG_monopole) ? true : false;

  if (PARAM_MONOPOLE::gn < 1) {
    throw std::invalid_argument(
        "MGamma::yyffbar: Parameter Dirac n less than 1");
  }
  ...
  if (MONOPOLE_MODE) {
    ...
    if (PARAM_MONOPOLE::coupling == "beta-dirac") {
      ...
      // Faster way
      const double beta =
          msqrt(1.0 - 4.0 * pow2(PARAM_MONOPOLE::M0) / lts.s_hat);
      COUPL = pow4(g * beta);
    ...
    // summed
    const double amp2 =
        2.0 * COUPL * ((lts.u_hat - mass2) / (lts.t_hat - mass2) +
...
std::complex<double> MGamma::yyHiggs(gra::LORENTZSCALAR &lts) const {
  ...
  const double Gamma_yy =
      2.27E-3 * Gamma; // Higgs to gamma-gamma calculated (GeV)
...
//
std::complex<double> MGamma::yyX(const gra::LORENTZSCALAR &lts,
