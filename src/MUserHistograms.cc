...
void MUserHistograms::InitHistograms() {
	...
	h1["Pt"]       = MH1<double>(Nbins, "System Pt (GeV)");
	...
	h2["costhetaphi_PG"] = MH2(Nbins, -1.0, 1.0, Nbins, -PI, PI, "(cos(theta), phi) [Pseudo-GJ frame]");
	h2["costhetaphi_GJ"] = MH2(Nbins, -1.0, 1.0, Nbins, -PI, PI, "(cos(theta), phi) [Gottfried-Jackson frame]");
	h2["costhetaphi_SR"] = MH2(Nbins, -1.0, 1.0, Nbins, -PI, PI, "(cos(theta), phi) [Non-Rotated rest frame]");
	...
	h2["Mphi_HE"]        = MH2(Nbins,  1.0, 1.5, Nbins, -PI, PI, "(M, phi) [Helicity frame]");
	h2["Mphi_AH"]        = MH2(Nbins,  1.0, 1.5, Nbins, -PI, PI, "(M, phi) [Anti-Helicity frame]");
	h2["Mphi_PG"]        = MH2(Nbins,  1.0, 1.5, Nbins, -PI, PI, "(M, phi) [Pseudo-GJ frame]");
	//h2["Mphi_GJ"]      = MH2(Nbins,  1.0, 1.5, Nbins, -PI, PI, "(M, phi) [Gottfried-Jackson frame]");
	h2["Mphi_SR"]        = MH2(Nbins,  1.0, 1.5, Nbins, -PI, PI, "(M, phi) [Non-Rotated rest frame]");
	//h2["Mphi_LA"]      = MH2(Nbins,  1.0, 1.5, Nbins, -PI, PI, "(M, phi) [LAB frame]");
	*/
}
...
void MUserHistograms::FillCosThetaPhi(double totalweight, const gra::LORENTZSCALAR& lts) {
	...
	for (const auto& k : indices(frametype)) {
		...
		//h2["Mphi_"+frametype[k]].Fill(gra::math::msqrt(lts.m2), pfout[0].Phi(), totalweight);		
	}
	// ------------------------------------------------------------------
	...
	h2["costhetaphiHE"].Fill(std::cos(pfHE[0].Theta()), pfHE[0].Phi(), totalweight);
	*/
