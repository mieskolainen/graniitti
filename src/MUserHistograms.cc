...
void MUserHistograms::InitHistograms() {
  ...
  h1["dPhi_pp"] =
      MH1<double>(Nbins, 0.0, gra::math::PI, "Forward deltaphi (rad)");
  ...
  h2["costhetaphi_CS"] = MH2(Nbins, -1.0, 1.0, Nbins, -PI, PI,
                             "(cos(theta), phi) [Collins-Soper frame]");
  h2["costhetaphi_HE"] = MH2(Nbins, -1.0, 1.0, Nbins, -PI, PI,
                             "(cos(theta), phi) [Helicity frame]");
  h2["costhetaphi_AH"] = MH2(Nbins, -1.0, 1.0, Nbins, -PI, PI,
                             "(cos(theta), phi) [Anti-Helicity frame]");
  h2["costhetaphi_PG"] = MH2(Nbins, -1.0, 1.0, Nbins, -PI, PI,
                             "(cos(theta), phi) [Pseudo-GJ frame]");
  h2["costhetaphi_GJ"] = MH2(Nbins, -1.0, 1.0, Nbins, -PI, PI,
                             "(cos(theta), phi) [Gottfried-Jackson frame]");
  h2["costhetaphi_SR"] = MH2(Nbins, -1.0, 1.0, Nbins, -PI, PI,
                             "(cos(theta), phi) [Non-Rotated rest frame]");
  h2["costhetaphi_LA"] =
...
void MUserHistograms::FillHistograms(double totalweight,
  ...
  if (HIST >= 1) {
    ...
    h1["dPhi_pp"].Fill(std::abs(lts.pfinal[1].DeltaPhi(lts.pfinal[2])),
                       totalweight);
...
// event weight
void MUserHistograms::FillCosThetaPhi(double totalweight,
                                      const gra::LORENTZSCALAR &lts) {
  ...
  gra::kinematics::LorentFramePrepare(pf, lts.pbeam1, lts.pbeam2, pb1boost,
                                      pb2boost, pfboost);
  ...
  for (const auto &k : indices(frametype)) {
    ...
    gra::kinematics::LorentzFrame(pfout, pb1boost, pb2boost, pfboost,
                                  frametype[k], direction);
    h2["costhetaphi_" + frametype[k]].Fill(std::cos(pfout[0].Theta()),
                                           pfout[0].Phi(), totalweight);
  ...
  */
  ...
  h2["costhetaphi_GJ"].Fill(std::cos(pfGJ[0].Theta()), pfGJ[0].Phi(),
                            totalweight);
