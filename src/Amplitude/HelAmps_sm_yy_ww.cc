...
void ixxxxx(double p[4], double fmass, int nhel, int nsf,
	...
	if (fmass != 0.0) {
		...
		if (sqp0p3 == 0.0) {
			chi[1] =
			    complex<double>(-nhel * sqrt(2.0 * p[0]), 0.0);
		} else {
...
void txxxxx(double p[4], double tmass, int nhel, int nst,
	...
	if (nhel >= 0) {
		if (pp == 0) {
			...
			if (pt != 0) {
				pzpt = p[3] / (pp * pt) * sqh;
				ep[1] = complex<double>(
				    -p[1] * pzpt, -nst * p[2] / pt * sqh);
				ep[2] = complex<double>(-p[2] * pzpt,
				                        nst * p[1] / pt * sqh);
			} else {
				ep[1] = complex<double>(-sqh, 0);
				ep[2] =
				    complex<double>(0, nst * Sgn(sqh, p[3]));
			}
		}
	}
	...
	if (nhel == 2) {
		...
		if (nhel == 1) {
			for (j = 0; j < 4; j++) {
				for (i = 0; i < 4; i++)
					ft[i][j] = sqh * (ep[i] * e0[j] +
					                  e0[i] * ep[j]);
			}
		} else if (nhel == 0) {
			for (j = 0; j < 4; j++) {
				for (i = 0; i < 4; i++)
					ft[i][j] = sqs * (ep[i] * em[j] +
					                  em[i] * ep[j] +
					                  2.0 * e0[i] * e0[j]);
			}
		} else if (nhel == -1) {
...
void vxxxxx(double p[4], double vmass, int nhel, int nsv,
	...
	if (vmass != 0.0) {
		...
		if (pp == 0.0) {
			...
			vc[5] = complex<double>(
			    hel0 * p[3] * emp + hel * pt / pp * sqh, 0.0);
			if (pt != 0.0) {
		...
		if (pt != 0.0) {
			pzpt = p[3] / (pp * pt) * sqh * hel;
			vc[3] = complex<double>(-p[1] * pzpt,
			                        -nsv * p[2] / pt * sqh);
			vc[4] = complex<double>(-p[2] * pzpt,
			                        nsv * p[1] / pt * sqh);
		} else {
			vc[3] = complex<double>(-hel * sqh, 0.0);
...
void oxxxxx(double p[4], double fmass, int nhel, int nsf,
	...
	if (fmass != 0.000) {
		...
		if (pp == 0.000) {
			...
			pp = min(p[0], sqrt((p[1] * p[1]) + (p[2] * p[2]) +
			                    (p[3] * p[3])));
		...
		if (sqp0p3 == 0.000) {
			chi[1] =
			    complex<double>(-nhel, 0.00) * sqrt(2.0 * p[0]);
		} else {
			chi[1] = complex<double>(nh * p[1], -p[2]) / sqp0p3;
		}
		if (nh == 1) {
...
void VVV1_2(std::complex<double> V1[], std::complex<double> V3[],
	...
	TMP7 = (V1[2] * P2[0] - V1[3] * P2[1] - V1[4] * P2[2] - V1[5] * P2[3]);
	TMP10 =
	    (P2[0] * P3[0] - P2[1] * P3[1] - P2[2] * P3[2] - P2[3] * P3[3]);
	TMP3 = (V3[2] * V1[2] - V3[3] * V1[3] - V3[4] * V1[4] - V3[5] * V1[5]);
	TMP2 = (V3[2] * P2[0] - V3[3] * P2[1] - V3[4] * P2[2] - V3[5] * P2[3]);
	denom = COUP / ((P2[0] * P2[0]) - (P2[1] * P2[1]) - (P2[2] * P2[2]) -
	                (P2[3] * P2[3]) - M2 * (M2 - cI * W2));
	V2[2] = denom *
	        (OM2 * P2[0] * (TMP3 * (-cI * (TMP9) + cI * (TMP10)) +
	                        (-cI * (TMP2 * TMP8) + cI * (TMP0 * TMP7))) +
	         (TMP3 * (-cI * (P3[0]) + cI * (P1[0])) +
	          (V1[2] * (-cI * (TMP0) + cI * (TMP2)) +
	           V3[2] * (-cI * (TMP7) + cI * (TMP8)))));
	V2[3] = denom *
	        (OM2 * P2[1] * (TMP3 * (-cI * (TMP9) + cI * (TMP10)) +
	                        (-cI * (TMP2 * TMP8) + cI * (TMP0 * TMP7))) +
	         (TMP3 * (-cI * (P3[1]) + cI * (P1[1])) +
	          (V1[3] * (-cI * (TMP0) + cI * (TMP2)) +
	           V3[3] * (-cI * (TMP7) + cI * (TMP8)))));
	V2[4] = denom *
	        (OM2 * P2[2] * (TMP3 * (-cI * (TMP9) + cI * (TMP10)) +
	                        (-cI * (TMP2 * TMP8) + cI * (TMP0 * TMP7))) +
	         (TMP3 * (-cI * (P3[2]) + cI * (P1[2])) +
	          (V1[4] * (-cI * (TMP0) + cI * (TMP2)) +
	           V3[4] * (-cI * (TMP7) + cI * (TMP8)))));
	V2[5] = denom *
	        (OM2 * P2[3] * (TMP3 * (-cI * (TMP9) + cI * (TMP10)) +
	                        (-cI * (TMP2 * TMP8) + cI * (TMP0 * TMP7))) +
	         (TMP3 * (-cI * (P3[3]) + cI * (P1[3])) +
	          (V1[5] * (-cI * (TMP0) + cI * (TMP2)) +
	           V3[5] * (-cI * (TMP7) + cI * (TMP8)))));
}
...
void VVVV2_0(std::complex<double> V1[], std::complex<double> V2[],
	...
	TMP13 =
	    (V2[2] * V4[2] - V2[3] * V4[3] - V2[4] * V4[4] - V2[5] * V4[5]);
	TMP14 =
	    (V3[2] * V4[2] - V3[3] * V4[3] - V3[4] * V4[4] - V3[5] * V4[5]);
	TMP6 = (V3[2] * V2[2] - V3[3] * V2[3] - V3[4] * V2[4] - V3[5] * V2[5]);
	TMP1 = (V2[2] * V1[2] - V2[3] * V1[3] - V2[4] * V1[4] - V2[5] * V1[5]);
	TMP3 = (V3[2] * V1[2] - V3[3] * V1[3] - V3[4] * V1[4] - V3[5] * V1[5]);
	TMP12 =
	    (V1[2] * V4[2] - V1[3] * V4[3] - V1[4] * V4[4] - V1[5] * V4[5]);
	vertex = COUP * (-1.) * (-2. * cI * (TMP1 * TMP14) +
	                         cI * (TMP6 * TMP12 + TMP3 * TMP13));
}
...
void VVV1_3(std::complex<double> V1[], std::complex<double> V2[],
	...
	V3[2] = denom *
	        (OM3 * P3[0] * (TMP1 * (-cI * (TMP10) + cI * (TMP11)) +
	                        (-cI * (TMP4 * TMP8) + cI * (TMP5 * TMP7))) +
	         (TMP1 * (-cI * (P1[0]) + cI * (P2[0])) +
	          (V1[2] * (-cI * (TMP5) + cI * (TMP4)) +
	           V2[2] * (-cI * (TMP7) + cI * (TMP8)))));
	V3[3] = denom *
	        (OM3 * P3[1] * (TMP1 * (-cI * (TMP10) + cI * (TMP11)) +
	                        (-cI * (TMP4 * TMP8) + cI * (TMP5 * TMP7))) +
	         (TMP1 * (-cI * (P1[1]) + cI * (P2[1])) +
	          (V1[3] * (-cI * (TMP5) + cI * (TMP4)) +
	           V2[3] * (-cI * (TMP7) + cI * (TMP8)))));
	V3[4] = denom *
	        (OM3 * P3[2] * (TMP1 * (-cI * (TMP10) + cI * (TMP11)) +
	                        (-cI * (TMP4 * TMP8) + cI * (TMP5 * TMP7))) +
	         (TMP1 * (-cI * (P1[2]) + cI * (P2[2])) +
	          (V1[4] * (-cI * (TMP5) + cI * (TMP4)) +
	           V2[4] * (-cI * (TMP7) + cI * (TMP8)))));
	V3[5] = denom *
	        (OM3 * P3[3] * (TMP1 * (-cI * (TMP10) + cI * (TMP11)) +
	                        (-cI * (TMP4 * TMP8) + cI * (TMP5 * TMP7))) +
	         (TMP1 * (-cI * (P1[3]) + cI * (P2[3])) +
	          (V1[5] * (-cI * (TMP5) + cI * (TMP4)) +
