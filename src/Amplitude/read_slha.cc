...
double SLHABlock::get_entry(std::vector<int> indices, double def_val) {
	if (_entries.find(indices) == _entries.end()) {
		std::cout << "Warning: No such entry in " << _name
		...
		if (line != "" && line[0] != '#') {
			...
			if (line.find("decay ") == 0) {
				...
				if (linestr >> pdg_code >> value)
					set_block_entry("decay", pdg_code,
					                value);
				else
...
double SLHAReader::get_block_entry(std::string block_name,
                                   std::vector<int> indices, double def_val) {
	if (_blocks.find(block_name) == _blocks.end()) {
		std::cout << "No such block " << block_name
		          << ", using default value " << def_val << std::endl;
		return def_val;
	}
...
void SLHAReader::set_block_entry(std::string block_name,
