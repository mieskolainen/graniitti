...
void AMP_MG5_yy_ll::initProc(string param_card_name) {
...
double AMP_MG5_yy_ll::CalcAmp2(gra::LORENTZSCALAR &lts, double alphas) {
...
void AMP_MG5_yy_ll::calculate_wavefunctions(const int perm[], const int hel[]) {
  ...
  FFV1_2(w[2], w[0], pars.GC_3, pars.ZERO, pars.ZERO, w[4]);
  FFV1_1(w[3], w[0], pars.GC_3, pars.ZERO, pars.ZERO, w[5]);

  // Calculate all amplitudes
  // Amplitude(s) for diagram number 0
  FFV1_0(w[4], w[3], w[1], pars.GC_3, amp[0]);
  FFV1_0(w[2], w[5], w[1], pars.GC_3, amp[1]);
}
double AMP_MG5_yy_ll::matrix_1_aa_epem() {
  int i, j;
  // Local variables
  const int            ngraphs = 2;
  const int            ncolor  = 1;
