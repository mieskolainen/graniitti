...
std::complex<double> MAmpMG5_yy_ll_2to4::CalcAmp(gra::LORENTZSCALAR &lts) {
  ...
  const std::vector<double> masses = {
      lts.pbeam2.M(),    lts.pbeam1.M(),          lts.pfinal[1].M(),
      lts.pfinal[0].M(), lts.decaytree[0].p4.M(), lts.decaytree[1].p4.M()};
  mME = masses;

  // *** Set particle 4-momentum: [E,px,py,pz] convention here! ***
  double p1[] = {lts.pbeam2.E(), lts.pbeam2.Px(), lts.pbeam2.Py(),
                 lts.pbeam2.Pz()};
  double p2[] = {lts.pbeam1.E(), lts.pbeam1.Px(), lts.pbeam1.Py(),
                 lts.pbeam1.Pz()};
  double p3[] = {lts.pfinal[1].E(), lts.pfinal[1].Px(), lts.pfinal[1].Py(),
                 lts.pfinal[1].Pz()};
  double p4[] = {lts.pfinal[0].E(), lts.pfinal[0].Px(), lts.pfinal[0].Py(),
                 lts.pfinal[0].Pz()};
  double p6[] = {lts.decaytree[0].p4.E(), lts.decaytree[0].p4.Px(),
                 lts.decaytree[0].p4.Py(), lts.decaytree[0].p4.Pz()};
  double p5[] = {lts.decaytree[1].p4.E(), lts.decaytree[1].p4.Px(),
...
void MAmpMG5_yy_ll_2to4::calculate_wavefunctions(const int perm[],
                                                 const int hel[]) {
