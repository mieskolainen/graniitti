...
void AMP_MG5_yy_ww_evev::initProc(string param_card_name) {
...
double AMP_MG5_yy_ww_evev::CalcAmp2(gra::LORENTZSCALAR &lts, double alphas) {
  ...
  if (sum_hel == 0 || ntry < 10) {
    ...
    for (int ihel = 0; ihel < ncomb; ihel++) {
      if (goodhel[ihel] || ntry < 2) {
        ...
        t[0] = matrix_1_aa_wpwm_wp_epve_wm_emvex();
...
double AMP_MG5_yy_ww_evev::sigmaHat() {
...
void AMP_MG5_yy_ww_evev::calculate_wavefunctions(const int perm[], const int hel[]) {
  ...
  FFV2_3(w[6], w[5], pars.GC_100, pars.mdl_MW, pars.mdl_WW, w[7]);
  VVV1_2(w[0], w[4], -pars.GC_3, pars.mdl_MW, pars.mdl_WW, w[8]);
  VVV1_3(w[0], w[7], -pars.GC_3, pars.mdl_MW, pars.mdl_WW, w[9]);
...
double AMP_MG5_yy_ww_evev::matrix_1_aa_wpwm_wp_epve_wm_emvex() {
  int i, j;
  // Local variables
  const int            ngraphs = 3;
  const int            ncolor  = 1;
