...
std::complex<double> MTensorPomeron::ME3(gra::LORENTZSCALAR &lts,
  ...
  const std::array<std::vector<std::complex<double>>, 2> u_a =
      SpinorStates(pa, "u");
  const std::array<std::vector<std::complex<double>>, 2> u_b =
      SpinorStates(pb, "u");

  const std::array<std::vector<std::complex<double>>, 2> ubar_1 =
      SpinorStates(p1, "ubar");
  const std::array<std::vector<std::complex<double>>, 2> ubar_2 =
      SpinorStates(p2, "ubar");
  ...
  if (J == 0 && P == 1) {
    ...
    throw std::invalid_argument(
        "MTensorPomeron::ME3: Unknown spin-parity input");
  }
  // ====================================================================
      ...
      for (const auto &h1 : {0, 1}) {
        for (const auto &h2 : {0, 1}) {
          ...
          const std::complex<double> A =
              (-zi) * iG_1a(mu1, nu1) * iDP_1(mu1, nu1, alpha1, beta1) *
...
std::complex<double> MTensorPomeron::ME4(gra::LORENTZSCALAR &lts) const {
  ...
  // t-channel Pomeron propagators
  const Tensor4<std::complex<double>, 4, 4, 4, 4> iDP_13 =
      iD_P(lts.ss[1][3], lts.t1);
  const Tensor4<std::complex<double>, 4, 4, 4, 4> iDP_24 =
      iD_P(lts.ss[2][4], lts.t2);

  // u-channel Pomeron propagators
  const Tensor4<std::complex<double>, 4, 4, 4, 4> iDP_14 =
      iD_P(lts.ss[1][4], lts.t1);
  const Tensor4<std::complex<double>, 4, 4, 4, 4> iDP_23 =
      iD_P(lts.ss[2][3], lts.t2);
  ...
  const std::array<std::vector<std::complex<double>>, 2> v_3 =
      SpinorStates(p3, "v");
  const std::array<std::vector<std::complex<double>>, 2> ubar_4 =
      SpinorStates(p4, "ubar");
  ...
  if (lts.decaytree[0].p.spinX2 == 0 && lts.decaytree[1].p.spinX2 == 0) {
    ...
    throw std::invalid_argument("MTensorPomeron::ME4: Invalid daughter spin "
    ...
    for (const auto &hb : {0, 1}) {
      for (const auto &h1 : {0, 1}) {
        for (const auto &h2 : {0, 1}) {
          ...
          if (SPINMODE == 2) {
            // t-channel blocks
            const Tensor4<std::complex<double>, 4, 4, 4, 4> iG_tA =
                iG_Pvv(pt, -p3);
            const Tensor2<std::complex<double>, 4, 4> iDMES_t =
                iD_VMES(pt, M_mes, Gamma_mes);
            const Tensor4<std::complex<double>, 4, 4, 4, 4> iG_tB =
                iG_Pvv(p4, pt);

            // u-channel blocks
            const Tensor4<std::complex<double>, 4, 4, 4, 4> iG_uA =
                iG_Pvv(p4, pu);
            const Tensor2<std::complex<double>, 4, 4> iDMES_u =
                iD_VMES(pu, M_mes, Gamma_mes);
            const Tensor4<std::complex<double>, 4, 4, 4, 4> iG_uB =
                iG_Pvv(pu, -p3);
            ...
            // t-channel
            {
              ...
              // Apply off-shell form FACTORs
              M_t(rho3, rho4) =
                  A(rho3, rho1) * iDMES_t(rho1, rho2) * B(rho2, rho4) *
                  pow2(PARAM_REGGE::Meson_FF(pt.M2(), pow2(M_mes)));
            }
            ...
            // u-channel
            {
              ...
              // Apply off-shell form FACTORs
              M_u(rho3, rho4) =
                  A(rho4, rho1) * iDMES_u(rho1, rho2) * B(rho2, rho3) *
                  pow2(PARAM_REGGE::Meson_FF(pu.M2(), pow2(M_mes)));
            }
            ...
            if (OPTION == 2) {
              ...
              for (const auto &h3 : {0, 1, 2}) {
                for (const auto &h4 : {0, 1, 2}) {
                  ...
                  // amplitude
                  // (ha,hb,h1,h2,h3,h4)
                  const std::complex<double> amp = eps_3_conj[h3](rho3) *
                                                   eps_4_conj[h4](rho4) *
                                                   M(rho3, rho4);
...
std::array<std::vector<std::complex<double>>, 2>
MTensorPomeron::SpinorStates(const M4Vec &p, std::string type) const {
...
// 0,1,2]
std::array<Tensor1<std::complex<double>, 4>, 3>
MTensorPomeron::Spin1States(const M4Vec &p, std::string type) const {
...
//
//
Tensor1<std::complex<double>, 4>
MTensorPomeron::iG_yee(const M4Vec &prime, const M4Vec &p,
...
//
Tensor1<std::complex<double>, 4>
MTensorPomeron::iG_ypp(const M4Vec &prime, const M4Vec &p,
                       const std::vector<std::complex<double>> &ubar,
                       const std::vector<std::complex<double>> &u) const {
...
Tensor2<std::complex<double>, 4, 4>
MTensorPomeron::iG_PppHE(const M4Vec &prime, const M4Vec p) const {
  Tensor2<std::complex<double>, 4, 4> T;
  const M4Vec psum = prime + p;

  const std::complex<double> FACTOR =
      -zi * 3.0 * betaPNN * F1((prime - p).M2());
...
//
Tensor2<std::complex<double>, 4, 4>
MTensorPomeron::iG_Ppp(const M4Vec &prime, const M4Vec &p,
                       const std::vector<std::complex<double>> &ubar,
                       const std::vector<std::complex<double>> &u) const {
  const double t = (prime - p).M2();
  const M4Vec psum = prime + p;
...
Tensor4<std::complex<double>, 4, 4, 4, 4> MTensorPomeron::iG_PppbarP(
    const M4Vec &prime, const std::vector<std::complex<double>> &ubar,
    const M4Vec &pt, const MMatrix<std::complex<double>> &iSF,
    const std::vector<std::complex<double>> &v, const M4Vec &p) const {
  const std::complex<double> lhs_FACTOR =
      -zi * 3.0 * betaPNN * F1((prime - pt).M2());
  const std::complex<double> rhs_FACTOR =
      -zi * 3.0 * betaPNN * F1((pt - p).M2());
  ...
  for (const auto &mu2 : LI) {
    for (const auto &nu2 : LI) {
      lhs_A = (gamma_lo[mu2] * (lhs_psum % nu2) +
               gamma_lo[nu2] * (lhs_psum % mu2)) *
              0.5;
...
//
Tensor4<std::complex<double>, 4, 4, 4, 4>
MTensorPomeron::iG_PPS_total(const M4Vec &q1, const M4Vec &q2, double M0,
...
//
Tensor4<std::complex<double>, 4, 4, 4, 4>
MTensorPomeron::iG_PPS_1(double g_PPS) const {
  ...
  for (const auto &u : LI) {
    for (const auto &v : LI) {
      for (const auto &k : LI) {
        for (const auto &l : LI) {
          T(u, v, k, l) = FACTOR * (g[u][k] * g[v][l] + g[u][k] * g[v][k] -
                                    0.5 * g[u][v] * g[k][l]);
        }
      }
    }
...
//
Tensor4<std::complex<double>, 4, 4, 4, 4>
MTensorPomeron::iG_PPS_2(const M4Vec &q1, const M4Vec &q2, double g_PPS) const {
  const double S0 = 1.0; // Mass scale (GeV)
...
//
Tensor4<std::complex<double>, 4, 4, 4, 4>
MTensorPomeron::iG_PPPS_1(const M4Vec &q1, const M4Vec &q2,
                          double g_PPPS) const {
  const double S0 = 1.0; // Mass scale (GeV)
    ...
    for (const auto &v : LI) {
      for (const auto &k : LI) {
        for (const auto &l : LI) {
          ...
          for (const auto &r : LI) {
            for (const auto &s : LI) {
              T(u, v, k, l) += FACTOR * (g[u][k] * eps_lo(v, l, r, s) +
...
//
Tensor4<std::complex<double>, 4, 4, 4, 4>
MTensorPomeron::iG_PPPS_2(const M4Vec &q1, const M4Vec &q2,
                          double g_PPPS) const {
  const double S0 = 1.0; // Mass scale (GeV)
  const M4Vec q1_q2 = q1 - q2;
  const M4Vec q = q1 + q2;
...
// Input as contravariant (upper index) 4-vector, M0 the resonance peak mass
//
MTensor<std::complex<double>>
MTensorPomeron::iG_PPT_total(const M4Vec &q1, const M4Vec &q2, double M0,
                             const std::vector<double> &g_PPT) const {
  MTensor<std::complex<double>> T =
      MTensor({4, 4, 4, 4, 4, 4}, std::complex<double>(0.0));
  auto FM_ = [](double t) -> double {
    const double LAMBDA = 1.0; // GeV
    return 1.0 / (1.0 - t / pow2(LAMBDA));
  };
                    ...
                    for (auto const &r1 : LI) {
                      for (auto const &s1 : LI) {
                        for (auto const &u1 : LI) {
                          if (v1 != a1 || l1 != r1 || s1 != u1) {
                            continue;
                          } // speed it up

                          T({u, v, k, l, r, s}) =
                              FACTOR * R_DDDD(u, v, u1, v1) *
...
//
MTensor<std::complex<double>> MTensorPomeron::iG_PPT_23(const M4Vec &q1,
                                                        const M4Vec &q2,
                                                        int mode,
                                                        double g_PPT) const {
  MTensor<std::complex<double>> T =
                    ...
                    for (auto const &u1 : LI) {
                      T({u, v, k, l, r, s}) =
                          ...
                          (q1q2 * R_DDDD(u, v, r1, a1) * R_DDDU(k, l, s1, a1) +
                           ...
                           sign * (q1 ^ (u1)) * (q2 % (s1)) *
                               R_DDDD(u, v, r1, a1) * R_DDDU(k, l, u1, a1) +
...
//
Tensor1<std::complex<double>, 4>
MTensorPomeron::iG_rhopipi(const M4Vec &k1, const M4Vec &k2) const {
...
//
Tensor2<std::complex<double>, 4, 4>
MTensorPomeron::iG_f2pipi(const M4Vec &k1, const M4Vec &k2) const {
  // Form FACTOR
...
//
Tensor4<std::complex<double>, 4, 4, 4, 4>
MTensorPomeron::iG_f2yy(const M4Vec &k1, const M4Vec &k2) const {
  const double e = msqrt(alpha_QED * 4.0 * PI); // ~ 0.3
  ...
  const std::complex<double> FACTOR =
      zi * FM(k1.M2()) * FM(k2.M2()) * F_f2yy((k1 + k2).M2());
...
//
Tensor2<std::complex<double>, 4, 4>
MTensorPomeron::iG_Ppsps(const M4Vec &prime, const M4Vec &p) const {
  double beta_P = 0.0;
...
//
Tensor4<std::complex<double>, 4, 4, 4, 4>
MTensorPomeron::iG_Pvv(const M4Vec &prime, const M4Vec &p) const {
  // Couplings (phi meson)
...
//
Tensor4<std::complex<double>, 4, 4, 4, 4> MTensorPomeron::iD_P(double s,
                                                               double t) const {
  Tensor4<std::complex<double>, 4, 4, 4, 4> T;
  ...
  for (const auto &u : LI) {
    for (const auto &v : LI) {
      for (const auto &k : LI) {
        for (const auto &l : LI) {
          T(u, v, k, l) = FACTOR * (g[u][k] * g[v][l] + g[u][l] * g[v][k] -
                                    0.5 * g[u][v] * g[k][l]);
        }
      }
    }
...
//
Tensor4<std::complex<double>, 4, 4, 4, 4>
MTensorPomeron::iD_2R(double s, double t) const {
  Tensor4<std::complex<double>, 4, 4, 4, 4> T;
...
// Input as (sub)-Mandelstam invariants s,t
//
Tensor2<std::complex<double>, 4, 4> MTensorPomeron::iD_1R(double s,
                                                          double t) const {
  Tensor2<std::complex<double>, 4, 4> T;
...
// Input as (sub)-Mandelstam invariants s,t
//
Tensor2<std::complex<double>, 4, 4> MTensorPomeron::iD_O(double s,
                                                         double t) const {
  Tensor2<std::complex<double>, 4, 4> T;
...
//
std::complex<double> MTensorPomeron::iD_MES(const M4Vec &p, double M0,
                                            double Gamma) const {
...
//
Tensor2<std::complex<double>, 4, 4>
MTensorPomeron::iD_VMES(const M4Vec &p, double M0, double Gamma) const {
  Tensor2<std::complex<double>, 4, 4> T;
...
//
Tensor4<std::complex<double>, 4, 4, 4, 4>
MTensorPomeron::iD_TMES(const M4Vec &p, double M0, double Gamma) const {
  const double m2 = p.M2();
  Tensor4<std::complex<double>, 4, 4, 4, 4> T;
...
//
Tensor4<std::complex<double>, 4, 4, 4, 4>
MTensorPomeron::Gamma0(const M4Vec &k1, const M4Vec &k2) const {
...
//
Tensor4<std::complex<double>, 4, 4, 4, 4>
MTensorPomeron::Gamma2(const M4Vec &k1, const M4Vec &k2) const {
  const double k1k2 = k1 * k2; // 4-dot product
  Tensor4<std::complex<double>, 4, 4, 4, 4> T;
...
void MTensorPomeron::CalcRTensor() {
  ...
  R(a, b, c, d) = 0.5 * gT(a, c) * gT(b, d) + 0.5 * gT(a, d) * gT(b, c) +
                  0.25 * gT(a, b) * gT(c, d);
