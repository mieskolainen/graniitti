...
double MFactorized::EventWeight(const std::vector<double> &randvec,
                                AuxIntData &aux) {
  double W = 0.0;
  ...
  if (aux.Valid()) {
    ...
    double exact = 0.0;
    DecayWidthPS(exact);
    lts.DW_sum_exact.Add(gra::kinematics::MCW(exact, pow2(exact), 1),
                         aux.vegasweight);
    ...
    // ** EVENT WEIGHT **
    W = C_space * (1.0 / S_factor) * B51PhaseSpaceWeight() *
        B51IntegralVolume() * MatESQ * GeV2barn / MollerFlux();
  }
...
void MFactorized::PrintInit(bool silent) const {
  if (!silent) {
    ...
    printf("- System rapidity (Rap) [min, max] = [%0.2f, %0.2f]     \t(user) \n"
           ...
           "\t(fixed/user) \n",
           gcuts.Y_min, gcuts.Y_max, gcuts.M_min, gcuts.M_max,
...
bool MFactorized::B51BuildKin(double pt1, double pt2, double phi1, double phi2,
  ...
  double p1z =
      gra::kinematics::SolvePz(m1, m2, pt1, pt2, pX.Pz(), pX.E(), lts.s);
  ...
  if (lts.decaytree.size() == 2) {
    w = gra::kinematics::TwoBodyPhaseSpace(lts.pfinal[0], msqrt(m2X), masses,
                                           products, random);
    // 3-body
    ...
    w = gra::kinematics::NBodyPhaseSpace(lts.pfinal[0], msqrt(m2X), masses,
                                         products, UNWEIGHT, random);
  }
...
double MFactorized::B51IntegralVolume() const {
  ...
  return (pow2(M_MAX) - pow2(M_MIN)) * (2.0 * gra::math::PI) *
         (2.0 * gra::math::PI) * (gcuts.forward_pt_max - gcuts.forward_pt_min) *
         (gcuts.forward_pt_max - gcuts.forward_pt_min) *
         (gcuts.Y_max - gcuts.Y_min) * M2_forward_volume;
}
...
double MFactorized::B51PhaseSpaceWeight() const {
  const double J =
      1.0 / std::abs(lts.pfinal[1].Pz() / lts.pfinal[1].E() -
                     lts.pfinal[2].Pz() /
                         lts.pfinal[2].E()); // Jacobian, close to 0.5
