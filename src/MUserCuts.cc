...
bool UserCut(int id, const gra::LORENTZSCALAR& lts) {
	...
	// STAR/RHIC \sqrt{s} = 200 GeV pi+pi- (+ other cuts needed in .json file)
		...
		const double xi1 =
		    (lts.pbeam1.Pz() - lts.pfinal[1].Pz()) / lts.pbeam1.Pz();
		const double xi2 =
		    (lts.pbeam2.Pz() - lts.pfinal[2].Pz()) / lts.pbeam2.Pz();
	...
	// https://arxiv.org/abs/hep-ex/170804053
		...
		if (12 <= M && M < 30) { // GeV
			if (lts.decaytree[0].p4.Pt() > 6 &&
			    lts.decaytree[1].p4.Pt() > 6) {
				// fine
			} else {
				return false; // not passed
			}                        
		} else if (30 <= M && M <= 70) { // GeV
			if (lts.decaytree[0].p4.Pt() > 10 &&
			    lts.decaytree[1].p4.Pt() > 10) {
				// fine
			} else {
				return false; // not passed
			}
	...
	// c.f. cut |t| > 0.03 GeV^2 seems to give more physical results)
		...
		std::string str =
		    "MUserCuts:: Unknown cut ID: " + std::to_string(id);
