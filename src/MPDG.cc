...
void MPDG::ReadParticleData(const std::string& filepath) {
	...
	while (std::getline(infile, line)) {
		...
		sscanf(line.c_str(), "%33c %35c %35c %21c", cid, cmass, cwidth,
		       cname);
		...
		for (std::size_t k = 0; k < id.size(); ++k) {
			...
			if (p.spinX2 == 0 || p.spinX2 == 2 || p.spinX2 == 4 || p.spinX2 == 8 || p.spinX2 == 10) {  // Is a boson
			...
			if (p.spinX2 == 1 || p.spinX2 == 3 || p.spinX2 == 5 || p.spinX2 == 7 || p.spinX2 == 9) { // Is a fermion
			...
			if (((int)std::abs(p.spinX2 / 2.0) !=
			         (double)std::abs(p.spinX2 / 2.0) &&
			     id.size() < 3) ||
...
void MPDG::TokenizeProcess(const std::string& str, int depth, std::vector<gra::MDecayBranch>& branches) const {
	...
	for (std::string::size_type i = 0; i < str.size(); ++i) {
		...
		if (str[i] == '>') {
			...
			for (std::string::size_type j = i + 1; j < str.size();
				...
				if ((L.size() == R.size()) &&
				    L.size() >
				        0) { // Found outer closing brackets
					a = L[0];
					b = R[R.size() - 1];
    ...
    while (it != PDG_table.end()) {
		...
		printf("%d\t%10d\t%0.6f\t%0.6f\t%2s\t%s%s%s\t%s \n",
				++counter, p.pdg, p.mass, p.width,
		       	...
		       	gra::aux::ParityToString(p.P).c_str(),
