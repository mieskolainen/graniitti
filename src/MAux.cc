...
void AutoDownloadLHAPDF(const std::string pdfname) {
  std::cout << rang::fg::red
            << "aux::AutoDownloadLHAPDF: Trying automatic download:"
  ...
  if (INSTALLPATH.find("command not found") != std::string::npos) {
    throw std::invalid_argument(
        "aux::AutoDownloadLHAPDF: Failure: lhapdf-config command missing");
  }

  // Download, untar
  std::string cmd1 =
      "wget http://www.hepforge.org/archive/lhapdf/pdfsets/6.2/" + pdfname +
...
std::string GetInputData(const std::string &inputfile) {
  ...
  if (gra::aux::FileExist(inputfile) == false) {
    std::string str =
        "gra::aux::GetInputData: Inputfile '" + inputfile + "' does not exist";
...
std::string GetVersionTLatex() {
  ...
  snprintf(buff, sizeof(buff),
           "#color[16]{#scale[0.6]{GRANIITTI #scale[0.8]{%0.2f}#it{#beta}}}",
           GetVersion());
  std::string str = buff;
  return str;
}
...
std::string GetWebTLatex() {
  char buff[100];
  snprintf(buff, sizeof(buff),
           "#color[16]{#scale[0.5]{#LTgithub.com/mieskolainen#GT}}");
  std::string str = buff;
  return str;
}
  ...
  for (std::size_t i = 0; i < subcmd.size(); ++i) {
    ...
    if (Lpos == std::string::npos && Rpos == std::string::npos) {
      ...
      if (strip.size() == 1) {
        ...
        throw std::invalid_argument(
            "gra::SplitCommands: @Syntax invalid with '" + subcmd[i] + "'");
      }
      ...
      for (std::size_t i = 0; i < keyvals.size(); ++i) {
        ...
        if (strip.size() != 2) {
          throw std::invalid_argument(
              "gra::SplitCommands: @Syntax not good with '" + keyvals[i] + "'");
        }
