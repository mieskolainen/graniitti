...
#define BAR "||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||||"
	...
	std::string GetVersionTLatex() {
		...
		snprintf(buff, sizeof(buff),
				 "#color[16]{#scale[0.6]{GRANIITTI #scale[0.8]{%0.2f}#it{#beta}}}", GetVersion());
		std::string str = buff;
		return str;
	}
		...
		for(std::size_t i = 0; i < subcmd.size(); ++i) {
			...
			if(Lpos == std::string::npos && Rpos == std::string::npos) {
				...
				if(strip.size() == 1) {
					...
					throw std::invalid_argument("gra::SplitCommands: @Syntax invalid with '" +
												subcmd[i] + "'");
				}
				...
				for(std::size_t i = 0; i < keyvals.size(); ++i) {
					...
					if(strip.size() != 2) {
						throw std::invalid_argument("gra::SplitCommands: @Syntax not good with '" +
													keyvals[i] + "'");
					}
