  ...
  if (first == "X") {
    ...
    channels.insert(std::pair<std::string, std::string>(
        "ND", "Non-Diffractive                          [N-cut soft Pomerons] "
              ...
              "CONSTRUCTION @@"));
    descriptions.insert(
        std::pair<std::string, std::map<std::string, std::string>>("X",
                                                                   channels));
  } else if (first == "PP") {
    std::map<std::string, std::string> channels;
    channels.insert(std::pair<std::string, std::string>(
    ...
    channels.insert(std::pair<std::string, std::string>(
        "CON",
        "Regge continuum 2/4/6-body               [Pomeron] x [Pomeron]"));
    channels.insert(std::pair<std::string, std::string>(
        "CON-",
        "Regge continuum 2-body negative subamp   [Pomeron] x [Pomeron]"));
    ...
    channels.insert(std::pair<std::string, std::string>(
        "RES",
        "Regge parametric resonance               [Pomeron] x [Pomeron]"));
    channels.insert(std::pair<std::string, std::string>(
        "RESHEL", "Regge sliding helicity amplitudes        [Pomeron] x "
                  "[Pomeron] @@ UNDER "
                  "CONSTRUCTION @@"));
    descriptions.insert(
        std::pair<std::string, std::map<std::string, std::string>>("PP",
                                                                   channels));
  } else if (first == "yP") {
    std::map<std::string, std::string> channels;
    channels.insert(std::pair<std::string, std::string>(
    ...
    descriptions.insert(
        std::pair<std::string, std::map<std::string, std::string>>("yP",
                                                                   channels));
  } else if (first == "yy") {
    std::map<std::string, std::string> channels;
    channels.insert(std::pair<std::string, std::string>(
    ...
    channels.insert(std::pair<std::string, std::string>(
        "monopolium(0)", "Gamma-Gamma to Monopolium "
                         "(J=0)          [EPA] x [EPA]  "
                         "       @@ UNDER TESTING @@"));
    channels.insert(std::pair<std::string, std::string>(
        "CON", "Gamma-Gamma to l+l-, w+w-, monopoles     [EPA] x [EPA]"));
    channels.insert(std::pair<std::string, std::string>(
        "QED", "Gamma-Gamma qq -> q l+ l- q   "
               "           [FULL QED]         "
               "   @@ TEST PROCESS @@"));
    descriptions.insert(
        std::pair<std::string, std::map<std::string, std::string>>("yy",
                                                                   channels));
  } else if (first == "gg") {
    std::map<std::string, std::string> channels;
    channels.insert(std::pair<std::string, std::string>(
    ...
    channels.insert(std::pair<std::string, std::string>(
        "CON", "QCD continuum to gg, 2 x "
               "pseudoscalar    [Durham QCD]  "
               "        @@ UNDER TESTING @@"));
    descriptions.insert(
        std::pair<std::string, std::map<std::string, std::string>>("gg",
                                                                   channels));
  } else if (first == "yy_DZ") {
    std::map<std::string, std::string> channels;
    channels.insert(std::pair<std::string, std::string>(
    ...
    descriptions.insert(
        std::pair<std::string, std::map<std::string, std::string>>("yy_DZ",
                                                                   channels));
  } else if (first == "yy_LUX") {
    std::map<std::string, std::string> channels;
    channels.insert(std::pair<std::string, std::string>(
        "CON", "Collinear yy to l+l, w+w- or "
               "monopoles   [LUX] x [LUX]     "
               "    @@ UNDER TESTING @@"));
    descriptions.insert(
        std::pair<std::string, std::map<std::string, std::string>>("yy_LUX",
                                                                   channels));
  }
}
...
void MSubProc::SetTechnicalBoundaries(gra::GENCUT &gcuts,
                                      unsigned int EXCITATION) {
...
std::complex<double> MSubProc::GetBareAmplitude(gra::LORENTZSCALAR &lts) {
  if (ISTATE == "X") {
    ...
    throw std::invalid_argument("MSubProc::GetBareAmplitude: Unknown ISTATE '" +
                                ISTATE + '"');
  }
}
...
MSubProc::GetBareAmplitude_X(gra::LORENTZSCALAR &lts) {
  ...
  if (CHANNEL == "EL" || CHANNEL == "SD" || CHANNEL == "DD") {
    ...
    throw std::invalid_argument(
        "MSubProc::GetBareAmplitude: Unknown CHANNEL = " + CHANNEL);
  }
  return A;
}
...
// Pomeron-Pomeron
inline std::complex<double>
MSubProc::GetBareAmplitude_PP(gra::LORENTZSCALAR &lts) {
  // ------------------------------------------------------------------
  ...
  if (CHANNEL == "RES") {
    ...
    if (lts.decaytree.size() == 2) {
      ...
      std::transform(tempsum.begin(), tempsum.end(), lts.hamp.begin(),
                     tempsum.begin(), std::plus<std::complex<double>>());
    ...
    if (lts.decaytree.size() == 2) {
      ...
      throw std::invalid_argument(
          "MSubProc: Only 2-body final states for [CON-] process");
    }
...
// Gamma-Pomeron
inline std::complex<double>
MSubProc::GetBareAmplitude_yP(gra::LORENTZSCALAR &lts) {
  std::complex<double> A(0, 0);
...
// Gamma-Gamma
inline std::complex<double>
MSubProc::GetBareAmplitude_yy(gra::LORENTZSCALAR &lts) {
  std::complex<double> A(0, 0);
...
inline std::complex<double>
MSubProc::GetBareAmplitude_yy_DZ(gra::LORENTZSCALAR &lts) {
...
// *** UNDER TESTING ***
inline std::complex<double>
MSubProc::GetBareAmplitude_yy_LUX(gra::LORENTZSCALAR &lts) {
  // @@ MULTITHREADING LOCK @@
  gra::g_mutex.lock();
  ...
  retry:
    try {
      ...
      ++gra::pdf_trials;
      std::string str =
          "MSubProc::InitLHAPDF: Problem with reading '" + pdfname + "'";
...
// Durham gg
inline std::complex<double>
MSubProc::GetBareAmplitude_gg(gra::LORENTZSCALAR &lts) {
  std::complex<double> A(0, 0);
