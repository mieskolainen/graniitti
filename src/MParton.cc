...
double MParton::EventWeight(const std::vector<double> &randvec,
                            AuxIntData &aux) {
  double W = 0.0;
...
bool MParton::B2BuildKin(double x1, double x2) {
  ...
  if (lts.decaytree.size() == 2) {
    w = gra::kinematics::TwoBodyPhaseSpace(lts.pfinal[0], lts.pfinal[0].M(),
                                           masses, products, random);
    // 3-body
    ...
    w = gra::kinematics::NBodyPhaseSpace(lts.pfinal[0], lts.pfinal[0].M(),
                                         masses, products, UNWEIGHT, random);
  }
