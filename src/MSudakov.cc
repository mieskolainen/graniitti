...
void ReadParameters() {
  ...
  try {
    ...
    std::string str = "MSudakovNumerics::ReadParameters: Error parsing " +
...
void MSudakov::Init(double _sqrts, const std::string &PDFSET,
  ...
  MSudakovNumerics::x_MIN =
      math::pow2(MSudakovNumerics::M_MIN / MSudakovNumerics::sqrts);
...
void MSudakov::InitArrays() {
  // Init Sudakov variables
  {
    ...
    const std::string filename = "./sudakov/SUDA_" +
                                 gra::aux::ToString(veto.sqrts, 0) + "_" +
  ...
  // Init Shuvaev PDF variables
  {
    ...
    const std::string filename = "./sudakov/SHUV_" +
                                 gra::aux::ToString(spdf.sqrts, 0) + "_" +
                                 PDFSETNAME + "_" + std::to_string(hash);
...
double MSudakov::xg_xQ2(double x, double q2) const {
  ...
  try {
    ...
    std::string str = "MSudakov::xg_xQ2: Problem with x = " +
                      std::to_string(x) + ", q2 = " + std::to_string(q2);
    throw std::invalid_argument(str);
  }
}
...
double MSudakov::AlphaS_Q2(double q2) const {
  try {
    ...
    std::string str =
        "MSudakov::AlphaS_Q2: Problem with q2 = " + std::to_string(q2);
    throw std::invalid_argument(str);
  }
}
...
std::pair<double, double> MSudakov::Shuvaev_H(double q2, double x) {
  ...
  if (x >= MSudakovNumerics::x_MIN * (1 - EPS) &&
      x <= MSudakovNumerics::x_MAX * (1 + EPS) &&
...
std::pair<double, double> MSudakov::Sudakov_T(double qt2, double mu) {
  // Scale functor
  auto DeltaScale = [](double kt2, double M) -> double {
    return math::msqrt(kt2) / M;
  };
  ...
  const double dTg = Tg * AlphaS_Q2(qt2) / (2.0 * math::PI * qt2) *
                     (AP_gg(delta) + AP_qg(delta, qt2));
  return {Tg, dTg};
}
...
double MSudakov::AP_gg(double delta) const {
  ...
  return 2.0 * CA * (std::log(1.0 / delta) -
                     math::pow2(1.0 - delta) *
...
double MSudakov::AP_qg(double delta, double qt2) const {
  ...
  return TR * (-2.0 * math::pow3(delta) / 3.0 + math::pow2(delta) - delta +
               2.0 / 3.0) *
         NumFlavor(qt2);
}
...
void MSudakov::CalculateArray(
    IArray2D &arr, std::pair<double, double> (MSudakov::*f)(double, double)) {
  MTimer timer;
...
std::pair<double, double> IArray2D::Interpolate2D(double a, double b) const {
  ...
  for (std::size_t C = 2; C <= 3; ++C) {
    ...
    // Interpolated valued
    values[C - 2] = ((y2 - yval) / ystep) * ((x2 - xval) / xstep * Q11 +
                                             (xval - x1) / xstep * Q21) +
                    ((yval - y1) / ystep) *
                        ((x2 - xval) / xstep * Q12 + (xval - x1) / xstep * Q22);
  }
