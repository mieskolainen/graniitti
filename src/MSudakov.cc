...
namespace MSudakovNumerics {
	...
	void ReadParameters() {
		...
		try {
			...
			std::vector<bool> xx = j[XID]["SUDA"]["log_ON"]; SUDA_log_ON = xx;
			...
			std::string str =
			    "MSudakovNumerics::ReadParameters: Error parsing " + inputfile + " (Check for extra/missing commas)";
...
// Init Sudakov variables
{
	...
	veto.Set(0,"q2", MSudakovNumerics::q2_MIN, MSudakovNumerics::q2_MAX, MSudakovNumerics::SUDA_N[0], MSudakovNumerics::SUDA_log_ON[0]);
	veto.Set(1, "M", MSudakovNumerics::M_MIN,  MSudakovNumerics::M_MAX,  MSudakovNumerics::SUDA_N[1], MSudakovNumerics::SUDA_log_ON[1]);
	veto.InitArray(); // Initialize (call last!)

	const unsigned long hash = gra::aux::djb2hash(veto.GetHashString());
	const std::string filename = "./sudakov/SUDA_" + gra::aux::ToString(veto.sqrts,0) + "_" + PDFSETNAME + "_" + std::to_string(hash);
...
// Init Shuvaev PDF variables
{
	...
	spdf.Set(0,"q2", MSudakovNumerics::q2_MIN, MSudakovNumerics::q2_MAX, MSudakovNumerics::SHUV_N[0], MSudakovNumerics::SHUV_log_ON[0]);
	spdf.Set(1, "x", MSudakovNumerics::x_MIN,  MSudakovNumerics::x_MAX,  MSudakovNumerics::SHUV_N[1], MSudakovNumerics::SHUV_log_ON[1]);
	spdf.InitArray(); // Initialize (call last!)

	const unsigned long hash = gra::aux::djb2hash(spdf.GetHashString());
	const std::string filename = "./sudakov/SHUV_" + gra::aux::ToString(spdf.sqrts,0) + "_" + PDFSETNAME + "_" + std::to_string(hash);
...
double MSudakov::xg_xQ2(double x, double q2) const {
	...
	try {
		...
		std::string str = "MSudakov::xg_xQ2: Problem with x = " +
		                  std::to_string(x) + ", q2 = " +
		                  std::to_string(q2);
		throw std::invalid_argument(str);
	}
	...
	for (std::size_t i = 0; i < NM+1; ++i) {
		...
		for (std::size_t j = 0; j < NX+1; ++j) {
			...
			for (std::size_t k = 0; k < NQ2+1; ++k) {
				...
				printf("(x = %0.3E, q2 = %0.2f, M = %0.1f) : [gluon pdf: xg(x,q2), Durham flux: fg(x,q2,M)] = (%0.2f,%0.2f) \n",
				x, q2, M, xf, hxf);
			}
			std::cout << std::endl;
		}
...
double MSudakov::AlphaS_Q2(double q2) const {
	try {
		...
		std::string str = "MSudakov::AlphaS_Q2: Problem with q2 = " +
		                  std::to_string(q2);
		throw std::invalid_argument(str);
	}
}
...
// Calculate Sudakov real radiation suppression (veto) factor via direct numerical
// integral.
//
// T(Q_t^2, \mu) = exp( -\int_{Q_t^2}^{\mu^2} dk_t^2 \frac{1}{k_t^2} \frac{\alpha_s(k_t^2)}{2\pi}
//                       \int_0^{1-DELTA} dz[zP_gg(z) + \sum_q P_qg(z)] ),
//
// where DELTA = k_t/mu [REFERENCE: Coughlin, Forshaw, https://arxiv.org/abs/0912.3280v2]
//
...
std::pair<double,double> IArray2D::Interpolate2D(double a, double b) const {
	...
	if (a > MAX[0]*(1+EPS) || b > MAX[1]*(1+EPS)) {
		printf("Interpolate2D(%s,%s) Input out of grid domain: "
			...
			name[0].c_str(), name[1].c_str(), name[0].c_str(),
			a, MIN[0], MAX[0], name[1].c_str(), b, MIN[1], MAX[1]);
	}
	...
	for (std::size_t C = 2; C <= 3; ++C) {
		...
		// Interpolated valued
		values[C-2] =
		    ((y2 - yval) / ystep) * ((x2 - xval) / xstep * Q11 +
		                             (xval - x1) / xstep * Q21) +
		    ((yval - y1) / ystep) * ((x2 - xval) / xstep * Q12 +
		                             (xval - x1) / xstep * Q22);
	}
