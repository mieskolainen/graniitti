...
bool MContinuum::LoopKinematics(const std::vector<double> &p1p,
  ...
  double p1z =
      gra::kinematics::SolvePz(m1, m2, pt1, pt2, sumP.Pz(), sumP.E(), lts.s);
  ...
  if (std::abs(beamsum.Pz()) > 1e-6) {
    ...
    for (std::size_t i = 0; i < Kf; ++i) {
      kinematics::LorentzBoost(beamsum, lts.sqrt_s, lts.pfinal[i + offset],
                               sign);
    }
  }
  // ------------------------------------------------------------------
  ...
  if (!gra::math::CheckEMC(beamsum -
                           (lts.pfinal[1] + lts.pfinal[2] + lts.pfinal[0]))) {
    return false;
  }
...
// Get weight
double MContinuum::EventWeight(const std::vector<double> &randvec,
                               AuxIntData &aux) {
  double W = 0.0;
  ...
  if (aux.Valid()) {
    ...
    const double MatESQ =
        (FLATAMP == 0) ? abs2(S3ScreenedAmp()) : GetFlatAmp2(lts);
...
void MContinuum::PrintInit(bool silent) const {
  if (!silent) {
    ...
    for (const auto &i : indices(lts.decaytree)) {
      char buff[250];
      snprintf(buff, sizeof(buff), "x---------> %d (%s) [Q=%s, J=%s]",
               lts.decaytree[i].p.pdg, lts.decaytree[i].p.name.c_str(),
    ...
    std::cout << rang::style::bold << "Generation cuts:" << rang::style::reset
              << std::endl
              << std::endl;
    printf("- Final state rapidity (Rap) [min, max] = [%0.2f, %0.2f]     "
           "\t(user)       \n"
           ...
           "\t(fixed/user) "
           "\n",
           gcuts.rap_min, gcuts.rap_max, gcuts.kt_min, gcuts.kt_max,
...
bool MContinuum::BNRandomKin(unsigned int Nf,
                             const std::vector<double> &randvec) {
  const unsigned int Kf = Nf - 2; // Central system multiplicity

  const double pt1 = gcuts.forward_pt_min +
                     (gcuts.forward_pt_max - gcuts.forward_pt_min) * randvec[0];
  const double pt2 = gcuts.forward_pt_min +
                     (gcuts.forward_pt_max - gcuts.forward_pt_min) * randvec[1];
...
bool MContinuum::BNBuildKin(unsigned int Nf, double pt1, double pt2,
                            double phi1, double phi2,
                            const std::vector<double> &kt,
                            const std::vector<double> &phi,
                            const std::vector<double> &y, double m1,
                            double m2) {
  const unsigned int Kf = Nf - 2; // Central system multiplicity
  static const M4Vec beamsum = lts.pbeam1 + lts.pbeam2;

  if (lts.decaytree.size() != Kf) {
    std::string str = "MContinuum::BNBuildKin: Decaytree first level topology "
                      "= " +
                      std::to_string(lts.decaytree.size()) + " (should be " +
                      std::to_string(Kf) + " for this process)!";
...
void MContinuum::BLinearSystem(std::vector<M4Vec> &p,
                               const std::vector<M4Vec> &q, const M4Vec &p1f,
  ...
  */
  ...
  static const std::vector<std::vector<std::vector<double>>> A = {
      ...
      {{11.0 / 7.0, 5.0 / 7.0, -5.0 / 7.0, -4.0 / 7.0, -3.0 / 7.0, -2.0 / 7.0,
        -1.0 / 7.0},
       {15.0 / 14.0, 17.0 / 14.0, -5.0 / 7.0, -4.0 / 7.0, -3.0 / 7.0,
        -2.0 / 7.0, -1.0 / 7.0},
      ...
      {{29.0 / 16.0, 15.0 / 16.0, -3.0 / 4.0, -5.0 / 8.0, -1.0 / 2.0,
       ...
       {21.0 / 16.0, 23.0 / 16.0, -3.0 / 4.0, -5.0 / 8.0, -1.0 / 2.0,
        -3.0 / 8.0, -1.0 / 4.0, -1.0 / 8.0},
       {13.0 / 16.0, 15.0 / 16.0, 1.0 / 4.0, -5.0 / 8.0, -1.0 / 2.0, -3.0 / 8.0,
        -1.0 / 4.0, -1.0 / 8.0},
       {5.0 / 16.0, 7.0 / 16.0, 1.0 / 4.0, 3.0 / 8.0, -1.0 / 2.0, -3.0 / 8.0,
        -1.0 / 4.0, -1.0 / 8.0},
       {-3.0 / 16.0, -1.0 / 16.0, 1.0 / 4.0, 3.0 / 8.0, 1.0 / 2.0, -3.0 / 8.0,
        -1.0 / 4.0, -1.0 / 8.0},
       {-11.0 / 16.0, -9.0 / 16.0, 1.0 / 4.0, 3.0 / 8.0, 1.0 / 2.0, 5.0 / 8.0,
        -1.0 / 4.0, -1.0 / 8.0},
       {-19.0 / 16.0, -17.0 / 16.0, 1.0 / 4.0, 3.0 / 8.0, 1.0 / 2.0, 5.0 / 8.0,
        3.0 / 4.0, -1.0 / 8.0},
       {-27.0 / 16.0, -25.0 / 16.0, 1.0 / 4.0, 3.0 / 8.0, 1.0 / 2.0, 5.0 / 8.0,
        3.0 / 4.0, 7.0 / 8.0}}};
...
double MContinuum::BNPhaseSpaceWeight() const {
  ...
  const double factor =
      pow4(2.0 * PI) * (1.0 / std::pow(2.0 * pow3(2.0 * PI), Nf)) *
