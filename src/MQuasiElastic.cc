...
bool MQuasiElastic::LoopKinematics(const std::vector<double> &p1p,
  ...
  // SOLVE pz and E
  const double p1z = kinematics::SolvePz(m1, m2, lts.pfinal[1].Pt(),
                                         lts.pfinal[2].Pt(), 0, 0, lts.s);
  const double p2z = -p1z; // by momentum conservation

  // pz and E of protons/N*
  lts.pfinal[1].SetPzE(p1z,
                       msqrt(pow2(m1) + pow2(lts.pfinal[1].Pt()) + pow2(p1z)));
  lts.pfinal[2].SetPzE(p2z,
                       msqrt(pow2(m2) + pow2(lts.pfinal[2].Pt()) + pow2(p2z)));
...
// Get weight
double MQuasiElastic::EventWeight(const std::vector<double> &randvec,
                                  AuxIntData &aux) {
  double W = 0.0;
...
bool MQuasiElastic::EventRecord(HepMC3::GenEvent &evt) {
  ...
  if (ProcPtr.CHANNEL == "ND") {
    ...
    for (const auto &i : indices(etree)) {
      ...
      for (std::size_t c = 0; c < NCHAIN; ++c) {
        ...
        if (ExciteContinuum(pomeron4vec, gen_X, evt, Q2_scale, B_sum, Q_sum) ==
            1) {
  ...
  if (lts.excite1 == true) {
    ...
    // Try to fragment
    if (ExciteContinuum(lts.pfinal[1], gen_p1f, evt, lts.pfinal[1].M2(), 1,
                        1) == 1) {
      return false; // failed
    }
  }
  ...
  if (lts.excite2 == true) {
    ...
    // Try to fragment
    if (ExciteContinuum(lts.pfinal[2], gen_p2f, evt, lts.pfinal[2].M2(), 1,
                        1) == 1) {
      return false; // failed
    }
  }
...
//
std::complex<double>
MQuasiElastic::PolySoft(const std::vector<double> &randvec) {
  // Boundary conditions
...
bool MQuasiElastic::B3BuildKin(double s3, double s4, double t) {
  ...
  // frame
  const double pnorm =
      kinematics::DecayMomentum(lts.sqrt_s, msqrt(s3), msqrt(s4));
