...
void ReadParameters() {
  ...
  const std::string inputfile =
      gra::aux::GetBasePath(2) + "/modeldata/" + "NUMERICS.json";
  ...
  try {
    ...
    std::string str = "MEikonalNumerics::ReadParameters: Error parsing " +
...
void MEikonal::S3Constructor(double s_in,
  ...
  // Proton density
  {
    ...
    MBT.Set("bt", MEikonalNumerics::MinBT, MEikonalNumerics::MaxBT,
            MEikonalNumerics::NumberBT, MEikonalNumerics::logBT);
    MBT.InitArray(); // Initialize (call last!)

    const std::string hstr =
        std::to_string(s) + std::to_string(INITIALSTATE[0].pdg) + "_" +
    ...
    const std::string filename = gra::aux::GetBasePath(2) + "/eikonal/" +
                                 "MBT_" + std::to_string(INITIALSTATE[0].pdg) +
                                 "_" + std::to_string(INITIALSTATE[1].pdg) +
                                 "_" + gra::aux::ToString(msqrt(s), 0) + "_" +
                                 std::to_string(hash);
  ...
  if (onlyeikonal == false) {
    ...
    MSA.Set("kt2", MEikonalNumerics::MinKT2, MEikonalNumerics::MaxKT2,
            MEikonalNumerics::NumberKT2, MEikonalNumerics::logKT2);
    MSA.InitArray(); // Initialize (call last!)

    const std::string hstr =
        std::to_string(s) + std::to_string(INITIALSTATE[0].pdg) +
        std::to_string(INITIALSTATE[1].pdg) + PARAM_SOFT::GetHashString() +
        MEikonalNumerics::GetHashString();

    const unsigned long hash = gra::aux::djb2hash(hstr);
    const std::string filename = gra::aux::GetBasePath(2) + "/eikonal/" +
                                 "MSA_" + std::to_string(INITIALSTATE[0].pdg) +
                                 "_" + std::to_string(INITIALSTATE[1].pdg) +
                                 "_" + gra::aux::ToString(msqrt(s), 0) + "_" +
                                 std::to_string(hash);
    ...
    while (!ok) { // Problem, re-calculate
      // Pointer to member function: ReturnType
      // (ClassType::*)(ParameterTypes...)
      std::complex<double> (MEikonal::*f)(double) const =
          &MEikonal::S3Screening;
...
std::complex<double> MEikonal::S3Density(double bt) const {
  ...
  const double kt_STEP =
      (MEikonalNumerics::FBIntegralMaxKT - MEikonalNumerics::FBIntegralMinKT) /
  ...
  if (INITIALSTATE[0].pdg == PDG::PDG_p && INITIALSTATE[1].pdg == PDG::PDG_p) {
    // TBD
    // ppbar
  } else if ((INITIALSTATE[0].pdg == PDG::PDG_p &&
              INITIALSTATE[1].pdg == -PDG::PDG_p) ||
             (INITIALSTATE[0].pdg == -PDG::PDG_p &&
              INITIALSTATE[1].pdg == PDG::PDG_p)) {
    // TBD
  }
  ...
  for (const auto &i : indices(f)) {
    ...
    std::complex<double> A =
        gra::math::pow2(PARAM_SOFT::gN_P) * F_i * F_k *
...
std::complex<double> MEikonal::S3Screening(double kt2) const {
  ...
  for (const auto &i : indices(f)) {
    ...
    const std::complex<double> A =
        gra::math::zi * (1.0 - std::exp(gra::math::zi * XI / 2.0));
...
void MEikonal::S3CalcXS() {
  ...
  const MMatrix<double> cc = {{1, 1, 1, 1},    // pp
                              ...
                              {1, 1, -1, -1}}; // N*N*
  // Eigenvalues
  const std::vector<double> lambda = {gra::math::pow2(1.0 - PARAM_SOFT::gamma),
                                      gra::math::pow2(1.0 + PARAM_SOFT::gamma),
                                      1.0 - gra::math::pow2(PARAM_SOFT::gamma),
                                      1.0 - gra::math::pow2(PARAM_SOFT::gamma)};
  // ----------------------------------------------------------------------------
  ...
  std::vector<std::vector<std::complex<double>>> f_2(
      4, std::vector<std::complex<double>>(N + 1, 0.0));
  ...
  for (const auto &i : indices(f_tot)) {
    ...
    for (std::size_t k = 0; k < 4; ++k) {
      // Amplitude squared
      f_2[k][i] = gra::math::abs2((sol[0] * cc[k][0] + sol[1] * cc[k][1] +
                                   sol[2] * cc[k][2] + sol[3] * cc[k][3]) /
                                  4.0);
      f_2[k][i] *= bt; // Jacobian
    }
  ...
  sigma_tot =
      2.0 * IC * std::real(gra::math::CSIntegral(f_tot, STEP)) * PDG::GeV2barn;
  sigma_el = IC * std::real(gra::math::CSIntegral(f_el, STEP)) * PDG::GeV2barn;
  sigma_inel =
      IC * std::real(gra::math::CSIntegral(f_in, STEP)) * PDG::GeV2barn;
  ...
  const double sigma_el_2 =
      IC * std::real(gra::math::CSIntegral(f_2[0], STEP)) * PDG::GeV2barn;
  const double sigma_sd_a =
      IC * std::real(gra::math::CSIntegral(f_2[1], STEP)) * PDG::GeV2barn;
  const double sigma_sd_b =
      IC * std::real(gra::math::CSIntegral(f_2[2], STEP)) * PDG::GeV2barn;
  const double sigma_dd =
      IC * std::real(gra::math::CSIntegral(f_2[3], STEP)) * PDG::GeV2barn;

  // Scale
  const double kappa = sigma_el / sigma_el_2;

  std::cout << "Two Channel normalized to Single Channel [PROTOTEST]"
            << std::endl;
...
void MEikonal::S3CalculateArray(IArray1D &arr,
                                std::complex<double> (MEikonal::*f)(double)
                                    const) {
  std::cout << "MEikonal::S3CalculateArray:" << std::endl;
  std::vector<std::future<std::complex<double>>>
      futures; // std::async return values
  MTimer timer(true);
...
bool IArray1D::WriteArray(const std::string &filename, bool overwrite) const {
  ...
  for (std::size_t i = 0; i < F.size_row(); ++i) {
    // Write to file
    file << std::setprecision(15) << std::real(F[i][X]) << ","
         << std::real(F[i][Y]) << "," << std::imag(F[i][Y]) << std::endl;
  }
  printf("Time elapsed %0.1f sec \n", timer.ElapsedSec());
  file.close();
  return true;
...
std::complex<double> IArray1D::Interpolate1D(double a) const {
  ...
  return F[i][Y] +
         (a - F[i][X]) * ((F[i + 1][Y] - F[i][Y]) / (F[i + 1][X] - F[i][X]));
}
...
void MEikonal::S3InitCutPomerons() {
  ...
  for (std::size_t j = 0; j < MEikonalNumerics::NumberBT + 1; ++j) {
    ...
    for (std::size_t m = 1; m < MCUT; ++m) {
      // Poisson ansatz
      double P_m =
          std::pow(2 * XI, m) / gra::math::factorial(m) * std::exp(-2 * XI);
