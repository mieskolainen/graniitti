...
void MEikonal::S3Constructor(double s_in, const std::vector<gra::MParticle>& initialstate_in, bool onlyeikonal) {
	...
	// Proton density
	{
		...
		MBT.Set("bt", MEikonalNumerics::MinBT, MEikonalNumerics::MaxBT, MEikonalNumerics::NumberBT, MEikonalNumerics::logBT);
		MBT.InitArray(); // Initialize (call last!)

		const std::string hstr = std::to_string(s) + std::to_string(INITIALSTATE[0].pdg) + "_" + std::to_string(INITIALSTATE[1].pdg) +
		                         PARAM_SOFT::GetHashString() + MEikonalNumerics::GetHashString();
		const unsigned long hash = gra::aux::djb2hash(hstr);
		const std::string filename = gra::aux::GetBasePath(2) +
									"/eikonal/" + "MBT_" + std::to_string(INITIALSTATE[0].pdg) + 
									 "_" + std::to_string(INITIALSTATE[1].pdg) +
		                             "_" + gra::aux::ToString(msqrt(s),0) + "_" + std::to_string(hash);

		// Try to read pre-calculated
		bool ok = MBT.ReadArray(filename);

		while (!ok) { // Problem, re-calculate
			// Pointer to member function: ReturnType (ClassType::*)(ParameterTypes...)
	...
	if (onlyeikonal == false) {
		...
		MSA.Set("kt2", MEikonalNumerics::MinKT2, MEikonalNumerics::MaxKT2, MEikonalNumerics::NumberKT2, MEikonalNumerics::logKT2);
		MSA.InitArray(); // Initialize (call last!)

		const std::string hstr = std::to_string(s) 
								 + std::to_string(INITIALSTATE[0].pdg)
								 + std::to_string(INITIALSTATE[1].pdg)
								 + PARAM_SOFT::GetHashString()
								 + MEikonalNumerics::GetHashString();
								 
		const unsigned long hash   = gra::aux::djb2hash(hstr);
		const std::string filename = gra::aux::GetBasePath(2) + "/eikonal/" + "MSA_" +
									 std::to_string(INITIALSTATE[0].pdg)  + "_" + std::to_string(INITIALSTATE[1].pdg) +
		                             "_" + gra::aux::ToString(msqrt(s),0) + "_" + std::to_string(hash);
...
// [REFERENCE: Desgrolard, Giffon, Martynov, Predazzi, https://arxiv.org/abs/hep-ph/9907451v2]
// [REFERENCE: Desgrolard, Giffon, Martynov, Predazzi, https://arxiv.org/abs/hep-ph/0001149]
//
// 
	...
	for (const auto& i : indices(f_tot)) {
		...
		for (std::size_t k = 0; k < 4; ++k) {
			...
			f_2[k][i] = gra::math::abs2(
			    (sol[0] * cc[k][0] + sol[1] * cc[k][1] +
			     sol[2] * cc[k][2] + sol[3] * cc[k][3]) / 4.0); 
			f_2[k][i] *= bt; // Jacobian
		}
...
void MEikonal::S3InitCutPomerons() {
	...
	// Numerical integral loop over impact parameter (b_t) space
	const double STEP = (MEikonalNumerics::MaxBT - MEikonalNumerics::MinBT) / MEikonalNumerics::NumberBT;
	P_array = std::vector<std::vector<double>>(MCUT, std::vector<double>(MEikonalNumerics::NumberBT+1, 0.0));
	...
	for (std::size_t m = 0; m < MCUT; ++m) {
		P_cut[m] = gra::math::CSIntegral(P_array[m], STEP) / (MEikonalNumerics::MaxBT - MEikonalNumerics::MinBT);
