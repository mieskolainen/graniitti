...
template <class T> void MH1<T>::Print(double width) const {
  ...
  for (std::size_t idx = 0; idx < static_cast<unsigned int>(XBINS); ++idx) {
    ...
    if (Nc > 0) { // non-zero bin
      ...
      const std::size_t D =
          std::floor(w * (N - 1) * (1 - relative_err)); // down
      // Print now
...
template <class T> double MH1<T>::GetMean() const {
  double sum = 0.0;
...
template <class T> double MH1<T>::GetSquareMean() const {
  double sum = 0.0;
  double norm = 0.0; // Normalization
  const double binwidth = (XMAX - XMIN) / XBINS;
...
template <class T> T MH1<T>::SumWeights() const {
  T sum = 0.0;
  for (std::size_t i = 0; i < static_cast<unsigned int>(XBINS); ++i) {
    sum += weights[i];
  }
...
template <class T> T MH1<T>::SumWeights2() const {
  T sum = 0.0;
  for (std::size_t i = 0; i < static_cast<unsigned int>(XBINS); ++i) {
    sum += weights2[i];
  }
...
// Overflow  returns -2
template <class T>
int MH1<T>::GetIdx(double value, double minval, double maxval, int nbins,
                   bool logbins) const {
  if (value < minval) {
    return -1;
  } // underflow
