...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    options.add_options("")("i,INPUT", "Input card",
    ...
    options.add_options("GENERALPARAM")
        ...
        ("o,OUTPUT", "Output name string", cxxopts::value<std::string>())(
            ...
            "w,WEIGHTED", "Weighted events (true/false)",
            cxxopts::value<std::string>())("c,CORES",
                                           "Number of CPU cores/threads",
                                           cxxopts::value<unsigned int>());

    options.add_options("PROCESSPARAM")("p,PROCESS", "Process",
    ...
    if (r.count("help") || NARGC == 0) {
      ...
      std::cout << options.help({"", "GENERALPARAM", "PROCESSPARAM"})
                << std::endl;
      ...
      std::cout << "  @OFFSHELL:X               how many +- full widths "
                ...
                << std::endl;
      std::cout
          << "  @FLATMASS2:true           flat sampling in mass^2 instead "
             "of relativistic Breit-Wigner f(M^2) in decay trees"
          << std::endl;
      std::cout
          << "  @FRAME:X                  set common Lorentz frame for "
      ...
      std::cout << "  PP[CON]<C> -> rho(770)0 > {pi+ pi-} rho(770)0 > {pi+ pi-}"
                << std::endl;
      std::cout << "  yy[CON]<F> -> pi+ pi- pi+ pi-" << std::endl;
      std::cout << "  yy[CON]<C> -> p+ p- @FLATAMP:2" << std::endl;
      std::cout << "  yy[CON]<C> -> 992 -992 @PDG[992]{M:1500,W:0}"
                << std::endl;
      std::cout << std::endl;
      ...
      std::cout << rang::style::bold
                ...
                << rang::style::reset << std::endl;
      std::cout << "  " << argv[0] << " -i ./input/test.json" << std::endl
                << std::endl;

      std::cout << rang::style::bold
                << " A steering card example + commandline input override:"
                << rang::style::reset << std::endl;
      std::cout << "  " << argv[0]
                << " -i ./input/test.json -p \"yy[C]<F> -> W+ W-\"" << std::endl
                << std::endl;
