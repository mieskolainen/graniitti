	...
	try {
		...
		options.add_options("GENERALPARAM")
		    ...
		    ("o,OUTPUT", "Output name string", cxxopts::value<std::string>())(
		        ...
		        cxxopts::value<std::string>())("c,CORES", "Number of CPU cores/threads",
		...
		if (r.count("help") || NARGC == 0) {
			...
			std::cout << options.help({"", "GENERALPARAM", "PROCESSPARAM"})
			          << std::endl;
			std::cout << rang::style::bold << " Arrow operators:" << rang::style::reset
			          << std::endl;
			...
			std::cout << rang::style::bold
			          << " PROCESS string examples:" << rang::style::reset << std::endl;
