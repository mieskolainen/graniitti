...
void experiment(bool screening) {
  ...
  /*
    input.push_back(MEASUREMENT("./tests/processes/el.json",            1.6e-12,
    0.5e-12,
    0.3e-12));

    // <https://cds.cern.ch/record/1495761/files/CERN-PH-EP-2012-353.pdf>
    input.push_back(MEASUREMENT("./tests/processes/TOTEM12_.json",      1.6e-12,
    0.5e-12,
    0.3e-12));

    // <https://arxiv.org/abs/1712.06153>
    input.push_back(MEASUREMENT("./tests/processes/TOTEM17_total.json", 1.6e-12,
    0.5e-12,
    0.3e-12));
  */
  ...
  input.push_back(
      MEASUREMENT("ATLAS15_ee.json", 0.428e-12, 0.035e-12, 0.018e-12));

  // <https://arxiv.org/abs/1506.07098>
  input.push_back(
      MEASUREMENT("ATLAS15_mumu.json", 0.628e-12, 0.032e-12, 0.021e-12));

  // <https://arxiv.org/abs/1708.04053>
  input.push_back(
      MEASUREMENT("ATLAS17_mumu.json", 3.12e-12, 0.07e-12, 0.14e-12));

  // <https://discoverycenter.nbi.ku.dk/teaching/thesis_page/MasterEmilBolsFinal.pdf>
  input.push_back(
      MEASUREMENT("ATLAS17_2pi.json", 18.75e-6, 0.048e-6, 0.770e-6));
  ...
  // input.push_back(MEASUREMENT("./tests/experiment/ALICE_2pi_PWA.json", 31e-6,
  // 0.5e-6,
  // 2e-6));

  // <https://arxiv.org/pdf/1806.04079.pdf>
  // input.push_back(MEASUREMENT("./tests/experiment/LHCb18_jpsi.json", 399e-12,
  // 16e-12,
  // 19e-12));
  ...
  */
  ...
  std::vector<std::vector<double>> xs0(input.size(),
  ...
  for (const auto &i : indices(input)) {
    printf("[%22s] DATA = %0.2E +- %0.2E (stat) +- %0.2E (syst) | MC BARE = "
           ...
           "%0.3f \n",
           input[i].card.c_str(), input[i].value, input[i].stat, input[i].syst,
           xs0[i][0], xs0[i][0] / input[i].value, xs0[i][1],
