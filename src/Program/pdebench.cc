...
//
void CNAB2(std::valarray<std::complex<double>> &u, int Lx, double dt, int Nt,
           int nplot, bool USE_EIGEN) {
  ...
  for (int n = 0; n < Nt; ++n) {
    ...
    if ((n % nplot) == 0) {
      ...
      for (unsigned int i = 0; i < temp2.size(); ++i) {
        maxval = gra::math::abs2(temp2[i]) > maxval ? gra::math::abs2(temp2[i])
                                                    : maxval;
      }
      std::cout << n << " ";
      for (unsigned int i = 0; i < temp2.size(); ++i) {
...
void testfft() {
  // FFT test
  std::valarray<float> xxx = gra::math::linspace<std::valarray>(
      (float)0.0, (float)10.0, (unsigned int)16);
...
int main(int argc, char *argv[]) {
  if (argc != 7) {
    std::cout << "Kuramoto-Sivashinsky 4th order PDE FFT-testbench"
              << std::endl;
