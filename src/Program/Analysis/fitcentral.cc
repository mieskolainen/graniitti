...
void SetSoftParam(std::map<std::string, gra::PARAM_RES> &RESONANCES,
                  double *par);
...
void SetSoftParam(std::map<std::string, gra::PARAM_RES> &RESONANCES,
  ...
  for (const auto &x : RESONANCES) {
    ...
    printf("[%s] \t A = %0.8f \t phi = %0.6f (%0.0f deg) ", x.first.c_str(), A,
           phi, phi / (2 * gra::math::PI) * 360);

    bool afix =
        std::find(afixlist.begin(), afixlist.end(), x.first) != afixlist.end();
    bool pfix =
        std::find(pfixlist.begin(), pfixlist.end(), x.first) != pfixlist.end();

    if (afix && pfix) {
      std::cout << rang::fg::red << "\t[AMPLITUDE & PHASE FIXED]"
                << rang::fg::reset << std::endl;
    } else if (afix) {
      std::cout << rang::fg::green << "\t[AMPLITUDE FIXED]" << rang::fg::reset
                << std::endl;
    } else if (pfix) {
      std::cout << rang::fg::blue << "\t[PHASE FIXED]" << rang::fg::reset
                << std::endl;
    } else if (std::find(zerolist.begin(), zerolist.end(), x.first) !=
               zerolist.end()) {
      std::cout << rang::fg::yellow << "\t[SET TO ZERO]" << rang::fg::reset
                << std::endl;
  ...
  bool fixcontinuum = (std::find(afixlist.begin(), afixlist.end(),
                                 "continuum") != afixlist.end()) ||
                      (std::find(pfixlist.begin(), pfixlist.end(),
                                 "continuum") != pfixlist.end());
...
// Cost function
void Chi2Func(int &npar, double *gin, double &f, double *par, int iflag) {
  std::cout << rang::fg::red << "fitcentral::Chi2Func: iter = " << iter
            << rang::fg::reset << std::endl;
  ...
  for (const auto &k : indices(datasets)) {
    ...
    try {
      ...
      gen->proc->h1["M"].ResetBounds(datasets[k].NBINS, datasets[k].MMIN,
                                     datasets[k].MMAX);
      ...
      double S =
          datasets[k].h1DATA->SumWeights() / gen->proc->h1["M"].SumWeights();
...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    options.add_options()(
        "i,input",
        "Input dataset collection numbers               <0,1,2,3,...>",
        cxxopts::value<std::string>())(
        "c,continuum", "Continuum form factor                          <0|1|2>",
        cxxopts::value<int>())(
        "a,afix",
        "Fix resonance amplitude to their input values  <f0_980,f2_1270,...>",
        cxxopts::value<std::string>())(
        "p,pfix",
        "Fix resonance phase to their input values      <f0_980,f2_1270,...>",
        cxxopts::value<std::string>())(
        "x,fix",
        "Fix resonance amplitude and phase              <f0_980,f2_1270,...>",
        cxxopts::value<std::string>())(
        "z,zero",
        "Fix resonances to zero                         <f2_1525,...>",
        cxxopts::value<std::string>())(
        "Y,allfixed",
        "Fix all resonances to their input values       <true|false>",
    ...
    if (r.count("help") || NARGC == 0) {
      ...
      std::cout << "  " << argv[0] << " -i 0,1 -c 2 -x 'rho_770,continuum' "
                << std::endl
                << std::endl;
      return EXIT_FAILURE;
    }
    ...
    std::vector<int> input =
        gra::aux::SplitStr2Int(r["input"].as<std::string>());
    ...
    if (r.count("x")) {
      std::vector<std::string> out1 =
          gra::aux::SplitStr2Str(r["x"].as<std::string>());
      fitcentral::afixlist.insert(fitcentral::afixlist.end(), out1.begin(),
                                  out1.end());

      std::vector<std::string> out2 =
          gra::aux::SplitStr2Str(r["x"].as<std::string>());
      fitcentral::pfixlist.insert(fitcentral::pfixlist.end(), out2.begin(),
                                  out2.end());
    }
    // Zero
    if (r.count("z")) {
    ...
    if (std::find(input.begin(), input.end(), 0) != input.end()) {
      ...
      temp.DATAFILE =
          BASEPATH + "/HEPdata/CD/" + "pipi_7_TeV_eta_09_pt_150.csv";
      temp.DATATYPE = 4;
      ...
      temp.MMAX = 3.0;
      temp.NBINS = 100;
      temp.h1DATA = new MH1<double>(temp.NBINS, temp.MMIN, temp.MMAX,
                                    "ALICE 7 TeV pi+pi-");
    ...
    if (std::find(input.begin(), input.end(), 1) != input.end()) {
      ...
      temp.NBINS = 50;
      temp.h1DATA =
          new MH1<double>(temp.NBINS, temp.MMIN, temp.MMAX, "ALICE 7 TeV K+K-");
    ...
    if (fitcentral::datasets.size() == 0) {
      throw std::invalid_argument(
          "fitcentral::datasets is empty (did not find any)");
    }
    ...
    for (const auto &x : RESONANCES) {
      bool setzero = false;
      if (std::find(fitcentral::zerolist.begin(), fitcentral::zerolist.end(),
                    x.first) != fitcentral::zerolist.end()) {
        setzero = true;
      }
      gMinuit->mnparm(++k, Form("RESONANCES[%s].g_A", x.first.c_str()),
                      !setzero ? std::abs(x.second.g) : 0.0, STEPSIZE_A, 1e-9,
                      20.0, ierflg); // couplings [1e-9,...,2.0]
      gMinuit->mnparm(++k, Form("RESONANCES[%s].g_phi", x.first.c_str()),
                      !setzero ? std::arg(x.second.g) : 0.0, STEPSIZE_phi,
                      -2 * gra::math::PI, 2 * gra::math::PI, ierflg);

      // Amplitude and Phase
      bool afix =
          std::find(fitcentral::afixlist.begin(), fitcentral::afixlist.end(),
                    x.first) != fitcentral::afixlist.end();
      bool pfix =
          std::find(fitcentral::pfixlist.begin(), fitcentral::pfixlist.end(),
                    x.first) != fitcentral::pfixlist.end();
    ...
    if (fitcentral::offshellFF == 0) {
      gMinuit->mnparm(++k, "PARAM_REGGE::a_EXP", PARAM_REGGE::b_EXP, STEPSIZE_A,
                      0.0, 2.0, ierflg);
      ...
      gMinuit->mnparm(++k, "PARAM_REGGE::b_POW", PARAM_REGGE::b_POW, STEPSIZE_A,
                      0.0, 2.0, ierflg);
    } else {
      throw std::invalid_argument(
          "fitcentral::offshellFF should be 0,1,2 (input is " +
    ...
    bool afix =
        std::find(fitcentral::afixlist.begin(), fitcentral::afixlist.end(),
                  "continuum") != fitcentral::afixlist.end();
    bool pfix =
        std::find(fitcentral::pfixlist.begin(), fitcentral::pfixlist.end(),
                  "continuum") != fitcentral::pfixlist.end();
