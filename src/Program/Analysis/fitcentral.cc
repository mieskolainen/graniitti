		...
		try {
			...
			for (std::size_t i = 0; i < datasets[k].h1DATA->XBins(); ++i) {
				if (datasets[k].h1DATA->GetBinWeight(i) > 0) {
					// Chi^2
					local_chi2 +=
					    gra::math::pow2(S * gen->proc->h1["M"].GetBinWeight(i) -
...
int main(int argc, char *argv[]) {
	...
	try {
		...
		options.add_options()(
		    "i,input", "Input dataset collection numbers               <0,1,2,3,...>",
		...
		if (std::find(input.begin(), input.end(), 0) != input.end()) {
			...
			temp.NBINS = 100;
			temp.h1DATA =
			    new MH1<double>(temp.NBINS, temp.MMIN, temp.MMAX, "ALICE 7 TeV pi+pi-");
		...
		// Do we fix it
		bool afix = std::find(fitcentral::afixlist.begin(), fitcentral::afixlist.end(),
		                      "continuum") != fitcentral::afixlist.end();
		bool pfix = std::find(fitcentral::pfixlist.begin(), fitcentral::pfixlist.end(),
