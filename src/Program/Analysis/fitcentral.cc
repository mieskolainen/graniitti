...
void SetSoftParam(std::map<std::string, gra::PARAM_RES>& RESONANCES, double* par) {
	...
	for(const auto& x : RESONANCES) {
		...
		if(afix && pfix) {
			std::cout << rang::fg::red << "\t[AMPLITUDE & PHASE FIXED]" << rang::fg::reset
					  << std::endl;
		} else if(afix) {
...
int main(int argc, char* argv[]) {
	...
	try {
		...
		if(fitcentral::offshellFF == 0) {
			gMinuit->mnparm(++k, "PARAM_REGGE::a_EXP", PARAM_REGGE::b_EXP, STEPSIZE_A, 0.0, 2.0,
							ierflg);
			...
			gMinuit->mnparm(++k, "PARAM_REGGE::b_POW", PARAM_REGGE::b_POW, STEPSIZE_A, 0.0, 2.0,
							ierflg);
		} else {
