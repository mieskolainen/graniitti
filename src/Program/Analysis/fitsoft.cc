...
const std::string INPUTFILE =
    gra::aux::GetBasePath(2) + "/fitcard/" + "EL.json";
...
std::vector<double> dsigma_el_dt(double *par, double sqrts,
                                 const std::vector<string> &beam,
                                 std::vector<double> &x);
...
bool ReadData(std::string inputfile, std::vector<double> &x,
  ...
  while (true) {
    int ret = fscanf(fp, "%lf %lf %lf %lf %lf %lf %lf %lf", &xval, &xval_low,
                     &xval_high, &yval, &yerr_stat_p, &yerr_stat_m,
                     &yerr_syst_p, &yerr_syst_m);
    if (ret == 8) {
      // Fine
    } else if (ret == EOF) {
      break;
    ...
    // Construct error
    double error =
        std::sqrt(gra::math::pow2((yerr_stat_p - yerr_stat_m) / 2.0) +
...
bool InitData() {
  ...
  if (!fitsoft::ReadData(BASEPATH + "TOTEM_7_low_t.csv", DIFFMEAS::x[N],
                         DIFFMEAS::y[N], DIFFMEAS::err[N])) {
    return false;
  }
  if (!fitsoft::ReadData(BASEPATH + "TOTEM_7.csv", DIFFMEAS::x[N],
                         DIFFMEAS::y[N], DIFFMEAS::err[N])) {
    return false;
  }
  ++N;
  ...
  */
  ...
  DIFFMEAS::initialstate[N] = {"p+", "p-"};
  if (!fitsoft::ReadData(BASEPATH + "ABE_1994_1800.csv", DIFFMEAS::x[N],
                         DIFFMEAS::y[N], DIFFMEAS::err[N])) {
    return false;
  }
  ++N;
  ...
  DIFFMEAS::initialstate[N] = {"p+", "p-"};
  if (!fitsoft::ReadData(BASEPATH + "FNAL_546.csv", DIFFMEAS::x[N],
                         DIFFMEAS::y[N], DIFFMEAS::err[N])) {
    return false;
  }
  if (!fitsoft::ReadData(BASEPATH + "SPS_546.csv", DIFFMEAS::x[N],
                         DIFFMEAS::y[N], DIFFMEAS::err[N])) {
    return false;
  }
  ++N;
  /*
    ...
    DIFFMEAS::initialstate[N] = {"p+","p-"};
    if (!ReadData(BASEPATH + "ISR_62.csv", DIFFMEAS::x[N], DIFFMEAS::y[N],
    DIFFMEAS::err[N]) )
    return false;
    ++N;
  */
...
void Chi2Func(int &npar, double *gin, double &f, double *par, int iflag) {
  ...
  for (const auto &i : indices(DIFFMEAS::sqrts)) {
    ...
    for (const auto &j : indices(values)) {
      chi2 += gra::math::pow2((DIFFMEAS::y[i][j] - values[j]) /
                              DIFFMEAS::err[i][j]);
      ++points;
    }
  }
...
int main(int argc, char *argv[]) {
  ...
  std::cout << rang::style::bold << "GRANIITTI - Eikonal Pomeron Fitter"
  ...
  gMinuit->mnparm(3, "g3P", PARAM_SOFT::g3P / PARAM_SOFT::gN_P, 0.1, 1e-9, 1.0,
                  ierflg); // Convention
  gMinuit->mnparm(4, "gamma", PARAM_SOFT::gamma, 0.01, 1e-9, 1.0 - 1e-9,
                  ierflg);
