  ...
  try {
    ...
    for (const auto &i : indices(values)) {
      ...
      values[i] = gra::math::abs2(A) /
                  (16.0 * gra::math::PI * s * (s - 4.0 * gra::math::pow2(PDG::mp))) * PDG::GeV2mb;
    }
...
void Chi2Func(int &npar, double *gin, double &f, double *par, int iflag) {
  ...
  for (const auto &i : indices(DIFFMEAS::sqrts)) {
    ...
    std::vector<double> values =
        fitsoft::dsigma_el_dt(par, DIFFMEAS::sqrts[i], DIFFMEAS::initialstate[i], DIFFMEAS::x[i]);
...
int main(int argc, char *argv[]) {
  ...
  // Pomeron trajectors
  gMinuit->mnparm(0, "DELTA_P", PARAM_SOFT::DELTA_P, 0.01, 1e-9, 0.5, ierflg);
