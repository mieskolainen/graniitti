...
void Init1DHistogram(std::map<std::string, std::unique_ptr<h1Multiplet>>& h,
	...
	name = "h1_S_M";
	h[name] = std::make_unique<h1Multiplet>(name, title + ";System M  (GeV);d#sigma/dM  (" + units +
													  "/GeV)",
											bM.N, bM.min, bM.max, legendtext);

	name = "h1_S_Pt";
	h[name] = std::make_unique<h1Multiplet>(name, title + ";System P_{T} (GeV);d#sigma/dP_{T}  (" +
													  units + "/GeV)",
	...
	name = "h1_PP_dpt";
	h[name] = std::make_unique<h1Multiplet>(name, title + ";Proton pair |#delta#bar{p}_{T}| "
...
void Init2DHistogram(std::map<std::string, std::unique_ptr<h2Multiplet>>& h,
	...
	name = "h2_S_M_sqrt1";
	h[name] = std::make_unique<h2Multiplet>(name, "d#sigma^2/dMd#sqrt{-t_{1}}  (" + units +
													  "/GeV/GeV) | " + title +
	...
	if(std::find(multiplicity.begin(), multiplicity.end(), 2) != multiplicity.end()) {
		...
		h[name] =
			std::make_unique<h2Multiplet>(name, "d#sigma^2/d#eta_{1}d#eta_{2}  (" + units + ") | " +
													title + ";#eta_{1}; #eta_{2}",
...
int main(int argc, char* argv[]) {
	...
	try {
		...
		options.add_options()("i,input",
			...
			cxxopts::value<std::string>())("M,mass", "plot mass limit", cxxopts::value<double>())(
		...
		for(const auto& SID : indices(inputfile)) {
			...
			cross_section[SID] = analysis[SID]->HepMC3_OracleFill(
				inputfile[SID], (uint)multiplicity[SID], finalstatePDG[SID], (uint)MAXEVENTS, h1,
				h2, hP, SID);
		}
		// Name
