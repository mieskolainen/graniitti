...
void Init1DHistogram(std::map<std::string, std::unique_ptr<h1Multiplet>> &h,
	...
	name = "h1_S_Pt";
	h[name] = std::make_unique<h1Multiplet>(
	    name, title + ";System P_{T} (GeV);d#sigma/dP_{T}  (" + units + "/GeV)", bP.N, bP.min,
	    bP.max, legendtext);

	name = "h1_S_Y";
	h[name] =
	    std::make_unique<h1Multiplet>(name, title + ";System Y;d#sigma/dY  (" + units + ")",
	                                  bY.N, bY.min, bY.max, legendtext);

	// Central track observables
	name = "h1_1B_pt";
	h[name] = std::make_unique<h1Multiplet>(
	    name, title + ";Central final state p_{T} (GeV);d#sigma/dp_{T}  (" + units + "/GeV)",
	...
	if (std::find(multiplicity.begin(), multiplicity.end(), 2) != multiplicity.end()) {
		...
		name = "h1_2B_diffrap";
		h[name] = std::make_unique<h1Multiplet>(
		    name, title + ";#deltay #equiv y_{1} - y_{2};d#sigma/d#deltay  (" + units + ")",
		    bY.N, bY.min, bY.max, legendtext);
	}
	...
	name = "h1_PP_dphi";
	h[name] = std::make_unique<h1Multiplet>(
	    name, title + ";Proton pair #delta#phi (rad);d#sigma/#delta#phi  (" + units + "/rad)",
...
void Init2DHistogram(std::map<std::string, std::unique_ptr<h2Multiplet>> &h,
	...
	name = "h2_S_M_Pt";
	h[name] =
	    std::make_unique<h2Multiplet>(name, "d#sigma^2/dMdP_{T}  (" + units + "/GeV/GeV) | " +
...
void InitPrHistogram(std::map<std::string, std::unique_ptr<hProfMultiplet>> &h,
	...
	h[name] = std::make_unique<hProfMultiplet>(
	    name, title + ";System M  (GeV); System #LTP_{T}#GT (GeV)", bM.N, bM.min, bM.max,
	    bP.min, bP.max, legendtext);

	name = "hP_S_M_PL2";
	h[name] = std::make_unique<hProfMultiplet>(
	    name, title + ";System M  (GeV); Legendre #LTP_{l=2}(cos #theta)#GT | r.f.", bM.N,
	    bM.min, bM.max, -0.4, 0.4, legendtext);

	name = "hP_S_M_PL4";
	h[name] = std::make_unique<hProfMultiplet>(
	    name, title + ";System M  (GeV); Legendre #LTP_{l=4}(cos #theta)#GT | r.f.", bM.N,
	    bM.min, bM.max, -0.4, 0.4, legendtext);
	...
	if (std::find(multiplicity.begin(), multiplicity.end(), 2) != multiplicity.end()) {
		...
		h[name] = std::make_unique<hProfMultiplet>(
		    name,
		    title + ";System M (GeV); Central final state pair #LT#delta#phi#GT  (rad)",
...
int main(int argc, char *argv[]) {
	...
	std::cout << rang::style::bold << "GRANIITTI - Fast Analyzer" << rang::style::reset
	          << std::endl
	          << std::endl;
	...
	try {
		...
		options.add_options()(
		    ...
		    cxxopts::value<std::string>())("l,labels",
		                                   "plot legend string       <input1,input2,...>",
		                                   cxxopts::value<std::string>())(
		    "t,title", "plot title string        <input>            ",
		    cxxopts::value<std::string>())("u,units",
		                                   "cross section unit       <barn|mb|ub|nb|pb|fb>",
		                                   cxxopts::value<std::string>())(
		...
		const std::string cmd = "gs -dBATCH -dNOPAUSE -q -sDEVICE=pdfwrite -sOutputFile=" +
		                        fullpath + "/merged.pdf " + fullpath + "/h*.pdf";
