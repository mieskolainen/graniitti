...
void Init1DHistogram(std::map<std::string, std::unique_ptr<h1Multiplet>> &h,
                     ...
                     std::vector<int> multiplicity, const std::string &title,
                     const std::string &units, const h1Bound &bM,
  ...
  h[name] = std::make_unique<h1Multiplet>(
      name, title + ";System M  (GeV);d#sigma/dM  (" + units + "/GeV)", bM.N,
      bM.min, bM.max, legendtext);

  name = "h1_S_Pt";
  h[name] = std::make_unique<h1Multiplet>(
      name, title + ";System P_{T} (GeV);d#sigma/dP_{T}  (" + units + "/GeV)",
      bP.N, bP.min, bP.max, legendtext);

  name = "h1_S_Y";
  h[name] = std::make_unique<h1Multiplet>(
      name, title + ";System Y;d#sigma/dY  (" + units + ")", bY.N, bY.min,
      bY.max, legendtext);

  // Central track observables
  name = "h1_1B_pt";
  h[name] = std::make_unique<h1Multiplet>(
      name, title + ";Central final state p_{T} (GeV);d#sigma/dp_{T}  (" +
                units + "/GeV)",
      bP.N, bP.min, bP.max, legendtext);

  name = "h1_1B_eta";
  h[name] = std::make_unique<h1Multiplet>(
      name, title + ";Central final state #eta;d#sigma/d#eta  (" + units + ")",
      bY.N, bY.min, bY.max, legendtext);

  // 2-Body observables
  if (std::find(multiplicity.begin(), multiplicity.end(), 2) !=
      multiplicity.end()) {
    name = "h1_2B_acop";
    h[name] = std::make_unique<h1Multiplet>(
        name, title + ";Central final state acoplanarity #rho = 1 - "
                      "|#delta#phi|/#pi;d#sigma/d#rho  (" +
                  units + "/rad)",
        100, 0.0, 1.0, legendtext);

    name = "h1_2B_diffrap";
    h[name] = std::make_unique<h1Multiplet>(
        name, title + ";#deltay #equiv y_{1} - y_{2};d#sigma/d#deltay  (" +
                  units + ")",
        bY.N, bY.min, bY.max, legendtext);
  }

  // 4-Body observables
  if (std::find(multiplicity.begin(), multiplicity.end(), 4) !=
      multiplicity.end()) {
    // ...
  }
  ...
  name = "h1_PP_dphi";
  h[name] = std::make_unique<h1Multiplet>(
      name, title + ";Proton pair #delta#phi (rad);d#sigma/#delta#phi  (" +
                units + "/rad)",
      100, 0.0, 3.14159, legendtext);

  name = "h1_PP_t1";
  h[name] = std::make_unique<h1Multiplet>(
      name, title + ";Mandelstam -t_{1} (GeV^{2});d#sigma/dt  (" + units +
                "/GeV^{2})",
      bP.N, bP.min, bP.max, legendtext);

  name = "h1_PP_dpt";
  h[name] = std::make_unique<h1Multiplet>(
      name, title + ";Proton pair |#delta#bar{p}_{T}| "
...
void Init2DHistogram(std::map<std::string, std::unique_ptr<h2Multiplet>> &h,
  ...
  h[name] = std::make_unique<h2Multiplet>(
      name, "d#sigma^2/dMdP_{T}  (" + units + "/GeV/GeV) | " + title +
                ";System M (GeV); System P_{T} (GeV)",
      bM.N, bM.min, bM.max, bP.N, bP.min, bP.max, legendtext);

  name = "h2_S_M_sqrt1";
  h[name] = std::make_unique<h2Multiplet>(
      name, "d#sigma^2/dMd#sqrt{-t_{1}}  (" + units + "/GeV/GeV) | " + title +
  ...
  h[name] = std::make_unique<h2Multiplet>(
      name,
      "d#sigma^2/dMd|#delta#bar{p}_{T}|  (" + units + "/GeV/GeV) | " + title +
  ...
  if (std::find(multiplicity.begin(), multiplicity.end(), 2) !=
    ...
    h[name] = std::make_unique<h2Multiplet>(
        name, "d#sigma^2/d#eta_{1}d#eta_{2}  (" + units + ") | " + title +
                  ";#eta_{1}; #eta_{2}",
...
void InitPrHistogram(std::map<std::string, std::unique_ptr<hProfMultiplet>> &h,
  ...
  h[name] = std::make_unique<hProfMultiplet>(
      name, title + ";System M  (GeV); System #LTP_{T}#GT (GeV)", bM.N, bM.min,
      bM.max, bP.min, bP.max, legendtext);

  name = "hP_S_M_PL2";
  h[name] = std::make_unique<hProfMultiplet>(
      name,
      title + ";System M  (GeV); Legendre #LTP_{l=2}(cos #theta)#GT | r.f.",
      bM.N, bM.min, bM.max, -0.4, 0.4, legendtext);

  name = "hP_S_M_PL4";
  h[name] = std::make_unique<hProfMultiplet>(
      name,
      title + ";System M  (GeV); Legendre #LTP_{l=4}(cos #theta)#GT | r.f.",
      bM.N, bM.min, bM.max, -0.4, 0.4, legendtext);
  ...
  if (std::find(multiplicity.begin(), multiplicity.end(), 2) !=
    ...
    h[name] = std::make_unique<hProfMultiplet>(
        name,
        title +
            ";System M (GeV); Central final state pair #LT#delta#phi#GT  (rad)",
        bM.N, bM.min, bM.max, 0.0, 3.14159, legendtext);
  }
...
int main(int argc, char *argv[]) {
  ...
  std::cout << rang::style::bold << "GRANIITTI - Fast Analyzer"
  ...
  try {
    ...
    options.add_options()(
        ...
        cxxopts::value<std::string>())(
        "u,units", "cross section unit       <barn|mb|ub|nb|pb|fb>",
        cxxopts::value<std::string>())("M,mass", "plot mass limit",
                                       cxxopts::value<double>())(
        "Y,rapidity", "plot rapidity limit", cxxopts::value<double>())(
        "P,momentum", "plot momentum limit", cxxopts::value<double>())(
        "X,maximum", "maximum number of events",
        cxxopts::value<int>())("S,scale", "scale histograms",
    ...
    std::vector<std::string> labels =
        gra::aux::SplitStr2Str(r["labels"].as<std::string>());
    std::vector<int> finalstatePDG =
        gra::aux::SplitStr2Int(r["pdg"].as<std::string>());
    std::vector<int> multiplicity =
        gra::aux::SplitStr2Int(r["number"].as<std::string>());
    ...
    if (inputfile.size() != finalstatePDG.size() ||
        inputfile.size() != multiplicity.size()) {
    ...
    for (const auto &SID : indices(inputfile)) {
      ...
      cross_section[SID] = analysis[SID]->HepMC3_OracleFill(
          inputfile[SID], (uint)multiplicity[SID], finalstatePDG[SID],
          (uint)MAXEVENTS, h1, h2, hP, SID);
    }
    // Name
