...
int main(int argc, char *argv[]) {
  ...
  try {
    ...
    options.add_options()(
        ...
        "i,input", "Input sample                              "
                   "<filename1,filename2,...> without .hepmc3",
        cxxopts::value<std::string>())(
        "t,titles",
        "Phase space titles (3 of them)            <titleA,titleB,titleC>",
        cxxopts::value<std::string>())(
        "l,legend",
        "Legend text                               <title1,title2,...>",
        cxxopts::value<std::string>())(
        "d,mode",
        "Input mode                                <MC|DATA,...>      ",
        cxxopts::value<std::string>())(
        ...
        cxxopts::value<std::string>())(
        "S,scale",
        "Scale plots (set -1 for unit normalized)  <scale1,scale2,...>",
        cxxopts::value<std::string>())

        ("f,frame",
         "Lorentz rest frame                        <HE|CS|GJ|PG|SR>",
         cxxopts::value<std::string>())(
            "g,lmax", "Maximum angular order                     <1|2|3|4|...>",
            cxxopts::value<unsigned int>())(
            "o,removeodd",
            "Remove negative M                         <true|false>",
            cxxopts::value<std::string>())(
            "v,removenegative",
            ...
            ("M,mass",
             "System mass binning                       <bins,min,max>",
             cxxopts::value<std::string>())(
                "P,pt",
                "System pt binning                         <bins,min,max>",
                cxxopts::value<std::string>())(
                "Y,rapidity",
    ...
    hparam.REMOVEODD =
        r["removeodd"].as<std::string>() == "true" ? true : false;
    ...
    if (hparam.L1REG < 0) {
      throw std::invalid_argument(
          "fitharmonic: Parameter 'l1reg' should be > 0");
    }
    ...
    ReadIn(ref, REFMC, FRAME, MAXEVENTS,
           true); // Always simulate reference here
...
void ReadIn(const std::string inputfile,
  ...
  if (input_file.failed()) {
    throw std::invalid_argument("MHarmonic::ReadIn: Failed to open <" +
                                total_input + ">");
  }
  // Event loop
  int events_read = 0;
  ...
  while (!input_file.failed()) {
    ...
    std::vector<HepMC3::GenParticlePtr> find_pip = HepMC3::applyFilter(
        HepMC3::Selector::PDG_ID == PDG::PDG_pip, evt.particles());

    std::vector<HepMC3::GenParticlePtr> find_pim = HepMC3::applyFilter(
