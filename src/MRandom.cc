...
double MRandom::CauchyRandom(double m0, double Gamma, double LIMIT) {
  ...
  while (true) {
    const double R = U(0, 1);
    const double value =
        (Gamma * 0.5) * std::tan(gra::math::PI * (R - 0.5)) + m0;

    if (value > 0 && value < (m0 + LIMIT * Gamma) &&
        value > (m0 - LIMIT * Gamma)) {
      mval = value;
      break;
    }
  }
...
void MRandom::DirRandom(const std::vector<double> &alpha,
                        std::vector<double> &y) {
...
double MRandom::NBDpdf(int n, double avgN, double k) {
  ...
  return tgamma(n + k) / (tgamma(n + 1) * tgamma(k)) *
         std::pow(avgN / (k + avgN), n) * std::pow(k / (k + avgN), k);
}
