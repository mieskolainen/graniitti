...
public:
  ...
  SpinorStates(const M4Vec &p, std::string type) const;
  std::array<FTensor::Tensor1<std::complex<double>, 4>, 3>
  Spin1States(const M4Vec &p, std::string type) const;

  // Scalar, Pseudoscalar, Tensor coupling structures
  FTensor::Tensor4<std::complex<double>, 4, 4, 4, 4>
  iG_PPS_1(double g_PPS) const;
  FTensor::Tensor4<std::complex<double>, 4, 4, 4, 4>
  iG_PPS_2(const M4Vec &q1, const M4Vec &q2, double g_PPS) const;
  FTensor::Tensor4<std::complex<double>, 4, 4, 4, 4>
  iG_PPPS_1(const M4Vec &q1, const M4Vec &q2, double g_PPPS) const;
  FTensor::Tensor4<std::complex<double>, 4, 4, 4, 4>
  iG_PPPS_2(const M4Vec &q1, const M4Vec &q2, double g_PPPS) const;
  MTensor<std::complex<double>> iG_PPT_1(double g_PPT) const;
  MTensor<std::complex<double>> iG_PPT_23(const M4Vec &q1, const M4Vec &q2,
                                          int mode, double g_PPT) const;

  // Vertex functions
  FTensor::Tensor4<std::complex<double>, 4, 4, 4, 4>
  iG_PPS_total(const M4Vec &q1, const M4Vec &q2, double M0,
               const std::string &mode, const std::vector<double> &g_PPS) const;
  MTensor<std::complex<double>>
  iG_PPT_total(const M4Vec &q1, const M4Vec &q2, double M0,
               const std::vector<double> &g_PPT) const;

  FTensor::Tensor2<std::complex<double>, 4, 4> iG_PppHE(const M4Vec &prime,
                                                        const M4Vec p) const;

  FTensor::Tensor1<std::complex<double>, 4>
  iG_yee(const M4Vec &prime, const M4Vec &p,
         const std::vector<std::complex<double>> &ubar,
         const std::vector<std::complex<double>> &u) const;

  FTensor::Tensor1<std::complex<double>, 4>
  iG_ypp(const M4Vec &prime, const M4Vec &p,
         const std::vector<std::complex<double>> &ubar,
         const std::vector<std::complex<double>> &u) const;

  FTensor::Tensor2<std::complex<double>, 4, 4>
  iG_Ppp(const M4Vec &prime, const M4Vec &p,
         const std::vector<std::complex<double>> &ubar,
         const std::vector<std::complex<double>> &u) const;

  FTensor::Tensor4<std::complex<double>, 4, 4, 4, 4>
  iG_PppbarP(const M4Vec &prime, const std::vector<std::complex<double>> &ubar,
             const M4Vec &pt, const MMatrix<std::complex<double>> &iSF,
             const std::vector<std::complex<double>> &v, const M4Vec &p) const;

  FTensor::Tensor2<std::complex<double>, 4, 4> iG_Ppsps(const M4Vec &prime,
                                                        const M4Vec &p) const;
  FTensor::Tensor4<std::complex<double>, 4, 4, 4, 4>
  iG_Pvv(const M4Vec &prime, const M4Vec &p) const;

  FTensor::Tensor1<std::complex<double>, 4> iG_rhopipi(const M4Vec &k1,
                                                       const M4Vec &k2) const;
  FTensor::Tensor2<std::complex<double>, 4, 4> iG_f2pipi(const M4Vec &k1,
                                                         const M4Vec &k2) const;
  FTensor::Tensor4<std::complex<double>, 4, 4, 4, 4>
  iG_f2yy(const M4Vec &k1, const M4Vec &k2) const;
  FTensor::Tensor2<std::complex<double>, 4, 4> yV(std::string type) const;

  // Propagators
  FTensor::Tensor4<std::complex<double>, 4, 4, 4, 4> iD_P(double s,
                                                          double t) const;
  FTensor::Tensor4<std::complex<double>, 4, 4, 4, 4> iD_2R(double s,
                                                           double t) const;
  ...
  FTensor::Tensor2<std::complex<double>, 4, 4>
  iD_VMES(const M4Vec &p, double M0, double Gamma) const;
  FTensor::Tensor4<std::complex<double>, 4, 4, 4, 4>
  iD_TMES(const M4Vec &p, double M0, double Gamma) const;

  // Tensor functions
  FTensor::Tensor4<std::complex<double>, 4, 4, 4, 4>
  Gamma0(const M4Vec &k1, const M4Vec &k2) const;
  FTensor::Tensor4<std::complex<double>, 4, 4, 4, 4>
  Gamma2(const M4Vec &k1, const M4Vec &k2) const;
  void CalcRTensor();
