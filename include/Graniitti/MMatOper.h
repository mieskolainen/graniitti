...
template <typename T> inline std::vector<T> Unit(const std::vector<T> &x) {
  double norm = 0.0;
...
//       which gives c (1 x p)
template <typename T>
inline std::vector<T> VecMatMultiply(const std::vector<T> &A,
                                     const MMatrix<T> &B) {
...
// X is (n x 1)
// Y is (1 x n)
template <typename T> inline std::vector<T> VecDagger(const std::vector<T> &X) {
...
// Negate vector
template <typename T> inline std::vector<T> Negat(const std::vector<T> &a) {
...
inline void PrintMatrix(const MMatrix<T> &A, const std::string name = "") {
  ...
  for (std::size_t i = 0; i < A.size_row(); ++i) {
    for (std::size_t j = 0; j < A.size_col(); ++j) {
      ...
      printf("%6.3f+i%6.3f%s ", std::real(A[i][j]), std::imag(A[i][j]),
             delim.c_str());
    }
    std::cout << std::endl;
  }
...
template <typename T> inline void PrintMatrixSeparate(const MMatrix<T> &A) {
  // Print real part
