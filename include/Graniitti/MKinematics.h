...
namespace kinematics {
  ...
  // Case pzlab = 0 (LHC pp) and m3, m4 any
  ...
  // Case pzlab any and m3 = m4
  inline double SolvePz3_C(double m3, double pt3, double pt4, double pz5, double E5, double s, double pzlab) {

    const double  t2 = sqrt(s);
    const double  t3 = E5*E5;
    const double  t4 = pzlab*pzlab;
    const double  t5 = pz5*pz5;
    const double  t6 = pt3*pt3;
    const double  t7 = pt4*pt4;
    const double  t8 = m3*m3;
    const double  t9 = pz5*pzlab*2.0;
    ...
    const double  t33 = pz5*pzlab*s*4.0;
    const double  t34 = pz5*pzlab*t3*4.0;
    const double  t35 = t13+t15+t16+t17+t18+t19+t20+t21+t22+t23+t24+t25+t26+t27+t28+t29+t30+t31+t32+t33+t34-E5*t14*4.0-s*t4*2.0-s*t5*2.0-s*t6*2.0-s*t7*2.0-s*t8*4.0-t3*t4*2.0-t3*t5*2.0-t3*t6*2.0-t3*t7*2.0-t3*t8*4.0-t6*t7*2.0-E5*t2*t3*4.0-pz5*pzlab*t4*4.0-pz5*pzlab*t5*4.0-pz5*pzlab*t6*4.0-pz5*pzlab*t7*4.0-pz5*pzlab*t8*8.0-E5*pz5*pzlab*t2*8.0;
    const double  t36 = std::sqrt(t35);
    const double  t0 = pz5*(-1.0/2.0)+pzlab*(1.0/2.0)-E5*t11*t36*(1.0/2.0)-pz5*t6*t11*(1.0/2.0)+pz5*t7*t11*(1.0/2.0)+pzlab*t6*t11*(1.0/2.0)-pzlab*t7*t11*(1.0/2.0)+t2*t11*t36*(1.0/2.0);

    return t0;
  }


  // Case pzlab any and m3,m4 any
  inline double SolvePz3_D(double m3, double m4, double pt3, double pt4, double pz5, double E5, double s, double pzlab) {
  ...
  syms E3 E4 E5 pz3 pz4 pz5 m3 m4 pt3 pt4 pzlab s
  ...
  sol_A = simplify(subs(S, [m4 pzlab], [m3 0]), 'Steps', 25);
  ccode(sol_A, 'File', 'temp.c'); readwritetext('temp.c','solution_A.c', str);
  
  sol_B = simplify(subs(S, pzlab, 0), 'Steps', 25);
  ...
  sol_D = simplify(subs(S, pzlab, 0), 'Steps', 25);
  ...
  */
  ...
  //
  // pzlab denotes the (sum of) initial state momentum. For example, LHC pp has pzlab = 0
  //
  inline double SolvePz(double m3, double m4, double pt3, double pt4, double pz5, double E5, double s, double pzlab) {

    constexpr double EPS = 1e-9;

    if (std::abs(pzlab) < EPS) {     // symmetric beam momentum case
      ...
      if (std::abs(m3 - m4) < EPS) { // identical mass case (elastic-X-elastic)
        return SolvePz3_C(m3, pt3, pt4, pz5, E5, s, pzlab);
      } else {
        return SolvePz3_D(m3, m4, pt3, pt4, pz5, E5, s, pzlab);
      }
    }
  ...
  // -s/2(1 + cos(theta*))
  inline double mandelstam_u(const M4Vec& p2, const M4Vec& p4) {
    return -2.0*(p2*p4);
  }
  ...
  inline double dPhi1(double MAX, double m) {
    ...
    return pow2(MAX) / (12 * pow2(gra::math::PI)) *
           std::pow(1.0 - pow2(m) / pow2(MAX), 3.0 / 2.0);
  }
  ...
  inline double PSnMassless(double M2, int n) {
    ...
    const double PS =
        (1.0 / (2.0 * std::pow(4.0 * gra::math::PI, 2*n - 3))) *
  ...
  inline void Two2TwoLimit(double s, double E1, double E2, double E3, double E4,
    ...
    const double C = (E3 - E1) * (E4 - E2) +
                     (E1 + E4 - E2 - E3) * (E1 * E4 - E2 * E3) / s;
    tmin = -0.5 * (A + B);
    tmax = C / tmin;
  }
  ...
  public:
    ...
    MCW operator * (double scale) {
      return MCW(GetW()*scale, GetW2()*scale*scale, GetN());
    }
    ...
    double IntegralError() const {
      return gra::math::msqrt(IntegralError2());
    }
    double GetW() const {
      return W;
    }
  ...
  inline void LorentzBoost(const T& boost, double M0, T& p, int sign) {
    ...
    // Mother beta-vector
    const std::valarray<double> beta = {sign * boost.Px() / boost.E(),
                                        sign * boost.Py() / boost.E(),
  ...
  // M0 ----*----- m1
  //
  // For floating point / efficiency reasons, the mother mass needs to be provided outside
  //
  template <typename T1, typename T2>
  ...
  //
  // Return: phase space weight for a valid fragmentation and -1.0 for a kinematically impossible.
  // When unweight == true, This function returns also number of trials for proper MC error treatment (MCW)
  //
  template <typename T1, typename T2>
  ...
  inline MCW NBodySetup(const T1& mother, double M0, const std::vector<double>& m, std::vector<double>& M_eff,
    ...
    do {
      ...
      //std::sort(randvec.begin(), randvec.end(), [](const double a, const double b) { return a > b; }); // Descending
  ...
  // Flat N-body phase space by implementing a number of (N-2) recursive 2-body decays.
  // Input as [mother 4-vector, vector of daughter masses, vector of final 4-vectors]
  //
  ...
  // 
  // Return: weight for a valid fragmentation and -1.0 for a kinematically impossible
  //
  // When unweight == true, This function returns also number of trials for proper MC error treatment (MCW obj)
  //
  template <typename T1, typename T2>
  inline MCW NBodyPhaseSpace(const T1& mother, double M0, const std::vector<double>& m,
    ...
    for (std::size_t i = N - 1; i >= 1; --i) {
      ...
      p[i] = T1(pt * std::cos(phi), pt * std::sin(phi),
                pnorm[i] * costheta, msqrt(pow2(m[i]) + pow2(pnorm[i])));
  ...
  //
  // Collins-Soper: Quantization z-axis defined by the bi-sector vector between initial state
  // p1 and (-p2) (NEGATIVE) directions in the (resonance) system rest frame,
  // where p1 and p2 are the initial state proton 3-momentum.
  //
  // Anti-Helicity: Quantization z-axis defined by the bisector vector between initial state
  // p1 and (p2) (POSITIVE) directions in the (resonance) system rest frame,
  // where p1 and p2 are the initial state proton 3-momentum.
  //
  // Helicity: Quantization axis defined by the resonance system 3-momentum vector in
  // the colliding beams frame (lab frame).
  //
  // Pseudo-Gottfried-Jackson: Quantization axis defined by the initial state proton
  ...
  inline void LorentzFrame(std::vector<T>& pfout, const T& pb1boost, const T& pb2boost,
                    const std::vector<T>& pfboost, const std::string& frametype, int direction) {
    ...
    // @@ COLLINS-SOPER FRAME POLARIZATION AXIS DEFINITION @@
      ...
      zaxis = gra::matoper::Unit( gra::matoper::Minus(gra::matoper::Unit(pb1boost3), gra::matoper::Unit(pb2boost3)) );
    }
    // @@ ANTI-HELICITY FRAME POLARIZATION AXIS DEFINITION @@
    else if (frametype == "AH") {
      zaxis = gra::matoper::Unit( gra::matoper::Plus(gra::matoper::Unit(pb1boost3), gra::matoper::Unit(pb2boost3)) );
    }
    ...
    // @@ PSEUDO-GOTTFRIED-JACKSON AXIS DEFINITION: [1] or [2] @@
      ...
      if      (direction == -1) { zaxis = gra::matoper::Unit(pb1boost3); }
      else if (direction ==  1) { zaxis = gra::matoper::Unit(pb2boost3); }
      else { throw std::invalid_argument("gra::kinematics::LorentzFrame: Invalid direction <"
    ...
    // y-axis
    if (frametype != "SR") {
      yaxis = gra::matoper::Unit( gra::matoper::Cross( gra::matoper::Unit(pb2boost3), gra::matoper::Unit(pb1boost3)) );
    }
  ...
  inline void HEframe(std::vector<T>& p) {
    ...
    for (const auto& i : gra::aux::indices(p)) {
      gra::kinematics::LorentzBoost(XNEW, XNEW.M(), p[i],
  ...
  template <typename T>
  inline void PGframe(std::vector<T>& p, const int direction,
                     const T& p_beam_plus,
    ...
    if (direction == 1) {
      ...
      printf("PGframe:: Input direction %d not valid (1,-1)!",
             direction);
      return;
    }
    ...
    if (p.J != 0) {
      ...
      for (std::size_t i = 0; i < hc.rho.size_row(); ++i) {
        for (std::size_t j = 0; j < hc.rho.size_col(); ++j) {
          ...
          printf("%6.3f+i%6.3f%s ", std::real(hc.rho[i][j]), std::imag(hc.rho[i][j]), delim.c_str());
        }
        std::cout << std::endl;
      }
      std::cout << std::endl << std::endl;
