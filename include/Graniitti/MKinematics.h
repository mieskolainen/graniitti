...
inline double SolvePz3_A(double m3, double pt3, double pt4, double pz5, double E5, double s) {
	...
	const double t22 = t8 + t10 + t11 + t12 + t13 + t14 + t15 + t16 + t17 + t18 + t19 + t20 +
	                   t21 - E5 * t9 * 4.0 - s * t3 * 2.0 - s * t4 * 2.0 - s * t5 * 2.0 -
	                   s * t6 * 4.0 - t2 * t3 * 2.0 - t2 * t4 * 2.0 - t2 * t5 * 2.0 -
...
inline double SolvePz3_B(double m3, double m4, double pt3, double pt4, double pz5, double E5,
	...
	const double t29 = t9 + t11 + t12 + t13 + t14 + t15 + t16 + t17 + t18 + t19 + t20 + t21 +
...
inline MCW ThreeBodyPhaseSpace(const T1 &mother, double M0, const std::vector<double> &m,
	...
	double w_max =
	    DecayMomentum(M0, m[0], m12bound[0]) * DecayMomentum(m12bound[1], m[1], m[2]);
	if (unweight == false) {
		w_max = 0;
	}
...
inline void LorentzFrame(std::vector<T> &pfout, const T &pb1boost, const T &pb2boost,
	...
	// @@ COLLINS-SOPER FRAME POLARIZATION AXIS DEFINITION @@
		...
		zaxis = gra::matoper::Unit(gra::matoper::Minus(gra::matoper::Unit(pb1boost3),
		                                               gra::matoper::Unit(pb2boost3)));
	}
	// @@ ANTI-HELICITY FRAME POLARIZATION AXIS DEFINITION @@
	else if (frametype == "AH") {
		zaxis = gra::matoper::Unit(gra::matoper::Plus(gra::matoper::Unit(pb1boost3),
		                                              gra::matoper::Unit(pb2boost3)));
	}
	...
	// @@ PSEUDO-GOTTFRIED-JACKSON AXIS DEFINITION: [1] or [2] @@
		...
		throw std::invalid_argument("gra::kinematics::LorentzFrame: Unknown frame <" +
		                            frametype + ">");
	}

	// y-axis
	if (frametype != "SR") {
		yaxis = gra::matoper::Unit(gra::matoper::Cross(gra::matoper::Unit(pb2boost3),
		                                               gra::matoper::Unit(pb1boost3)));
	}
...
class PARAM_RES {
	...
	void PrintParam(double sqrts) const {
		std::cout << rang::style::bold
		          << "Custom resonance parameters:" << rang::style::reset << std::endl
		          << std::endl;
