...
inline double SqrtKallenLambda(double x, double y, double z) {
  // Hyperboloid formula
  return gra::math::msqrt(x * x + y * y + z * z -
                          2.0 * (x * y + x * z + y * z));
...
inline double DecayMomentum(double x, double y, double z) {
  ...
  // numerically stable
  return gra::math::msqrt((x - y - z) * (x + y + z) * (x + y - z) *
                          (x - y + z)) /
         (2.0 * x);
}
...
inline void Two2TwoLimit(double s, double E1, double E2, double E3, double E4,
  ...
  const double C =
      (E3 - E1) * (E4 - E2) + (E1 + E4 - E2 - E3) * (E1 * E4 - E2 * E3) / s;
  tmin = -0.5 * (A + B);
  tmax = C / tmin;
}
...
template <typename T> inline void Rotate(T &p, double theta, double phi) {
...
template <typename T> inline void RotateX(T &p, double angle) {
...
template <typename T> inline void RotateY(T &p, double angle) {
  const double costh = std::cos(angle);
  const double sinth = std::sin(angle);
...
template <typename T> inline void RotateZ(T &p, double angle) {
  const double costh = std::cos(angle);
  const double sinth = std::sin(angle);
...
template <typename T>
inline void LorentFramePrepare(const std::vector<T> &pf, const T &pbeam1,
                               const T &pbeam2, T &pb1boost, T &pb2boost,
                               std::vector<T> &pfboost) {
...
inline void LorentzFrame(std::vector<T> &pfout, const T &pb1boost,
  ...
  const std::vector<double> pb1boost3 = {pb1boost.Px(), pb1boost.Py(),
                                         pb1boost.Pz()};
  const std::vector<double> pb2boost3 = {pb2boost.Px(), pb2boost.Py(),
                                         pb2boost.Pz()};
  ...
  // @@ COLLINS-SOPER FRAME POLARIZATION AXIS DEFINITION @@
    ...
    zaxis = gra::matoper::Unit(gra::matoper::Minus(
        gra::matoper::Unit(pb1boost3), gra::matoper::Unit(pb2boost3)));
  }
  // @@ ANTI-HELICITY FRAME POLARIZATION AXIS DEFINITION @@
  else if (frametype == "AH") {
    zaxis = gra::matoper::Unit(gra::matoper::Plus(
        gra::matoper::Unit(pb1boost3), gra::matoper::Unit(pb2boost3)));
  }
  // @@ HELICITY FRAME POLARIZATION AXIS DEFINITION @@
  else if (frametype == "HE") {
    zaxis = gra::matoper::Unit(
        gra::matoper::Negat(gra::matoper::Plus(pb1boost3, pb2boost3)));
  }
  ...
  // y-axis
  if (frametype != "SR") {
    yaxis = gra::matoper::Unit(gra::matoper::Cross(
        gra::matoper::Unit(pb2boost3), gra::matoper::Unit(pb1boost3)));
  }

  // x-axis
  xaxis = gra::matoper::Unit(
      gra::matoper::Cross(yaxis, zaxis)); // x = y [cross product] z
  ...
  for (const auto &k : gra::aux::indices(pfout)) {
    const std::vector<double> p3 = {pfout[k].Px(), pfout[k].Py(),
                                    pfout[k].Pz()};
    const std::vector<double> p3new = R * p3; // Spatial part rotation Rp -> p'
    pfout[k] = T(p3new[0], p3new[1], p3new[2],
                 pfout[k].E()); // Full 4-momentum [px; py; pz; E]
  }
}
...
template <typename T> inline void CSframe(std::vector<T> &p) {
...
template <typename T> inline void GJframe(std::vector<T> &p, const T &q) {
  ...
  gra::kinematics::LorentzBoost(X, X.M(), propagator,
...
template <typename T> inline void HEframe(std::vector<T> &p) {
  // Sum to get the system 4-momentum
  T X(0, 0, 0, 0);
  for (const auto &i : gra::aux::indices(p)) {
    X += p[i];
...
//
// Input is a vector of final state 4-momentum
template <typename T> inline void SRframe(std::vector<T> &p) {
  // Sum to get the system 4-momentum
  T X(0, 0, 0, 0);
  for (const auto &i : gra::aux::indices(p)) {
    X += p[i];
...
struct MParticle {
  ...
  void print() const {
    ...
    std::cout << " J^PC:   " << aux::Spin2XtoString(spinX2) << "^"
              << aux::ParityToString(P) << aux::ParityToString(C) << std::endl
              << std::endl;
  }
};
...
public:
  ...
  void PrintParam(double sqrts) const {
    std::cout << rang::style::bold
              << "Custom resonance parameters:" << rang::style::reset
              << std::endl
              << std::endl;
    ...
    if (p.spinX2 != 0) {
      ...
      std::cout << std::endl;
      std::cout << "- Spin polarization density matrix [rho]:" << std::endl
                << std::endl;
      ...
      for (std::size_t i = 0; i < hc.rho.size_row(); ++i) {
        for (std::size_t j = 0; j < hc.rho.size_col(); ++j) {
          ...
          printf("%6.3f+i%6.3f%s ", std::real(hc.rho[i][j]),
                 std::imag(hc.rho[i][j]), delim.c_str());
        }
        std::cout << std::endl;
      }
      std::cout << std::endl << std::endl;
