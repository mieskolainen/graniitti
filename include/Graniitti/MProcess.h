...
public:
  ...
  virtual double operator()(const std::vector<double> &randvec,
                            AuxIntData &aux) = 0;
  virtual double EventWeight(const std::vector<double> &randvec,
                             AuxIntData &aux) = 0;
  ...
  // Set initial state
  void SetInitialState(const std::vector<std::string> &beam,
  ...
  void SetExcitation(int in) {
    if (in > 2 || in < 0) {
      std::string str = "MProcess::SetExcitation: Not valid input "
                        "(0,1,2) : " +
                        std::to_string(in) + " !";
  ...
  void SetFLATAMP(int in) {
    if (in > 0) {
      aux::PrintWarning();
      std::cout << rang::fg::red
                << "MProcess::SetFLATAMP: Flat matrix element FLATAMP : " << in
                << rang::fg::reset << std::endl;
    }
    FLATAMP = in;
  }
...
protected:
  ...
  void SampleForwardMasses(std::vector<double> &mvec,
  ...
  int ExciteContinuum(const M4Vec &nstar,
                      const HepMC3::GenParticlePtr gen_nstar,
                      HepMC3::GenEvent &evt, double Q2_scale, int B_sum,
                      int Q_sum);
  ...
  bool CheckInfNan(double &W) {
    ...
    if (N_nan > 50) {
      throw std::invalid_argument("MProcess::CheckInfNan: Too many NaN weights "
                                  "- Check model parameters "
                                  "and cuts!");
    }
    if (N_inf > 50) {
      throw std::invalid_argument("MProcess::CheckInfNan: Too many Inf weights "
                                  "- Check model parameters "
                                  "and cuts!");
    }
    return true;
  ...
  unsigned int EXCITATION =
      0; // Forward proton excitation (0 = off, 1 = single, 2 = double)
