...
public:
    ...
    MMatrix operator + (const MMatrix& rhs) const {
        ...
        if (rows != rhs.size_row() || cols != rhs.size_col()) {
            throw std::invalid_argument("MMatrix:: Matrix + Matrix with invalid dimensions");
        }
    ...
    // "safety" reasons (can lead to accidental results), thus only multiplicative operations.
    ...
    std::vector<T> operator * (const std::vector<T>& rhs) const {
        ...
        if (rhs.size() != cols) {
            throw std::invalid_argument("MMatrix:: matrix * vector product with invalid dimension on rhs");
        }
    ...
    MMatrix operator * (const MMatrix& rhs) const {
        ...
        if (cols != rhs.size_row())  {
            throw std::invalid_argument("MMatrix:: matrix * matrix product with invalid dimensions");
        }
