...
public:
  ...
  void Set(std::string _name, double _min, double _max, double _N,
           double _logarithmic) {
...
public:
  ...
  void S3GetRandomCutsBt(unsigned int &m, double &bt, T &rng) {
    ...
    thread_local std::uniform_int_distribution<unsigned int> randm(
        1,
        MCUT - 1); // 1,2,...
  ...
  template <typename T> unsigned int S3GetRandomCuts(T &rng) {
    ...
    // C++11, thread_local is also static
    thread_local std::uniform_real_distribution<double> flat(0, 1);
    thread_local std::uniform_int_distribution<unsigned int> RANDI(
        1, P_cut.size() - 1);
...
private:
  ...
  void S3CalculateArray(IArray1D &add,
