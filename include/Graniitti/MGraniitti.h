    ...
    for (std::size_t j = 0; j < FDIM; ++j) {
      ...
      for (std::size_t i = 0; i < param.BINS; ++i) {
        ...
        rvec[i] = std::pow((1.0 - f2mat[i][j] / dcache[j]) /
                               (std::log(dcache[j]) - std::log(f2mat[i][j])),
                           param.LAMBDA);
        ac += rvec[i];
      }
...
public:
  ...
  void ReadInput(const std::string &inputfile,
                 const std::string cmd_PROCESS = "null");
  ...
  void ReadProcessParam(const std::string &inputfile,
                        const std::string cmd_PROCESS = "null");
...
private:
  ...
  int Vegas(unsigned int init, unsigned int calls, unsigned int iter,
            unsigned int N);
