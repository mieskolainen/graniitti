...
inline double abs2(const std::complex<double>& M) {
	return pow2(std::abs(M));
}
...
constexpr double Deg2Rad(double deg) {
	return (deg / 180.0) * gra::math::PI;
}

// Radians to degrees
constexpr double Rad2Deg(double rad) {
	return (rad * 180.0) / gra::math::PI;
}
...
inline T CS38Integral(const std::vector<T>& f, double hstep) {
	...
	T I = 3 * hstep *
	      (f[0] + 3 * I_first + 2 * I_second + f[3 * N]) / 8;
	...
	if (type == 1) {
		...
		for (std::size_t i = 0; i < outset.size(); ++i) {
			...
			for (std::size_t j = 0; j < outset[0].size() - 1; j += 2) {
				...
				if ((charge.at(a - offset) +
				     charge.at(b - offset)) != 0)
					ok = false;
				// printf("%d%d", a, b);
			}
			if (ok) {
...
inline double LegendrePl(unsigned int l, double x) {
	...
	if        (l == 0) {
		...
		return (231 * x * x * x * x * x * x - 315 * x * x * x * x +
		        105 * x * x - 5) / 16;
	} else if (l == 7) {
		return (429 * x * x * x * x * x * x * x -
		        693 * x * x * x * x * x + 315 * x * x * x -
		        35 * x) / 16;
	} else if (l == 8) {
		return (6435 * x * x * x * x * x * x * x * x -
		        12012 * x * x * x * x * x * x +
		        6930 * x * x * x * x - 1260 * x * x + 35) / 128;
	} else {
...
// Algorithm from:
//
// [REFERENCE: S. Jemma, The Nitty Gritty Details of Spherical Harmonics, SIGRAPH2003 Paper]
...
// Manually written Spherical Harmonics to cross-check algorithmic implementations
inline std::complex<double> Y_complex_basis_ref(double costheta, double phi, int l, int m) {
	...
	// ------------------------------------------------------------------
	if (l == 1 && m == -1)
		return 0.5 * std::sqrt(3.0 / (2.0 * PI)) *
		       std::exp(-gra::math::zi * phi) * std::sin(theta);
	...
	if (l == 2 && m == -2)
		return 0.25 * std::sqrt(15.0 / (2.0 * PI)) *
		       std::exp(-2.0 * gra::math::zi * phi) *
		       std::pow(std::sin(theta), 2);

	if (l == 2 && m == -1)
		return 0.5 * std::sqrt(15.0 / (2.0 * PI)) *
		       std::exp(-gra::math::zi * phi) * std::sin(theta) * costheta;

	if (l == 2 && m == 0)
		return 0.25 * std::sqrt(5.0 / PI) *
		       (3.0 * std::pow(costheta, 2) - 1.0);
	...
	if (l == 3 && m == -1)
		return 1.0 / 8.0 * std::sqrt(21.0 / PI) *
		       std::exp(-gra::math::zi * phi) * std::sin(theta) *
	...
	if (l == 4 && m == -1)
		return 3.0 / 8.0 * std::sqrt(5.0 / PI) *
		       std::exp(-gra::math::zi * phi) * std::sin(theta) *
		       (7.0 * std::pow(costheta, 3) - 3.0 * costheta);

	if (l == 4 && m == 0)
		return 3.0 / 16.0 * std::sqrt(1.0 / PI) *
		       (35.0 * std::pow(costheta, 4) -
		        30.0 * std::pow(costheta, 2) + 3.0);
	...
	throw std::invalid_argument("Y_complex_basis_ref:: Not supported l = " + std::to_string(l) + ", m = " + std::to_string(m));
}
