...
//       which gives C (n x 1)
template <typename T>
inline std::vector<T> vv_vMultiply(const std::vector<std::vector<T>> &A,
...
template <typename T> inline std::vector<T> vabs(const std::vector<T> &x) {
  std::vector<T> y(x.size());
  for (std::size_t k = 0; k < x.size(); ++k) {
    y[k] = std::abs(x[k]);
  }
...
inline container_type<value_type> linspace(value_type start, value_type stop,
                                           std::size_t size) {
...
template <template <typename T> class container_type, class value_type>
inline container_type<value_type> arange(value_type start, value_type step,
                                         value_type stop) {
...
template <typename T> constexpr T pow3(T x) { return x * pow2(x); }
template <typename T> constexpr T pow4(T x) { return pow2(pow2(x)); }
template <typename T> constexpr T pow5(T x) { return x * pow4(x); }
template <typename T> constexpr T pow6(T x) { return pow2(pow3(x)); }
template <typename T> constexpr T pow7(T x) { return x * pow6(x); }
template <typename T> constexpr T pow8(T x) { return pow2(pow4(x)); }
...
// ||v||^2
template <typename T> inline double vpow2(const std::vector<T> &v) {
  double sum = 0.0;
...
// ||v||
template <typename T> inline double l2norm(const std::vector<T> &v) {
  double sum = vpow2(v);
  return msqrt(sum);
}
...
inline T CTrapIntegral(const std::vector<T> &f, double hstep) {
  if ((f.size() - 1) % 2 != 0) { // Must be even
    std::string str = "FATAL ERROR: gra::math::CTrapIntegral N = " +
                      std::to_string(f.size() - 1) + " is not even!";
...
inline T CSIntegral(const std::vector<T> &f, double hstep) {
  if ((f.size() - 1) % 2 != 0) { // Must be even
    std::string str = "FATAL ERROR: gra::math::CSIntegral N = " +
                      std::to_string(f.size() - 1) + " is not even!";
    throw std::invalid_argument(str);
  }
...
template <typename T>
inline T Simpson13Integral2D(const MMatrix<T> &f, const MMatrix<double> &W,
                             double hstepM, double hstepN) {
  if (((f.size_row() - 1) % 2 != 0) ||
      ((f.size_col() - 1) % 2 != 0)) { // Must be even
    std::string str = "FATAL ERROR: gra::math::Simpson13Integral2D M,N must be "
                      "even!";
    throw std::invalid_argument(str);
  }
...
template <typename T>
inline T Simpson38Integral2D(const MMatrix<T> &f, const MMatrix<double> &W,
                             double hstepM, double hstepN) {
  if (((f.size_row() - 1) % 3 != 0) || ((f.size_col() - 1) % 3 != 0)) {
    std::string str = "FATAL ERROR: gra::math::Simpson38Integral2D M,N must be "
                      "multiple of 3!";
    throw std::invalid_argument(str);
  }
...
inline double BESSJ0(double X) {
  ...
  VOL.5, 1962.
  ************************************************************************/
  double P1 = 1.0, P2 = -0.1098628627E-2, P3 = 0.2734510407E-4,
         P4 = -0.2073370639E-5, P5 = 0.2093887211E-6, Q1 = -0.1562499995E-1,
         Q2 = 0.1430488765E-3, Q3 = -0.6911147651E-5, Q4 = 0.7621095161E-6,
  ...
  if (AX < 8.0) {
    ...
    TMP = std::sqrt(0.636619772 / AX) *
          (FP * std::cos(XX) - Z * FQ * std::sin(XX));
  }
  return TMP;
}
...
inline double BESSJ1(double X) {
  ...
  VOL.5, 1962.
  ***********************************************************************/
  double P1 = 1.0, P2 = 0.183105E-2, P3 = -0.3516396496E-4,
  ...
  if (AX < 8.0) {
    ...
    FP = P1 + Y * (P2 + Y * (P3 + Y * (P4 + Y * P5)));
    FQ = Q1 + Y * (Q2 + Y * (Q3 + Y * (Q4 + Y * Q5)));
    TMP = std::sqrt(P6 / AX) * (std::cos(XX) * FP - Z * std::sin(XX) * FQ) *
          gra::math::sign(S6, X);
  }
  return TMP;
}
  ...
  if (type == 1) {
    ...
    for (std::size_t i = 0; i < outset.size(); ++i) {
      ...
      for (std::size_t j = 0; j < outset[0].size() - 1; j += 2) {
        ...
        if ((charge.at(a - offset) + charge.at(b - offset)) != 0)
          ok = false;
        // printf("%d%d", a, b);
      }
      if (ok) {
...
inline double LegendrePl(unsigned int l, double x) {
  if (l == 0) {
    ...
    return (231 * x * x * x * x * x * x - 315 * x * x * x * x + 105 * x * x -
            5) /
           16;
  } else if (l == 7) {
    return (429 * x * x * x * x * x * x * x - 693 * x * x * x * x * x +
            315 * x * x * x - 35 * x) /
           16;
  } else if (l == 8) {
    return (6435 * x * x * x * x * x * x * x * x -
            12012 * x * x * x * x * x * x + 6930 * x * x * x * x -
            1260 * x * x + 35) /
           128;
  } else {
    throw std::invalid_argument("LegendrePl: Not valid l = " +
                                std::to_string(l));
  }
}
...
inline double Y_real_basis(double costheta, double phi, int l, int m) {
  ...
  auto K = [](int l, int m) -> double {
    ...
    double temp = ((2.0 * l + 1.0) * factorial(l - m)) /
                  static_cast<double>(4.0 * PI * factorial(l + m));
    return msqrt(temp);
  };
  const double sqrt2 = msqrt(2.0);

  if (m < 0) {
    return std::pow(-1, m) * sqrt2 * K(l, -m) * std::sin(-m * phi) *
           sf_legendre(l, -m, costheta);
  } else if (m == 0) {
    return K(l, 0) * sf_legendre(l, m, costheta);
  } else { // m > 0
    return std::pow(-1, m) * sqrt2 * K(l, m) * std::cos(m * phi) *
           sf_legendre(l, m, costheta);
  }
}

// Complex Basis Laplace Spherical Harmonics
inline std::complex<double> Y_complex_basis(double costheta, double phi, int l,
                                            int m) {
  if (m < 0) {
...
// implementations
inline std::complex<double> Y_complex_basis_ref(double costheta, double phi,
                                                int l, int m) {
  const double theta = std::acos(costheta);

  if (l == 0 && m == 0)
    return 0.5 * std::sqrt(1.0 / PI);

  // ------------------------------------------------------------------
  if (l == 1 && m == -1)
    return 0.5 * std::sqrt(3.0 / (2.0 * PI)) * std::exp(-gra::math::zi * phi) *
           std::sin(theta);

  if (l == 1 && m == 0)
    return 0.5 * std::sqrt(3.0 / PI) * costheta;
  ...
  // ------------------------------------------------------------------
  if (l == 2 && m == -2)
    return 0.25 * std::sqrt(15.0 / (2.0 * PI)) *
           std::exp(-2.0 * gra::math::zi * phi) * std::pow(std::sin(theta), 2);

  if (l == 2 && m == -1)
    return 0.5 * std::sqrt(15.0 / (2.0 * PI)) * std::exp(-gra::math::zi * phi) *
           std::sin(theta) * costheta;

  if (l == 2 && m == 0)
    return 0.25 * std::sqrt(5.0 / PI) * (3.0 * std::pow(costheta, 2) - 1.0);
  ...
  if (l == 3 && m == -3)
    return 1.0 / 8.0 * std::sqrt(35.0 / PI) *
           std::exp(-3.0 * gra::math::zi * phi) * std::pow(std::sin(theta), 3);
  ...
  if (l == 3 && m == -1)
    return 1.0 / 8.0 * std::sqrt(21.0 / PI) * std::exp(-gra::math::zi * phi) *
           std::sin(theta) * (5.0 * std::pow(costheta, 2) - 1.0);

  if (l == 3 && m == 0)
    return 0.25 * std::sqrt(7.0 / PI) *
           (5.0 * std::pow(costheta, 3) - 3.0 * costheta);
  ...
  if (l == 4 && m == -4)
    return 3.0 / 16.0 * std::sqrt(35.0 / (2.0 * PI)) *
           std::exp(-4.0 * gra::math::zi * phi) * std::pow(std::sin(theta), 4);

  if (l == 4 && m == -3)
    return 3.0 / 8.0 * std::sqrt(35.0 / (PI)) *
           std::exp(-3.0 * gra::math::zi * phi) * std::pow(std::sin(theta), 3) *
           costheta;

  if (l == 4 && m == -2)
    return 3.0 / 8.0 * std::sqrt(5.0 / (2.0 * PI)) *
           std::exp(-2.0 * gra::math::zi * phi) * std::pow(std::sin(theta), 2) *
           (7.0 * std::pow(costheta, 2) - 1.0);

  if (l == 4 && m == -1)
    return 3.0 / 8.0 * std::sqrt(5.0 / PI) * std::exp(-gra::math::zi * phi) *
           std::sin(theta) * (7.0 * std::pow(costheta, 3) - 3.0 * costheta);
  ...
  throw std::invalid_argument("Y_complex_basis_ref:: Not supported l = " +
