...
public:
  ...
  void ResetBounds(int xbins, double xmin, double xmax, int ybins, double ymin,
                   double ymax);
  ...
  MH2 operator+(const MH2 &rhs) {
    ...
    MH2 h(this->XBINS, this->XMIN, this->XMAX, this->YBINS, this->YMIN,
          this->YMAX, this->name);
  ...
  void GetBounds(int &xbins, double &xmin, double &xmax, int &ybins,
...
private:
  ...
  int GetIdx(double value, double minval, double maxval, int nbins,
             bool logbins) const;
