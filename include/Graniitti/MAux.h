...
namespace aux {
	...
	*/
	...
	bool AssertRatio(T value, T reference, T threshold, const std::string& name = "",
		...
		if(!ok && dothrow) {
			throw std::invalid_argument("AsserRatio: Input '" + name + "' = " +
										std::to_string(value) + " not within reference = " +
										std::to_string(reference) + " under threshold = " +
	...
	bool AssertRange(T value, std::vector<T> range, const std::string& name = "",
		...
		if(!ok && dothrow) {
			throw std::invalid_argument(
				"AssertRange: Input '" + name + "' = " + std::to_string(value) + " out of range [" +
	...
	bool AssertSet(T value, std::vector<T> set, const std::string& name = "",
		...
		if(!ok && dothrow) {
			throw std::invalid_argument("AssertSet: Input '" + name + "' = " +
										std::to_string(value) + " not found from the input set");
		}
		return ok;
	}
