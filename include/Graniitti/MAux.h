...
template <typename T> inline Eigen::MatrixXd Matrix2Eigen(const MMatrix<T> &M) {
...
// String splitting
std::vector<std::string> SplitStr2Str(std::string input,
                                      const char delim = ',');
...
bool AssertRatio(T value, T reference, T threshold,
  ...
  if (!ok && dothrow) {
    throw std::invalid_argument(
        "AsserRatio: Input '" + name + "' = " + std::to_string(value) +
