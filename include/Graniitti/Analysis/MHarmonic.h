...
class MHarmonic {
  public:
	...
	struct HPARAM {
		...
		// integer)
		bool REMOVEODD = true; // Fix odd moments to zero (due to lacking spesific spin
		// states, for example)
	...
	void PlotFigures(const std::map<gra::spherical::Meta, MTensor<gra::spherical::SH>>& tensor,
					 unsigned int OBSERVABLE, const std::string& TYPESTRING, int barcolor,
					 const std::string& outputpath) const;

	void PlotFigures2D(const std::map<gra::spherical::Meta, MTensor<gra::spherical::SH>>& tensor,
