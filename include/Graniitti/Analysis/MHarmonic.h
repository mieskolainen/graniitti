...
public:
  ...
  struct EDGE {
    ...
    double center() const { return (max + min) / 2.0; }
  };
  ...
  struct HPARAM {
    ...
    int LMAX = 2; // Maximum spherical harmonic truncation degree (non-negative
                  // integer)
    bool REMOVEODD =
        true; // Fix odd moments to zero (due to lacking spesific spin
              // states, for example)
    ...
    double SVDREG =
        0.001; // SVD regularization strength in algebraic inverse (put 0
    ...
    void Print() {
      ...
      std::cout << "REMOVEODD       = " << (REMOVEODD ? "true" : "false")
                << std::endl;
      std::cout << "REMOVENEGATIVEM = " << (REMOVENEGATIVEM ? "true" : "false")
                << std::endl;
      std::cout << "EML             = " << (EML ? "true" : "false")
                << std::endl;
  ...
  void MomentFit(const gra::spherical::Meta &META,
                 const std::vector<std::size_t> &cell,
                 void (*fitfunc)(int &, double *, double &, double *, int));

  double PrintOutHyperCell(const gra::spherical::Meta &META,
                           const std::vector<std::size_t> &cell);
  void logLfunc(int &npar, double *gin, double &f, double *par,
                int iflag) const;
  bool PrintLoop(const std::string &output) const;
  void PlotAll(const std::string &outputpath) const;

  void Plot2DExpansion(
  ...
  void Plot1DEfficiency(unsigned int OBSERVABLE,
                        const std::string &outputpath) const;
