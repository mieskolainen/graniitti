...
inline dual logexp(dual z) {
  ...
  return 1.0 / alpha *
         log((1.0 + exp(alpha * z)) / (1.0 + exp(alpha * (z - 1.0))));
}
...
dual gaussprob(const VectorXdual &z, double mu, double sigma) {
  ...
  for (int i = 0; i < z.size(); ++i) {
    prod *= (1.0 / sqrt(2 * PI * sigma * sigma)) *
            exp(-(z[i] - mu) * (z[i] - mu) / 2.0);
  }
  return prod;
}
...
public:
  ...
  std::vector<double> RandomInit(NetParams &par, double sigma) {
    ...
    for (std::size_t l = 0; l < par.L.size(); ++l) {
      for (std::size_t i = 0; i < (unsigned int)par.L[l].w.rows(); ++i) {
        for (std::size_t j = 0; j < (unsigned int)par.L[l].w.cols(); ++j) {
          w[k] = randx.G(0, sigma) *
                 sqrt(2.0 / par.L[l].w.cols()); // He-et all style
          ++k;
        }
      }
    }
  ...
  static double CostGrad(const std::vector<double> &w,
  ...
  void NaiveGradDesc(std::vector<double> &w, unsigned int MAXITER,
                     double rate) {
    // -----------------------------------------------------------------------
    // Gradient Descent
