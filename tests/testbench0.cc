...
// Test Lorentz frame transformations
TEST_CASE("gra::kinematics::Frames", "[LorentzTransforms]") {
	...
	do {
		...
		// Random mother 4-momentum
		const double PX = rng.U(-100,100);
		const double PY = rng.U(-100,100);
		const double PZ = rng.U(-100,100);
		const M4Vec mother(PX, PY, PZ, sqrt( M0*M0 + sqrt(pow2(PX) + pow2(PY) + pow2(PZ)) ));
