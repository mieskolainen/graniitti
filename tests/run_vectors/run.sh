
#!/bin/sh
#
#
# Test for 2->6 amplitudes

#./bin/gr -i ./tests/run_vectors/test.json -p "PP[CONTENSOR]<F> -> rho(770)0 rho(770)0" -h 0
#./bin/gr -i ./tests/run_vectors/test.json -p "PP[CONTENSOR]<F> -> rho(770)0 > {pi+ pi-} rho(770)0 > {pi+ pi-}" -h 0

#./bin/gr -i ./tests/run_vectors/test.json -p "PP[CONTENSOR24]<F> -> rho(770)0 > {pi+ pi-} rho(770)0 > {pi+ pi-}" -h 0
#./bin/gr -i ./tests/run_vectors/test.json -p "PP[CONTENSOR24]<F> -> pi+ pi- pi+ pi-" -h 0

# -----------------------------------------------

./bin/gr -i ./tests/run_vectors/test.json -p "PP[CONTENSOR]<F> -> phi(1020)0 phi(1020)0" -h 0
#./bin/gr -i ./tests/run_vectors/test.json -p "PP[CONTENSOR]<F> -> phi(1020)0 > {K+ K-} phi(1020)0 > {K+ K-}" -h 0

#./bin/gr -i ./tests/run_vectors/test.json -p "PP[CONTENSOR24]<F> -> phi(1020)0 > {K+ K-} phi(1020)0 > {K+ K-} @OFFSHELL:15" -h 0
#./bin/gr -i ./tests/run_vectors/test.json -p "PP[CONTENSOR24]<F> -> K+ K- K+ K-" -h 0
